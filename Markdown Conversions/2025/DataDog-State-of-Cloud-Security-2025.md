...
Analyzing access key age over time also shows that organizations using long-lived credentials are struggling to rotate them. The percentage of access keys older than three years has mostly increased across all clouds. This increase is concerning, because as long as they exist and continue to grow stale, these access keys become increasingly risky.
...
To avoid mistakenly exposing S3 buckets, organizations should turn on S3 Block Public Access at the account level and protect the configuration with an SCP.

<a id="fact-6"></a>
## Fact 6: Overprivileged and misconfigured IAM roles for third-party integrations remain risks for AWS accounts

For this fact, we reviewed IAM roles trusting known AWS accounts that correspond to SaaS providers. We found that on average, an organization deploys 13 third-party integration roles (up from 10.2 in 2024).
...
Managed Kubernetes services (EKS, AKS, GKE) often lack adequate security in their default configurations. We found that the proportion of managed Kubernetes clusters exposing their managed API server to the internet remains relatively high: 39% of EKS clusters, 34% of AKS clusters, and 54% of GKE clusters are exposed to the internet.

For EKS, we determined that 13% of clusters have a dangerous node role that has full administrator access. In Google Cloud, 10% of GKE clusters have a privileged service account.

<a id="fact-8"></a>
## Fact 8: Organizations continue to run risky workloads

Assigning overprivileged permissions to EC2 instances or Google Compute Engine (GCE) virtual machines (VMs) continues to create substantial risk.
...
In Google Cloud, we found that 17% of VMs have full administrator “editor” permissions on the project they run in. In total, nearly one in four Google Cloud VMs (23%) has overly permissive access to a project.
...
![Methodology details regarding AWS Organizations, IAM condition keys, CloudTrail event analysis, and specific cloud provider resource configurations.]
