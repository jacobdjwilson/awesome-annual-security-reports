...
optimistic shift toward prioritizing AI security, with
...
Top 3 Sources of AI Attacks: Criminal Hacking Groups, Third-Party Service Providers, Freelance Hackers

Top 3 Motivations for AI Attacks: Data Theft, Financial Gain, Business Disruption

Disclosure & Transparency of AI Breaches

42% of IT leaders strongly agree that companies should be legally required to disclose AI-related security breaches to the public, but 45% of companies have opted not to report an AI-related security incident due to concerns about public backlash.

Rising Security Breaches and Vulnerabilities

88% of IT leaders are concerned about vulnerabilities in third-party AI integrations.

Top 3 Third-Party Gen AI Applications Currently In Use at Organizations: ChatGPT, Microsoft Co-Pilot, Gemini

Global Origins of AI Attacks

![Image description: Pie chart showing the global origins of AI attacks: North America 51%, South America 21%, Europe 34%, Africa 17%, Asia 32%, Unknown 14%.]
...
Top 3 Common Measures to Secure AI Include: Building relationships with AI & security teams, Creating an inventory of AI models, Determining sources of origins of AI models
...
Top 3 Frameworks Used to Secure AI Include: Google Secure AI Framework, IBM Framework for Securing Generative AI, Gartner AI Trust, Risk, and Security Management
...
42% of IT leaders believe the AI development team should be held accountable for errors, whereas 27% believe the security team should be held responsible.
...
FEB LeftoverLocals: Listening to LLM responses through leaked GPU local memory
...
APR Six critical vulnerabilities providing a full attack chain found in ClearML

APR Path traversal and out-of-bound read vulnerabilities disclosed in ONNXserialization format

MAY

MAY First model-stealing technique that extracts precise information from LLMs

MAY OpenSSF launches Model Signing Special Interest Group

JUN

JUN Arbitrary code execution vulnerability disclosed in R

JUN Arbitrary code execution and command injection vulnerabilities found in AWS Sagemaker

JUN OpenAI introduces GPT-4o

JUN Elaborate deepfake video scam attack against WPP

JUN LLM jailbreak backdoor published at ICLR conference

JUN Knowledge Return Oriented Prompting - new LLM prompt injection technique

JUL

JUL CTID launches the Secure AI research project

JUL Agility Robotics' Digit humanoid robot deployed in production at large factories

JUL Arbitrary code execution and XSS vulnerabilities found in Ydata-proﬁling

JUL Ten code execution vulnerabilities disclosed in MLFlow framework

AUG

AUG Coalition for Secure AI established under the OASIS global standards body

AUG NIST expands its AIRMF with the Generative Artiﬁcial Intelligence Proﬁle

SEP

SEP Deepfake clip of Kamala Harris shared by Elon Musk on X

SEP Critical vulnerability in Wyze camera enables researchers to bypass the embedded AI's object detection

SEP EU Artificial Intelligence Act enacted into force

SEP New GPU Memory Exploitation techniques unveiled at USENIX

SEP Two arbitrary code execution vulnerabilities found in LlamaIndex

SEP U.S., UK, and EU sign the Council of Europe’s Framework Convention on AI

OCT

OCT Microsoft shuts down ﬁrst cybercriminal service providing users with access to jailbroken GenAI

OCT Ten arbitrary code execution vulnerabilities and one critical WebUI vulnerability disclosed in MindsDB

OCT High severity vulnerabilities found in Autolabel, Cleanlab, and Guardrails
...
NOV Apple Intelligence release in the US
...
DEC UK establishes the Laboratory for AI Security Research (LASR)
...
Microsoft, Gemini 2.0 from Google, Chameleon from Meta,
...
automating attacks to spreading political misinformation.

PHISHING & SCAM

Since its inception, one of the predominant
...
attacks and their success rate. In the past year, we saw
several sophisticated phishing campaigns against Gmail
users using AI voice.

In one of these attacks, a phishing email requesting
...
impersonate their CEO in a Microsoft Teams call
with another executive. The incident was spotted
by WPP staff, but its sophistication was almost
unprecedented.

Deepfake scams can also happen outside of workplace
settings and target different aspects of people’s personal
...
face-swapping applications at their fingertips, attackers can
...
malware. These scripts, written in either VBScript or
...
sale, offering access to proprietary AI platforms that are
often jailbroken to allow the generation of restricted
...
The use of AI in political campaigning brings on
...
turbulence, terrorist attacks, and natural catastrophes.
These events attracted vast amounts of AI-generated
...
people of color supporting Trump in an attempt to
...
It's difficult to assess the level of influence that
...
attributing any manipulation attempts to a foreign influence
...
challenging task. Little legislation exists on this
...
the effects of AI interactions on human wellbeing.
...
hallucinations often are. With a growing number of
...
The recently launched Apple Intelligence service,
...
recording after a Zoom meeting ended, capturing
confidential discussions among venture capitalists.
Despite Otter AI's assurances about user privacy,
such occurrences highlight the risks associated
with AI technology in professional settings.
...
One of the most tragic results of emotional
...
*   Generative AI System Attacks - attacks against AI’s filters and restrictions intended to generate harmful or illegal content
...
![Image description: Diagram illustrating Attacks Against AI - Model Evasion. Training Data -> Training Process -> Trained Model. Input -> Decision Process -> Prediction.]

Early evasion techniques focused on minimally perturbed
adversarial examples, inputs modified so slightly that
humans wouldn't notice the difference, which caused the
model to produce an attacker-desired outcome. Recent
...
imperceptible noise, advanced attackers exploit the
...
*   The study "DiffAttack: Evasion Attacks Against Diffusion-Based Adversarial Purification" introduces a framework that effectively compromises diffusion-based defenses by inducing inaccurate density gradient estimations during intermediate diffusion steps.
*   "EvadeDroid: A Practical Evasion Attack on Machine Learning for Black-box Android Malware Detection" demonstrates a practical approach to evading black-box Android malware detection by constructing problem-space transformations from benign donors sharing opcode-level similarity with malware apps. Using an n-gram-based approach and query-efficient optimization, EvadeDroid successfully morphs malware instances to appear benign in both