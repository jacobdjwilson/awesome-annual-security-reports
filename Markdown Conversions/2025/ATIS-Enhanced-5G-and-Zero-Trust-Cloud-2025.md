...
1. **T1**: All data sources and computing services are considered resources
2. **T2**: All communication is secured regardless of network location
3. **T3**: Access to individual resources is granted on a per-session basis
4. **T4**: Access to resources is determined by dynamic policy
5. **T5**: The operator monitors and measures the integrity and security posture of all owned and associated assets
6. **T6**: All resource authentication and authorization are dynamic and strictly enforced before access is allowed
7. **T7**: The operator collects information about the current state of assets, network infrastructure and communications and uses it to improve its security posture
...
5G cloud infrastructure is the set of computation, storage, and networking equipment with related software that offers physical and/or virtual cloud resources and services to host 5G