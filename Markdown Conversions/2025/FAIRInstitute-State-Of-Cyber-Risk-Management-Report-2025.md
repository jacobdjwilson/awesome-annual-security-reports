# 2025 State of Cyber Risk Management Report

**Organization:** FAIRInstitute  
**Report Title:** State-Of-Cyber-Risk-Management-Report  
**Year:** 2025  
**Date:** June 26, 2025

The 2025 State of Cyber Risk Management Report examines how leading organizations are adapting their cyber risk programs to meet increasing business, regulatory, and operational demands. Based on a global survey of 402 cyber risk leaders and practitioners, the report offers a data-driven examination of the practices, technologies, and outcomes shaping the future of the discipline.

From quantification and automation to executive governance and third-party oversight, this report highlights the strategies mature organizations use to reduce uncertainty, drive alignment, and turn cyber risk into a source of business resilience.

©2025 FAIR Institute. All Rights Reserved. [www.FAIRInstitute.org](http://www.FAIRInstitute.org)

## Table of Contents
- [Executive Summary](#executive-summary)
- [Our Research Methodology](#our-research-methodology)
- [The Evolution of Cyber Risk Management](#the-evolution-of-cyber-risk-management)
- [Those with CRM Programs Show Moderate or Better Maturity](#those-with-crm-programs-show-moderate-or-better-maturity)
- [Cyber Risk Leaders Prioritize Business-Aligned Outcomes](#cyber-risk-leaders-prioritize-business-aligned-outcomes)
- [Cyber Risk Management Creates Value](#cyber-risk-management-creates-value)
- [CRM Maturity Shifts Cybersecurity Posture](#crm-maturity-shifts-cybersecurity-posture)
- [FAIR Success Leads to Better Outcomes](#fair-success-leads-to-better-outcomes)
- [The Technology C-Suite Benefits the Most](#the-technology-c-suite-benefits-the-most)
- [CRM Is Integrated with Enterprise Risk](#crm-is-integrated-with-enterprise-risk)
- [Organizations Tackle Third-Party Cyber Risk](#organizations-tackle-third-party-cyber-risk)
- [CRM Programs Automate as They Mature](#crm-programs-automate-as-they-mature)
- [CRM Automation Improves Business Outcomes](#crm-automation-improves-business-outcomes)
- [Programs Integrate with Non-Cyber Operations](#programs-integrate-with-non-cyber-operations)
- [Data Is the Lifeblood of Cyber Risk Management](#data-is-the-lifeblood-of-cyber-risk-management)
- [AI Is Not Limited to Experimental Use](#ai-is-not-limited-to-experimental-use)
- [Challenges and Gaps Persist](#challenges-and-gaps-persist)
- [The Future of Cyber Risk Management](#the-future-of-cyber-risk-management)
- [Participant Demographics](#participant-demographics)
- [Report Contributors and Reviewers](#report-contributors-and-reviewers)
- [About Our Sponsors and the FAIR Institute](#about-our-sponsors-and-the-fair-institute)

---

## Executive Summary
Conducted in May and June 2025 by the FAIR Institute with sponsorship from GuidePoint Security and SAFE, the 2025 State of Cyber Risk Management research provides a data-driven perspective on how mature organizations are addressing this challenge and where CRM is headed next.

Based on the responses from 402 CRM professionals around the globe, this report provides the following key takeaways:
- **CRM is fueling business results.** Top outcomes include improved alignment with the business, greater risk reduction, and optimized cybersecurity spending.
- **Those with mature CRM programs are more proactive and business-aligned.** High-maturity organizations are more likely to have board-approved risk tolerances, quantify risk in financial terms, embed CRM across business functions, and have a more proactive cybersecurity posture.
- **Factor Analysis of Information Risk (FAIR) and cyber risk quantification (CRQ) are gaining momentum.** Nearly 45% of organizations use or plan to use FAIR. Among adopters, 90% report success.
- **Technology-focused C-suite decision makers benefit most.** CTOs, CIOs, and CISOs, along with Chief Risk Officers, are the primary consumers of cyber risk information, utilizing it to inform their strategy, investments, and resource allocation.
- **Automation and AI are delivering scale and impact.** Seventy-two percent of organizations have mostly or completely automated their CRM systems, and 48% are utilizing AI for CRM. Both CRM automation and the use of AI are strongly correlated with maturity and improved outcomes.
- **Data is foundational.** Organizations use a wide variety of telemetry, threat, and compliance data to inform their decisions. Those who can operationalize this data gain a clearer and more defensible picture of their risk exposure.
- **Demand for CRM is growing, particularly among those with mature programs.** Nearly all (95%) respondents said internal demand for CRM is growing. Among those reporting high or very high CRM maturity, 23% indicate that demand will increase significantly.
- **The board sets expectations for risk management but is not sufficiently engaged.** Nearly all respondents have defined risk appetite and tolerance levels that are formally approved by the boards; however, boards consume cyber risk information in less than half of the participating organizations.
- **Challenges and gaps remain.** Cultural resistance, lack of executive support, and gaps in governance and metrics persist even among more advanced organizations.

## Our Research Methodology
The 2025 State of Cyber Risk Management study was designed to explore how organizations with established cyber risk management (CRM) programs are evolving their practices, adopting new technologies, and aligning with business priorities. The research was developed and fielded by the FAIR Institute in collaboration with Hanover Research.

The study focused exclusively on professionals directly involved in CRM execution, including CISOs, risk managers, cyber risk analysts, and related roles. To ensure meaningful insights, respondents who reported “We have no cyber risk management capabilities” were disqualified. All respondents were required to pass a simple knowledge screener.

A total of 402 qualified responses were collected in May and June 2025. The sample was global and cross-industry, capturing perspectives from organizations of varying sizes, sectors, and geographic regions.

## The Evolution of Cyber Risk Management
Cyber risk management (CRM) is the process of identifying, assessing, measuring, and responding[^1] to cyber risks. Like all risk management disciplines, CRM deals with uncertainty and probabilities. Cyber risks are a function of probable loss events and probable loss magnitudes.

CRM has evolved into a formalized discipline defined and codified in numerous industry standards and regulatory frameworks, including ISO/IEC 27005, the NIST Cybersecurity Framework, the SEC Cybersecurity Disclosure Rule, and the EU Digital Operational Resilience Act. Sometimes referred to as cybersecurity risk management[^2] or information security risk management[^3], CRM is increasingly regarded as an element of due care under prevailing case law in many jurisdictions.

[^1]: Responding to cyber risks, or risk response, is sometimes called risk treatment.
[^2]: Per NIST Interagency Report (IR) 8286.
[^3]: Per ISO/IEC 27005:2022.

## Those with CRM Programs Show Moderate or Better Maturity
Our sample represents organizations that likely have established CRM programs. None of our respondents reported low maturity.

| Maturity Level | % of Total |
| :--- | :--- |
| Low maturity | 0% |
| Moderate maturity | 26% |
| High maturity | 31% |
| Very high maturity | 43% |

## Cyber Risk Leaders Prioritize Business-Aligned Outcomes
CRM is tied to business priorities. When asked which outcomes matter most, respondents identified financial impact and cost management, risk reduction, and compliance with regulations/standards as their top three priorities.

> “CISOs increasingly must think about enabling the business while protecting it. Cyber risk management, particularly when quantified based on FAIR, offers an effective mechanism for grounding our risk decisions in business outcomes.”
> — Mary Elizabeth Faulkner, Chief Information Security Officer/VP of IT Operations, Thrivent, and FAIR Institute Board Director

## Cyber Risk Management Creates Value
CRM is delivering clear business value. Rather than operating in isolation, our data show that CRM is helping organizations achieve outcomes that align directly with their enterprise goals of risk reduction, strategic alignment, and budget optimization.

| Business Outcome | Lower Maturity | Higher Maturity |
| :--- | :--- | :--- |
| Alignment of cybersecurity resources with business priorities | 38% | 41% |
| Greater risk reduction for the organization | 33% | 42% |
| Optimized cybersecurity spending | 27% | 38% |
| Improved credibility of the cybersecurity team | 24% | 37% |

## CRM Maturity Shifts Cybersecurity Posture
CRM is also shifting how organizations approach cybersecurity at a strategic level. The data shows a clear relationship between CRM maturity and a proactive cybersecurity posture. Organizations reporting higher maturity are significantly more likely to describe their cybersecurity efforts as more proactive.

> “As a CISO, I’ve seen firsthand how maturity transforms cybersecurity from a checklist exercise into a dynamic part of business strategy. Proactive risk management gives us the ability to anticipate threats, support smarter decisions, and stay aligned with the speed of the business.”
> — Alexander Antukh, CISO, AboitizPower, and FAIR Institute Board Director

## FAIR Success Leads to Better Outcomes
FAIR is the industry-leading approach to cyber risk quantification and the only open standard for CRQ. Among those who have implemented FAIR, 90% report being successful with it.

| Top 3 Business Outcome | All Respondents | Financial Quantifiers | Very Successful with FAIR |
| :--- | :--- | :--- | :--- |
| Greater risk reduction | 40% | 41% | 54% |
| Improved credibility of team | 56% | 53% | 77% |
| Optimized spending | 58% | 59% | 65% |

## The Technology C-Suite Benefits the Most
CRM improves executive awareness and decision-making, primarily among those in technology-focused roles.

- **Chief Technology Officer (CTO):** 94%
- **Chief Information Security Officer (CISO):** 92%
- **Chief Information Officer (CIO):** 87%
- **Chief Risk Officer (CRO):** 76%
- **Chief Financial Officer (CFO):** 62%
- **Legal / General Counsel:** 58%
- **Board of Directors:** 45%
- **Business Units / Product Leadership:** 6%

## CRM Is Integrated with Enterprise Risk
Nearly all respondents communicate cyber risks to ERM, while nearly four in ten both communicate and manage cyber risks together with enterprise risks.

## Organizations Tackle Third-Party Cyber Risk
Every respondent in this year’s survey indicated that their CRM programs extend to their third-party risk processes, with 64% applying CRM strictly and 36% doing so loosely.

## CRM Programs Automate as They Mature
Cyber risk management is no longer a manual process. 72% of organizations have mostly or completely automated their CRM systems. The majority (56%) rely on special-purpose CRM software, outpacing general-purpose GRC platforms (24%).

## CRM Automation Improves Business Outcomes
Automation is effective for driving essential outcomes. Organizations that have mostly or completely automated their CRM systems were more likely to cite essential outcomes among their top three.

## Programs Integrate with Non-Cyber Operations
The most common integration points are technical, involving IT asset management (96%) and IT service management (95%). However, CRM is also extending into finance and accounting (81%) and legal and compliance (77%).

## Data Is the Lifeblood of Cyber Risk Management
Organizations utilize a diverse range of data sources to inform their CRM decisions, including endpoint security data (78%), cyber threat intelligence (77%), and compliance audit results (76%).

## AI Is Not Limited to Experimental Use
48% of organizations already use AI for CRM, while 34% are actively experimenting. AI adoption is strongly correlated with CRM maturity.

> “AI is fast becoming a pervasive enabler of innovation and productivity. But the speed at which businesses are now innovating and the uncertainties it brings are creating new challenges for CISOs.”
> — Omar Khawaja, Field CISO, Databricks and FAIR Institute Board Director

## Challenges and Gaps Persist
The three most cited challenges are poor communication between departments (37%), resistance from peers (34%), and lack of executive commitment (33%).

## The Future of Cyber Risk Management
CRM is entering a new era defined by risk quantification, increased automation, and business impact. Trends include:
- Growing internal demand for CRM.
- Expansion of FAIR and quantification.
- Institutionalization of automation and AI.
- Embedding CRM into enterprise governance.
- Regulatory pressure acting as an accelerant.

## Participant Demographics
![Chart showing respondent country, industry, annual revenue, organization size, respondent office, job level, and years of experience.]

## Report Contributors and Reviewers
- Dr. Chon Abraham, PhD
- Alexander Antukh
- Glen Armes
- Brandon Bapst
- Zach Cossairt
- Pankaj Goyal
- Michelle Grist
- Dana Haubold
- Abhishek Iyer
- Jack Jones
- Caleb Juhnke
- Tony Martin-Vegue
- Ben Moreland
- Steve Reznik
- Shaun Roberts
- Nick Sanna
- Eugene Teo
- Todd Tucker
- Jay Vinda
- Laura Voicu
- Michael Walters
- Denny Wan

## About Our Sponsors and the FAIR Institute
- **GuidePoint Security:** Provides trusted cybersecurity expertise, solutions, and services.
- **SAFE:** Founder and technical advisor of the non-profit FAIR Institute; specializes in Agentic AI for cyber risk management.
- **The FAIR Institute:** A non-profit professional organization dedicated to advancing the discipline of measuring and managing cyber and operational risk.

<!-- CONVERSION_METADATA: {"source": "https://github.com/jacobdjwilson/awesome-annual-security-reports", "date": "2026-07-20", "model": "gemini-3.1-flash-lite"} -->
