...
![New secrets detected on GitHub (millions) graph]
...
![Secrets by detector nature graph]
...
![Active public repositories containing secrets graph]
...
![MySQL credentials were not impacted by GitHub Push Protection graph]

![MongoDB Credentials were not impacted by GitHub Push Protection graph]
...
![Google API Keys were not impacted by GitHub Push Protection graph]
...
![Private vs public repositories graph]
...
![Top 10 specific secrets leaked on public GitHub in 2024 graph]
...
![Top 10 specific secrets in private repositories graph]
...
![Top 5 generic secrets leaked in public GitHub repositories graph]

![Top 5 generic secrets leaked in private repositories graph]

## Fastest Growing Services

![Fastest growing types of secrets graph]
...
![SCM systems vs collaboration tools graph]
...
![Secrets in SCM also found in collaboration tools graph]

| Collaboration Tool | Leak Rate        | Average Instances per Customer | Likelihood of Finding Secrets | Key Observations                                                                                                                            |
| :----------------- | :--------------- | :----------------------------- | :---------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------ |
| Slack              | 2.4% of channels | 1,500 channels per workspace   | Very High                     | Secrets appear frequently, often with multiple leaks per channel due to real-time messaging. Most organizations likely have multiple exposed secrets. |
| Confluence         | 0.5% of spaces   | 400 spaces per customer        | Moderate                      | Secrets are less common but still present, likely due to documentation misuse. Some organizations may have a few exposed secrets.               |
| Jira               | 6.1% of tickets  | 150 projects per customer      | High                          | Highest leak rate. Secrets are frequently shared in tickets, probably for troubleshooting. Many organizations have some exposed secrets.        |

![% of sources with at least one secret incident graph]
...
![Instructions where secrets were detected graph]
...
![Breakdown of valid secrets found in public Docker images graph]
...
![Number of repositories with Copilot enabled graph]
...
![Copilot enabled repositories leaking secrets graph]
...
![% of secrets still valid after exposure graph]
...
![Public repositories with and without secrets managers graph]
...
![Leaked Secret types in repositories leveraging secrets managers graph]
...
![Excessive permissions granted to API keys and tokens graph]
...
*   Discovery of leaked token
*   Additional secret discovery in artifacts
*   Cloud resource creation
*   Permission enumeration
*   Resource mapping
*   Access to build configurations
...
Impact Multiplication: Each downstream system potentially affected

Company developers -> Package Registry -> Build Systems -> Production Env
