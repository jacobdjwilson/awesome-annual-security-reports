...
**Organization:** Armis  
**Report Title:** Early-Warning-Insights-for-Software-Supply-Chain-Attacks  
**Year:** 2025
...
| JavaScript (npm) | eslint-scope | 2018 | v3.7.2, 5.0.2 | Post-install Pastebin exfiltration. | Token revocation, secure versions. |
...
| JavaScript (npm) | node-ipc | 2022 | v10.1.1, 10.1.2 | Geo-targeted file wiper (protestware). | Versions removed, project forked. |
...
| Python (PyPI) | jeIlyfish / python3-dateutil | 2019 | N/A | SSH/GPG key theft (typosquatting). | Removed from PyPI. |
...
| Ruby (RubyGems) | misc Ruby gems | 2019 | 10+ packages, Multiple | Miner/credential theft. | Yanked from RubyGems. |
...
| Gentoo | GitHub | 2018 | N/A | `rm -rf /` payload in ebuild scripts. | 2FA enforced, repo lockdown. |
...
1. **Provenance Tracking with Software Bills of Materials (SBOMs)**: Generate and cryptographically sign SBOMs for every build, ensuring each dependency’s origin and version are auditable. Determine through installation if any packages are directly used or indirectly used, for example packages.json and packages-lock.json. Ensure versions are specifically set.
2. **Application Security Testing**: Leverage Static application security testing (SAST) and dynamic application security testing (DAST) to find vulnerabilities that could leave your organization’s applications susceptible to attack.
3. **Assess Business Impact**: Which specific applications, features, or parts of your system are impacted?
4. **Remediate and Mitigate**: Determine who is (most likely) responsible for the asset and the remediation. Leverage existing workflows and tools, and include clear, actionable remediation guidance.
...
- **Description**: Malicious code was injected into Gravity Forms plugin versions 2.9.1.1 and 2.9.12 on the official download site. An unauthorized actor embedded `gravityforms/common.php` containing calls to a rogue domain `gravityapi.org`.
- **Impact**: Between July 9–10, 2025.
- **Early Warning Details**: Intelligence processes driven by deception technology identified the backdoor and call out to the bad domain. Gravity Forms released version 2.9.13 on July 11 and removed backdoors.
...
### Early Warning Name: GitHub Actions (tj-actions/changed-files)
- **Description**: From Nov 2024 to Mar 2025, a multi-stage attack compromised multiple GitHub Actions. Attacker first infiltrated `reviewdog/action-setup@v1` by replacing its tag to point to malicious code. That code executed in dependent workflows. From there, a leaked PAT was used to gain write access to `tj-actions/changed-files`.
- **Impact**: Affected 23,000 repos. A malicious commit was pushed into `index.js`, disguised as from `renovate[bot]`, dumping memory-secrets to logs.
...
- **Mitigation**: Sign commits & protect branches—prevent unauthorized merges/tags. Pin actions to commit SHA, not mutable tags. Minimize PAT use and scope & rotate frequently. Avoid `pull_request_target`, which runs workflows from forks with secret access.

### Early Warning Name: NPM Install (UA-Parser-JS Malware)
- **Description**: Any CI/CD pipeline, developer machine, or production system installing the compromised versions became infected.
- **Impact**: Developers using `npm install` on affected versions could unknowingly run postinstall scripts that executed malware. Supply chain impact extended to countless downstream projects and frameworks that relied on `ua-parser-js` directly or indirectly.
...
- **Mitigation**: Immediately remove the affected versions (0.7.29, 0.8.0, 1.0.0). Reinstall clean versions (0.7.30, 0.8.1, 1.0.1). Audit systems for compromise, particularly for processes like `xmrig` or signs of credential exfiltration. Rotate any potentially exposed credentials or secrets.
...
The concept of “indicators of action,” more commonly known as Indicators of Attack (IoAs), represents a pivotal shift from reactive defense to proactive threat detection. While traditional cybersecurity often relies on Indicators of Compromise (IoCs), IoAs focus on the behaviors, methods, and intent of an adversary as an attack is unfolding or even being prepared.

### System Behavior Indicators
- Unexpected CPU spikes (e.g., cryptomining activity)
- Increased or erratic memory usage
- Files written to unusual or hidden directories (e.g., `/tmp`, `%APPDATA%`)
- New executables or scripts dropped to disk
- Files self-deleting after execution
- Unusual system calls or process behaviors

### Privilege & Credential Abuse Indicators
- Attempts to escalate privileges (e.g., `sudo` or admin access)
- Access to environment variables or memory (e.g., to harvest secrets)
- Reading sensitive files like `.env`, `~/.ssh/`, or keychains
- Injection of code into privileged or running processes

### Network Behavior Indicators
- Outbound connections to unknown or suspicious domains
- Beaconing behavior (repeated low-volume traffic to the same destination)
- DNS tunneling or use of DNS over HTTPS (DoH)
- Communication over unusual ports
- Use or download of Tor for anonymous communication

### Build & Installation Indicators
- Presence of unexpected pre- or post-installation scripts
- Obfuscated or encoded code (e.g., Base64, split/join string building)
- Sudden increase in package size without justification
- Addition of new or suspicious dependencies
- Changes in compiled artifacts without corresponding source changes

### Metadata & Source Control Anomalies
- New or unfamiliar maintainers added to the project
- Tags being reassigned to different commits
- Commits appearing to be from trusted bots (e.g., `renovate[bot]`) but unsigned or suspicious
- Rapid release of many versions (e.g., version churn)
- Forked repositories with similar names used in place of trusted originals

---

## Mitigation Strategies

A multi-faceted approach to avoid Software Supply Chain Attacks is essential.

### Package & Supply-Chain Integrity
- **Verify AI-Suggested Dependencies**: Always cross-check any AI-recommended package name against official documentation or the package registry before installation.
...
- **Dependency pinning and SBOMs**: Maintain a Software Bill of Materials (SBOM) and pin all third-party dependencies.
- **Registry allow-listing**: Restrict package installs to approved registries (e.g., your private PyPI or NPM proxy) and block direct access to the public repositories.
- **Automated fuzzy-hashing and typo-pkg detection**: Integrate tools into your CI pipeline to flag packages whose names differ from popular ones by one or two characters.
- **Dependency Scanning**: Integrate vulnerability scanners into your CI/CD pipeline to flag unknown or untrusted packages.
- **Least-Privilege Installation**: Run dependency installation in isolated, ephemeral environments (containers or virtualenvs) with strict network egress controls.
- **Dependency Auditing**: Maintaining a comprehensive inventory of all open-source components used.
- **Prompt Patching and Update**: Establishing a swift and efficient process for applying security patches.

### Developer & User Awareness
- **Security training**: Educate developers and DevOps teams to double-check package names and domain URLs.
- **Phishing simulations**: Regularly run phishing/emails that mimic slopsquatting domains.
- **IDE and terminal plugins**: Provide tooling that warns when importing unknown packages.
- **Community Engagement and Due Diligence**: Staying informed about security advisories from open-source communities.
- **Security Best Practices in Development**: Encouraging secure coding practices within development teams.

---

## Armis Vulnerability Intelligence Database

In today’s threat landscape, timing is everything. Armis Vulnerability Intelligence Database is a revolutionary, AI-powered vulnerability intelligence resource that goes well beyond traditional static databases. While many teams rely on the CISA Known Exploited Vulnerabilities (KEV) catalog to track risk, Armis Vulnerability Intelligence Database is built to extend that value by offering real-time context, community-driven prioritization, and actionable insights tailored to specific industries and threat levels.
...
CISA’s KEV catalog has played an important role in helping organizations prioritize known threats. But KEV documents vulnerabilities after they’ve been exploited in the wild. Armis layers in industry-specific context, prioritization signals, mitigation recommendations, and even identifies threats that haven’t yet reached public disclosure.

Additionally, Armis has been authorized by the Common Vulnerabilities and Exposures (CVE®) Program as a CVE Numbering Authority (CNA).
...
Armis Centrix™ for Early Warning combines asset intelligence with vulnerability intelligence. Asset intelligence shows you “what needs protecting,” while vulnerability intelligence reveals “what’s coming to attack it.” This empowers proactive, risk-based cybersecurity.
...
![Image: Armis Labs research and asset intelligence capabilities]
