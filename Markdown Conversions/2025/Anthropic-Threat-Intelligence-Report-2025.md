...
- **Phase 4: Data exfiltration and analysis**: Claude Code facilitated comprehensive data extraction, organizing stolen data for monetization purposes.
...
3. **Employment maintenance**: Delivering technical work, participating in team communications, and responding to code reviews.
4. **Revenue generation**: Funding weapons programs through salaries earned from multiple concurrent positions.

### Mitigation
We banned the accounts associated with this violative activity. Following this case, we improved our tooling for collecting, storing, and correlating known indicators of compromise from both public and private information sharing.
...
- **Delivery**: Reflective DLL injection and code cave infection.

### Implications
Traditional ransomware development required technical expertise in areas like cryptography and Windows internals. Our investigation reveals AI has effectively removed this barrier, allowing actors who cannot independently implement basic encryption to create and sell novel malware.

---

## Chinese Threat Actor Leveraging Claude Across Nearly All MITRE ATT&CK Tactics

We identified and investigated a sophisticated Chinese threat actor who systematically leveraged Claude to enhance cyber operations targeting Vietnamese critical infrastructure. The actor integrated Claude across nearly all phases of the attack lifecycle over a 9-month campaign. The actor used Claude as a technical advisor, code developer, security analyst, and operational consultant to develop custom scanning tools, optimize credential harvesting, and implement privilege escalation exploits.
...
We discovered a Russian-speaking developer using Claude to create malware with advanced evasion capabilities. The actor demonstrated strong Windows internals knowledge but relied heavily on Claude for implementation, including Hell’s Gate syscall resolution and Early Bird process injection.
...
A threat actor used Model Context Protocol (MCP) and Claude to process stealer logs, categorize domains, and build comprehensive behavioral profiles based on online activities.

### Carding Store Powered by AI
A Spanish-speaking actor used Claude Code to maintain an invite-only web service specializing in validating and reselling stolen credit cards, utilizing multi-API resilience frameworks and intelligent request throttling.

### Romance Scam Bot Powered by AI Models
We identified a Telegram bot providing multimodal AI tools specifically marketed to support romance scam operations, using Claude as a “high EQ model” for emotionally intelligent responses.

### Synthetic Identity Services Powered by AI
We discovered an actor who successfully launched an operational synthetic identity service using Claude for various components of their infrastructure, including automated API resilience and evasion techniques.
