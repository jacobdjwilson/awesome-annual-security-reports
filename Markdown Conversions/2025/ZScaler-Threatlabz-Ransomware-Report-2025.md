...
Ransomware attacks on the **Oil & Gas** sector have spiked more than 900% year-over-year, likely driven by a combination of increased automation—from drilling rigs to pipelines—inflating the attack surface and outdated security practices. In 2025, the Cybersecurity and Infrastructure Security Agency (CISA) warned[^1] that even “unsophisticated” threat actors are exploiting vulnerabilities in industrial control systems (ICS) and SCADA technology.

The **Agriculture** sector also saw ransomware attacks skyrocket by 677.4%. Farming operations are becoming more and more digitized (think smart tractors and IoT-enabled sensors), yet security defenses haven’t kept pace.

**Healthcare** remains one of the most frequently and consistently targeted, with attacks rising steadily 115.4% year-over-year. According to The HIPAA Journal,[^2] researchers have found that ransomware is now one of the leading causes of healthcare data breaches.
...
[^2]: The HIPAA Journal, Study Explores Extent of Hacking and Ransomware Attacks in Healthcare, May 16, 2025.
...
Ransomware attacks surged worldwide, with the top 15 countries by number of ransomware attacks experiencing double- and triple-digit percentage increases (see figure 9). Attacks in the US more than doubled to 3,671 attacks. Canada’s 194.5% increase reinforces how threat actors are expanding across North America.

![Figure 9: Year-over-year comparison of ransomware attacks by country]

#### EMEA: Intensifying Attacks in Europe and Beyond
The Europe, Middle East, and Africa (EMEA) region experienced widespread increases in ransomware activity, as shown in figure 10. Israel saw a 436.4% spike, a sharp relative increase that likely reflects growing geopolitical tensions and a rise in state-linked ransomware operations.
...
The Asia-Pacific (APAC) region recorded some of the highest year-over-year increases globally. As figure 11 shows, ransomware actors broadened their footprint across a region characterized by fast-paced digital transformation and inconsistent levels of cyber maturity.
...
Several highly active groups continued to dominate the ransomware ecosystem, with RansomHub leading the pack, claiming the highest number of publicly named victims at 833. Akira, associated with 520 victims, has steadily expanded its reach through numerous affiliates and initial access brokers. Clop, known for its focus on supply chain attacks, is close behind with 488 victims.
...
*   **Virtualization Software**: Hypervisors such as VMware ESXi concentrate workloads, making them high-value ransomware targets.
*   **VPNs**: VPN vulnerabilities typically provide attackers direct access to an organization’s internal network.
*   **Remote Access Tools**: RMM tools are critical in corporate IT environments but are common targets by ransomware groups.
*   **Backup Solutions**: Vulnerabilities in these systems allow direct access to steal information and disable backup solutions prior to file encryption.
*   **File Transfer Applications**: When exposed to the internet, any vulnerability present in the underlying application can be exploited to access the organization’s data.
...
There has been a shift away from initial access brokers using large-scale spam campaigns as the initial infection vector. Instead, more targeted attacks have been used effectively over the last two years.

Threat actors first perform reconnaissance using sites like ZoomInfo and LinkedIn to purchase contact information about employees at a targeted company. The attack kicks off with “spam bombing” to flood a victim's inbox. The threat actor then makes a phone call to that employee, impersonating an IT department member—a classic example of vishing. The caller requests remote access through an RMM tool (e.g., Microsoft Teams, Quick Assist, TeamViewer). If the victim falls for the ruse, the threat actor installs a backdoor.
...
Ransomware’s playbook in the healthcare sector has evolved with a new prescription: exfiltrate first, disrupt second. Because patient trust is critical in healthcare, a public breach can be as damaging as downtime. In 2024, the US Department of Health and Human Services Office for Civil Rights (OCR) reported 725 breaches involving 500 or more individuals, with around 275 million healthcare records exposed.

---

## Turning the Ransomware Tide: From the Front Lines

Despite the significant increase in ransomware attacks, there are success stories. Collaboration between global law enforcement and private industry researchers has proven effective.

*   **DecryptHub**: In early 2025, ThreatLabz discovered a vulnerability in RansomHub that made file decryption possible without paying a ransom. The team developed DecryptHub, a portal that helped dozens of victim organizations recover files.
*   **Operation Endgame**: A global initiative aimed at dismantling cybercriminal organizations and the malware ecosystem. Zscaler ThreatLabz has been supporting this operation over the past two years, providing vital resources and intelligence.
...
1.  **Dark Angels**: Known for highly lucrative, targeted attacks on large organizations. They exfiltrate vast amounts of data and avoid debilitating organizations to fly under the radar.
2.  **Clop/Cl0p**: Has shifted away from file encryption, focusing on data extortion via supply chain attacks exploiting zero-day vulnerabilities in file transfer applications.
3.  **DragonForce**: A group that has shown significant growth in exfiltration volume and victim count.
4.  **Akira**: Has steadily expanded its reach through numerous affiliates and initial access brokers.
5.  **Interlock**: A rising threat gang noted for its impact on the healthcare sector.
