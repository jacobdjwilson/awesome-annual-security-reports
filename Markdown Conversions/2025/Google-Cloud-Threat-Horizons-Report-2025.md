...
Organization: Google  
Report Title: Cloud-Threat-Horizons-Report  
Year: 2025
...
![Figure 1. H1 2025 Distribution of Initial Access Vectors Exploited by Threat Actors. A pie chart showing: Weak or absent credentials (47.1%), Misconfiguration (29.4%), API/UI compromise (11.8%), Leaked credentials (2.9%), Remote code execution (2.9%), Software issue (2.9%), and Other (2.9%).]
...
| Resiliency | Backup / Recovery Architecture | Advantages | Considerations | Recovery Time |
| :--- | :--- | :--- | :--- | :--- |
| **Highest** | **Isolated Recovery Environment** | • Highest resiliency to destructive attacks<br>• Backup data unlikely to be directly accessible by an attacker<br>• Backup systems and dependent infrastructure is isolated and pre-staged<br>• Integrated validation capabilities for assurance | • Additional operational and administrative planning, governance, and maintenance requirements | **Shortest** |
| | **Isolated Data Vault** | • Backup data unlikely to be directly accessible or modifiable by an attacker | • Moderate resiliency to destructive attacks<br>• Requires functioning (available) production backup servers to access backup data<br>• If suspected to be compromised, forensic examination may delay availability of production systems for secure data restoration<br>• Restoration of backup platforms (and dependent systems) will likely delay restoration efforts | |
| **Lowest** | **Online / Production Integrated Backups** | • Low capital and operational expenses<br>• Ease of integration and management | • Could provide direct access to backups for an attacker, based on the interconnected nature of the backup platforms and credentials that are integrated with a centralized cloud identity provider<br>• Requires functioning (available) production backup servers to access backup data<br>• If suspected to be compromised, forensic examination may delay availability of production systems for secure data restoration<br>• Restoration of backup platforms and dependent systems will likely delay restoration efforts | **Longest** |
...
![Figure 3. UNC4899 Attack Lifecycle in a Victim’s Google Cloud Environment. Stages: Initial Compromise (Social Engineering, Malicious Docker), Establish Foothold (gcloud CLI, PLOTTWIST), Escalate Privilege (Victim account had higher privileges), Internal Reconnaissance (Enumerated compute instances, gcloud CLI), Move Laterally (SSH to other instances, stolen cookies), Maintain Presence (PLOTTWIST, modified shell configs), Complete Mission (Cryptocurrency theft).]
...
![Figure 4. UNC4899 Attack Lifecycle in a Victim’s AWS Environment. Stages: Initial Compromise (Social engineering, fake job, Docker), Establish Foothold (MAZEWIRE, GLASSCANNON), Escalate Privilege (Stolen STS session cookies), Internal Reconnaissance (Enumeration of AWS resources, EKS environment), Move Laterally (Access across production/dev environments, Slack access), Maintain Presence (Backdoor, execution on EKS pods), Complete Mission (Uploaded malicious JS to S3, Cryptocurrency theft, Cleanup).]
...
![Figure 5. Common Kill Chain for Decoy File Distribution in Cloud Storage Services. Stages: Weaponization (Attacker uploads files to cloud storage), Delivery (Attacker distributes initial payload), Exploitation (Initial sample downloads legitimate PDF from cloud storage as decoy), Installation, C2, and Actions on Objectives.]
...
![Figure 6. APT42 Credential Harvesting Campaign Attack Lifecycle. Step 1: Spear-phishing emails with malicious links. Step 2: Fake website posing as legitimate services (e.g., Washington Post) with a decoy document. Step 3: Fake Google/Microsoft login page for credential harvesting.]
...
![Figure 7. Example of Decoy File. A document titled "ADVISORY NO 13" regarding "OPPORTUNITY FOR EXERCISE/ RE-EXERCISE OF OPTION FOR PAY FIXATION" addressed to various Headquarters.]
...
- **User Awareness and Social Engineering Defense**: Conduct frequent, engaging security awareness training that highlights the sophistication of social engineering tactics. Emphasize that even seemingly legitimate links or attachments from familiar cloud services can be malicious, especially when combined with urgent or enticing pretexts.
...
However, API access using OAuth tokens, a form of non-human identity, is not subject to these same security advancements. If a developer grants the OAuth scope that allows the extension management permission to the OAuth client associated with a malicious project, that malicious OAuth client will be able to leverage the account owner’s established reputation to widely distribute a malicious extension.
...
- **Promote Secure Developer Key Management Practices**: The Verified CRX Upload feature is currently opt-in, allowing developers to transition to a more secure workflow. To ensure added security is effective, developers should store their private key in a highly secure location, independent of their Google account credentials. The private key must not be uploaded to public repositories or stored in personal cloud storage, such as Google Drive, because if threat actors compromise developer account credentials, they would also gain access to this key. Storing the private key securely using a dedicated keystore solution, like PKCS#12 or Java Keystore, is strongly recommended.

![Figure 8. How Verified CRX Upload Helps Protect Users. A comparison diagram. Before Verified Upload: Compromised uploader can bypass code review and reach users. After Verified Upload: Key mismatches prevent the attack from reaching the Chrome Web Store and users.]
