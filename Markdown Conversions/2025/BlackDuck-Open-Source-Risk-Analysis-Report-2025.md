...
![Infographic showing 97% of codebases contained open source, 70% of scanned code had its origin in open source, 64% of OSS components were transitive dependencies, and the number of open source files in an average application has tripled in the last four years.]

![Table showing origin repositories for open source components including npm, yarn, pnpm, Cargo, Nuget, go_mod, Maven, packagist, and Gradle.]

### Vulnerabilities and Security
![Chart showing 81% of risk-assessed codebases contained high- or critical-risk vulnerable open source, 86% of top 10 high-risk vulnerabilities were found in jQuery.]
...
- 3,548: Maximum number of unique vulnerabilities found in a single codebase.
- 154: Mean number of unique vulnerabilities per codebase.
...
- **Code scanning**: Utilizing manifest, binary, hybrid, and snippet scanning.
...
- **SBOM generation**: Creating records in formats like SPDX or CycloneDX.
- **Continuous monitoring**: Keeping the SBOM up-to-date.

## Analyzing the Impact of a Vulnerability
Reachability and impact analysis helps determine if a vulnerability exists on a “busy road” within your application’s code, making it more likely to be exploited.
...
The Log4Shell vulnerability of 2021 and the Equifax breach of 2017 are reminders of the importance of visibility into the open source you’re using.
...
The prevalence of CWE-79 in our results, and all the CVEs related to cross-site scripting exploits of jQuery vulnerabilities, highlights the critical importance of input validation in web development. jQuery is not inherently insecure, but it is the component most likely to have vulnerabilities in our audits, usually due to the use of outdated versions.
...
- 56% of codebases had license conflicts.
- 33% of codebases contained open source with no license or a custom license.

## How Conflicts, Variants, and Lack of Licenses Create Risk
A declared license conflict arises when the license of an open source component clashes with the overall license declared for the entire project. Variants or customized versions of standard open source licenses can also place undesirable requirements on the licensee.

## The Impact of Transitive Dependencies on License Conflicts
Nearly 30% of component license conflicts found in our audits were caused by transitive dependencies.
...
- **Low-Risk (Permissive)**: Generally do not have many limiting conditions (e.g., MIT, Apache).
- **Medium-Risk (Weak Copyleft)**: Include a reciprocity obligation for modifications (e.g., Mozilla Public License).
- **High-Risk (Reciprocal/Copyleft)**: Restrictive licenses that may require releasing proprietary software under the same terms (e.g., GPL).

## How to Manage Open Source License Risk with SCA
The first step to managing risk is using an automated SCA tool to create an up-to-date, accurate SBOM of all open source components in your software.

## Industry Perspectives on License Conflicts
![Figure 2: Codebases Containing License Conflicts by Industry (reproduced)]
...
Mitigate legal
Avoid surprises may affect software
exposure
asset values
Resolve potential issues Build appropriate Plan integration and
before they affect the protections into the remediation of seller/
transaction deal terms buyer code
...
90% 91% 79% 88%
of codebases of codebases have of codebases contain of codebases have
contain open source components that components with no activity components with no activity
components that are have not seen new for the last 24 months, while for the last 24 months and
more than four years development in the past still using the latest version are not using the latest
out-of-date two years of the component version of the component
