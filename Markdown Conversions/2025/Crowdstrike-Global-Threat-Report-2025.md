...
## Introduction
The CrowdStrike 2025 Global Threat Report is the industry’s preeminent source on adversary intelligence, examining the emerging adversary trends of the past year. During 2024, adversaries matured faster than ever, innovating techniques and tools as well as finding creative solutions to circumvent modern defenses, all while staying laser-focused on their targets. 2024 was the year of the enterprising adversary.

eCrime adversaries exemplified such enterprising cyberattacks, constantly adapting to shifting environments and quickly scaling effective operations. Throughout 2024, initial access techniques began to shift — eCrime adversaries began moving away from phishing to alternative access methods. One such technique that proliferated in 2024 is social engineering leveraging telephony-based exploitation: Various eCrime adversaries are increasingly adopting vishing, callback phishing, and help desk social engineering attacks to gain a foothold into networks.

In 2024, China’s cyber espionage operations reached new levels of maturity, with adversaries maintaining a higher operational tempo than observed in 2023. CrowdStrike graduated seven new China-nexus adversaries and observed a 150% increase in China-nexus activity across all sectors on average compared to 2023.

![Graphic showing 26 new adversaries named in 2024, 257 total adversaries tracked, and 140 active malicious activity clusters.]
...
Cyberattacks are escalating in speed, volume, and sophistication. In 2024, 79% of the detections CrowdStrike observed were malware-free, indicating adversaries are instead using hands-on-keyboard techniques that blend in with legitimate user activity and impede detection.

![Infographic summary of 2024 threat statistics including breakout times, vishing growth, and malware-free detection rates.]

### The Growing Reliance on Identity Attacks and Vulnerability Exploits
Every breach starts with initial access, and identity-based attacks are among the most effective entry methods. Access broker activity surged in 2024, with advertised accesses increasing by nearly 50% over 2023. Meanwhile, valid account abuse was responsible for 35% of cloud-related incidents.

### The Continued Rise of Interactive Intrusions
Modern cyber threats are increasingly dominated by “interactive intrusion” techniques, where adversaries execute hands-on-keyboard actions to achieve objectives. In 2024, CrowdStrike observed a 35% year-over-year increase in interactive intrusion campaigns.
...
In 2024, CrowdStrike Intelligence observed a massive increase in the number of distinct campaigns using telephone-oriented social engineering techniques to gain initial access, including vishing and help desk social engineering.

### Generative Artificial Intelligence and the Enterprising Adversary
GenAI has emerged as an attractive tool for adversaries with a low barrier to entry. Adversaries increasingly adopted genAI throughout 2024, particularly in support of social engineering efforts and high-tempo IO campaigns.

### China’s Cyber Enterprise
China’s cyber espionage and intelligence collection capabilities reached an inflection point in 2024. China-nexus intrusions increased 150% across all sectors on average compared to 2023 and represent the most active targeted intrusion threats CrowdStrike Intelligence tracks.

### Cloud-Conscious Threat Actors Continue to Innovate
In 2024, new and unattributed cloud intrusions increased 26% compared to 2023. CrowdStrike observed more intrusions in which attackers gained initial access via valid accounts, leveraged cloud environment management tools for lateral movement, and abused cloud provider command line tools.
...
provide attackers with instant access to cloud credentials and email lists that can IN THE FIRST HALF OF 2024.
...
ERROR: INVALID ERROR: VALID SUCCESSFUL AUTOMATED
LOGIN EXFILTRATION
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 33
...
Credentials managers STORED
CREDENTIALS Access to new accounts
VALID
Actor accesses a valid account ACCOUNT
...
from compromised
VMs with cached
credentials VM management tools
...
Add SSH keys to VM PERSISTENCE de M p a l l o w y a m r e e n t
HOST-BASED
PERSISTENCE
...
Threat actor accesses cloud infrastructure CLOUD INFRASTRUCTURE
COLLECTION
directly for collection
Mining software
Create new resources for later use deployment
RESOURCE
HIJACKING
Ransomware
...
CVE-2024-5910 CVE-2024-9465
| Disclosure | First POC | Exploitation |     |     |     |     |
| ---------- | --------- | ------------ | --- | --- | --- | --- |
CVE-2024-9463 CVE-2024-9466
|     | O C T   9         | O C T   1 0     | O C T   11          | O C T   1 2                |     |     |
| --- | ----------------- | --------------- | ------------------- | -------------------------- | --- | --- |
|     | P u b l i c       | Fi r s t   P OC | Te c h n i cal Blog | F i r s t   O b s e r ved  |     |     |
|     | D i s c l o s ure |                 |                     | E x p l o i t a t io n     |     |     |
O C T   9
P u b l i c   D i s c l o s u re  O C T   1 1   O C T   1 8
|     |     |     | F i r s t   O b s e r ved  |     | F i r s t   O R | B 0 1   |
| --- | --- | --- | -------------------------- | --- | --------------- | ------- |
T e c h n i c a l   B l o g   E x p l o i t a t io n E x p l o i t a ti o n
Fi r s t   P O C
|     | O C T   9                        |     |     | O C T   1 | 4                 |     |
| --- | -------------------------------- | --- | --- | --------- | ----------------- | --- |
|     | P u b l i c   D i s c l o s ure  |     |     | F i r s t |   O b s e r v ed  |     |
E x p l o i t a t io n
|     | Te c h n i c a l   B l o g |     |     | A t t e m | p t s |     |
| --- | -------------------------- | --- | --- | --------- | ----- | --- |
JUL OCT
2024
...
|     |     | SMS DISTRIBUTION  | ENABLES THREAT ACTOR TO       |     |
| --- | --- | ----------------- | ----------------------------- | --- |
|     |     | APPLICATION       | DISTRIBUTE SMISHING MESSAGES  |     |
TO THIRD PARTIES
THREAT ACTOR COMPROMISED  DOCUMENT MANAGEMENT AND  ENABLES THREAT ACTOR TO
|     | SSO IDENTITY | STORAGE APPLICATION | IDENTIFY SENSITIVE DATA  |     |
| --- | ------------ | ------------------- | ------------------------ | --- |
FOR EXFILTRATION
|     |     | CREDENTIAL MANAGEMENT  | ENABLES THREAT       |     |
| --- | --- | ---------------------- | -------------------- | --- |
|     |     | APPLICATION            | ACTOR TO COMPROMISE  |     |
PRIVILEGED ACCOUNTS
...
This campaign targeted only the organizations’ database instances, and no INTERNALLY VIA EMAIL, MAKING THESE
lateral movement or malicious activity impacting other applications or systems RESOURCES VALUABLE TARGETS FOR
was observed. Threat actors could apply this tradecraft to steal data from other ADVERSARIES SEEKING TO PIVOT
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 43
Conclusion
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 44
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 45
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 47
CCrroowwddSSttrriikkee
FFaallccoonn PPllaattffoorrmm
AI and Cloud-Native
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 48
CCrroowwddSSttrriikkee PPrroodduuccttss
Endpoint Security
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 51
CCrroowwddSSttrriikkee SSeerrvviicceess
INCIDENT RESPONSE
...
CROWDSTRIKE 2025 GLOBAL THREAT REPORT 53
AAbboouutt
CCrroowwddSSttrriikkee
