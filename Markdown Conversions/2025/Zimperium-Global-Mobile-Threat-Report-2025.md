...
- **Rigorous, Continuous App Vetting**: Enforce strict processes for analyzing third-party apps.
- **Combat Mishing**: Implement AI-enabled mobile threat defense and provide regular employee training.
- **Mobile Vulnerability Management**: Define and enforce policy on timely OS and app updates.
- **Secure Your Third-Party Code**: Analyze app binaries early during development.
- **Mandate Device Attestation**: Ensure mobile app development teams implement device attestation.

---

## The Expanding Enterprise Mobile Footprint: Scale Meets Risk

As of the end of 2024, there are approximately 7.2 billion smartphone users worldwide[^1]. Mobile devices now routinely use apps to access sensitive systems, data, and workflows. By the end of 2024, there were around 1.96 million apps on the Apple App Store and 2.87 million on Google Play. A typical user has between 80 and 100 apps installed[^2], yet only 11 are work-related[^3]. Meanwhile, 66% of American employees use their personal smartphones for work[^4], and 70% of organizations support BYOD[^5].
...
- **Application Risks**: Malicious, unexpected, and/or risky behavior from sideloaded apps.
- **Mishing Attacks**: Malicious links embedded in mishing content.

### Android Threats
- **Device Vulnerabilities**: Rooting or privilege escalation, failure to patch OS vulnerabilities.
- **Network Risks**: Untrusted or insecure Wi-Fi networks.
...
![Figure 4: Malware Family Distribution (Spyware 36.9%, Riskware 25.1%, Trojan 14.6%, Banker 3.4%, Adware 1.0%, Other 18.0%)]

---

## Sideloaded Apps

zLabs identified 23.5% of mobile devices having the presence of one or more sideloaded apps.
...
Apps now need to be vetted continuously to keep up with their rapid updates. Without the insights provided by continuous app vetting, critical changes in privacy, communications, and compliance behavior can go unnoticed.
...
- **High “No Code Protection” Rates**: A notable percentage of Android apps (16–34%) and iOS apps (60%) have no code protection at all.
- **Free Tools Dominate Android**: Across all categories, most Android apps (over 60%) rely on open-source security tools.

---

## The Data Leaks You Don’t See

In apps across all iOS categories, 50–60% of apps are vulnerable to leaking personally identifiable information (PII).
...
62% of the top 100 widely used Android frameworks and 46% of the top 100 widely used iOS libraries are only available as precompiled binaries.

---

## Know Your Device - Device Attestation Is Critical

- 61.2% of Android devices ran an outdated OS during a 12-month period.
- 49.2% of iOS devices ran an outdated OS during a 12-month period.
...
Mobile is now a primary attack surface. Organizations must adopt a risk-based approach, including continuous app vetting, binary-level analysis, and device attestation to secure the mobile enterprise.
