...
- Consider a multi-vendor strategy to limit exposure to outages.
...
Verizon’s 2024 Data Breach Investigations Report (DBIR) indicates that over 80% of breaches involve compromised identity. In 2024, we saw affiliates with Akira and Fog ransomware opportunistically exploiting SSL VPN accounts without MFA enabled.
...
In 2024, we saw an increase in threat activity against critical infrastructure, including the Water and Wastewater Systems sector, the Energy sector, and Healthcare. There is suspicion that some of these incidents were intended to establish stealthy persistence within these environments.

### From Disruption to Destruction
The coming year could represent a geopolitical tipping point. Should a large-scale conflict arise, nation-state affiliated actors who have been refining their skills for economic benefit may soon be called upon to escalate from disruption to outright destruction.

### Recommendations
- Implement a vulnerability management program that prioritizes continuous remediation.
- Subscribe to security bulletins.
- Ensure network and endpoint logs are available for examination.
- Reduce the blast radius of compromised accounts and devices.
...
- Understand and account for the shared responsibility model of cloud services.
...
While frontier large language models (LLMs) already possess some programming capabilities, they haven’t yet led to a significant increase in new initial access techniques. However, once LLMs are able to competently reason about the flow of data through an application, they are expected to facilitate discovery of novel vulnerabilities or to chain together vulnerabilities in a manner that is more difficult for humans to achieve.

### An Ongoing Arms Race
The warning signs are already here: in 2024, OpenAI identified a cluster of ChatGPT accounts using the platform for scripting and vulnerability research tasks, all with nation-state ties.
...
Anticipating the future helps us prepare for it. By examining the evolution of attacker TTPs, we are able to propose reasonable answers to the question, “What will adversaries try next?” These answers help inform preventative measures. However, no defense is impenetrable. Just as important as preventative measures are the abilities to detect and respond — quickly and effectively — to cyber attacks that do occur.
