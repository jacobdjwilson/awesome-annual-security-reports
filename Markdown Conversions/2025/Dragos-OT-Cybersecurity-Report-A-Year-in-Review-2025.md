# 8th Annual OT/ICS Cybersecurity Year in Review 2025

## Table of Contents
- [Introduction](#introduction)
- [Defender’s Guide to the Current Threat Landscape](#defender’s-guide-to-the-current-threat-landscape)
  ...
  - [The Ukraine-Russian Conflict Fuels Activity for Established Dragos Threat Groups](#the-ukraine-russian-conflict-fuels-activity-for-established-dragos-threat-groups)
    - [KAMACITE Technical Update](#kamacite-technical-update)
      - [KAMACITE Campaigns](#kamacite-campaigns)
        - [Kapeka Campaign (2022-2023)](#kapeka-campaign-(2022-2023))
        - [DarkCrystal RAT (2022-2024)](#darkcrystal-rat-(2022-2024))
        - [GIE Conference Campaign (2024)](#gie-conference-campaign-(2024))
    - [ELECTRUM Technical Update](#electrum-technical-update)
      - [ELECTRUM Campaigns](#electrum-campaigns)
        - [KyivStar Attack and Hacktivists Cover (December 2023)](#kyivstar-attack-and-hacktivists-cover-(december-2023))
        - [AcidPour Wiper (March 2024)](#acidpour-wiper-(march-2024))
...
- [Dragos Identifies Two New Threat Groups in 2024](#dragos-identifies-two-new-threat-groups-in-2024)
  - [Introducing GRAPHITE](#introducing-graphite)
    - [GRAPHITE Campaigns](#graphite-campaigns)
      - [Spear-phishing campaign targeting natural gas pipeline operators and hydroelectric generation facilities (March 2022 to October 2023)](#spear-phishing-campaign-targeting-natural-gas-pipeline-operators-and-hydroelectric-generation-facilities-(march-2022-to-october-2023))
      - [Spear-phishing campaigns targeting energy and government entities (Throughout 2023)](#spear-phishing-campaigns-targeting-energy-and-government-entities-(throughout-2023))
      - [Spear-phishing campaign targeting government entities (2023 to February 2024)](#spear-phishing-campaign-targeting-government-entities-(2023-to-february-2024))
      - [Spear-phishing campaign targeting government entities (2023 to present)](#spear-phishing-campaign-targeting-government-entities-(2023-to-present))
      - [Spear-phishing campaign targeting Ukrainian entities (Early 2024)](#spear-phishing-campaign-targeting-ukrainian-entities-(early-2024))
      - [Simultaneous campaigns in 2024](#simultaneous-campaigns-in-2024)
...
*   **Stage 1: Intrusion**
    *   Spoofing Windows License Software (KMSAuto++)
    ...
    *   EXPLOIT
    *   COMMAND + CONTROL
    *   Requires internet access to kms-win11-update[.]net

###### GIE Conference Campaign (2024)

KAMACITE used LummaStealer and employed a commodity loader service, now tracked by Dragos as TAT24-97.¹¹, ¹² These capabilities are primarily delivered via spear phishing, using domains that resemble prominent technology provider names.
...
*   **Stage 1: Intrusion**
    *   Spear-phishing
    *   Malicious PDF
    *   Adobeprotectcheck[.]com, protectraid[.]com
    *   WEAPONIZE
    *   PowerShell+HTA
    *   Edam Dropper
    *   dccw.exe (Lummastealer)
    *   Cloudflare Proxies
    *   RECON
    *   DELIVER
    *   INSTALL
    ...
    *   EXPLOIT
    *   COMMAND + CONTROL
    *   GIE Conference-themed for ONG
    *   ertel-audit[.]com; bydgoszcz[.]pl; furqaanenergy[.]com; afi-ukraine[.]org
    *   gurt.duna[.]ua/programynauczania/ssowoface.dll, calendar.stib.com[.]ua
...
*   Deletes and overwrites itself (files) from infected host
*   Copies itself in-memory-only as a child process
*   Checks if /dev/null exists
*   Running as root
*   Wipes all files and directories EXCEPT • /dev • /lib • /proc • /sys • /usr • /boot
*   Specifically looks to wipe
*   AcidRain targets: • /dev/mtdXX • /dev/mtdblockXX • /dev/block/mtdblockXX • /dev/sd • /dev/sr • /dev/block • /dev/mmcblkXX • /dev/block/mmcblkXX • /dev/loopXX
*   Reboot/Power off
*   Note: AcidRain targets MIPS architectures while AcidPour targets x86 architectures
*   Extended Functionality
*   AcidPour: • /dev/ubiXX • /dev/dm-XX
...
*   GFW
*   Mass use GFW bypass IPs
*   ORBs
*   Leased VPS geolocated in PRC
*   ORBs
*   TAT23-38 & TAT24-04 Infrastructure
*   Botnet
*   Kv-botnet, jdy-botnet and others
*   Target’s Network Edge Devices
*   Internet-facing VPN gateways, Firewalls
*   Targets
*   Electric, Telecom, Emergency Services, Defense Industrial Base
...
*   **Stage 1: Intrusion**
    *   Exposed Ivanti Connect Secure VPN
    *   GLASSTOKEN webshell
    *   Unauthenticated RCE (CVE-2023-46805)
    *   Command injection (CVE-2024-21887)
    *   WEAPONIZE
    *   Wipe VPN logs
    *   Disable VPN logging
    *   Modify lastauthserverused.js and compcheckresult.cgi via Perl scripts
    *   Symantke[.]com
    *   Stolen creds
    *   S P T T H ) 3 4 4 / P C T (
    *   RECON
    *   DELIVER
    *   INSTALL
    *   ACT
    ...
    *   North America, Europe, Japan, South Korea, Guam, Phillippines
    *   Oil & Gas, Electric, Water and wastewater, Maritime
...
*   **Stage 1: Intrusion**
    *   GLASSTOKEN on various webservers
    *   WEAPONIZE
    *   VOLTZITE IP addresses
    *   Suggests attempted exploitation
    *   VOLTZITE IP addresses
    *   Exfil GIS data
    *   VOLTZITE IP addresses
    *   RECON
    ...
    *   COMMAND + CONTROL
    *   United States
    *   Telecommunications Service Provider (TSP); Geographic Information Systems (GIS)
...
*   **Stage 1: Intrusion**
    ...
    *   Compromised infrastructure
    *   Use webshell
    *   Future Password Spraying Attacks and other offensive functions
    *   WEAPONIZE
    *   VOLTZITE IP addresses
    *   RECON
    *   DELIVER
...
*   **Stage 1: Intrusion**
    ...
    *   Electric, oil & gas, pharmaceutical, water and wastewater, mining
...
Since at least March 2022, GRAPHITE conducted numerous campaigns achieving Stage 1 ICS Cyber Kill Chain impacts. Confirmed victims of GRAPHITE were found throughout Eastern