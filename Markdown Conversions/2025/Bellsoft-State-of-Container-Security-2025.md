...
![Bar chart showing the most important factors when choosing a base container image: Security (minimal number of CVEs) 29%, Performance 21%, Image size 17%, Java support 17%, License compliance 11%, Ease of use 4%, and Other 1%.]
...
![Pie chart showing Base Operating Systems: Alpine Linux 41%, Ubuntu/Debian 34%, Red Hat/Red Hat-based distributions (Rocky, AlmaLinux, CentOS) 21%, Alpaquita Linux 2%, and Other distributions 1%. A callout notes that 55% of respondents report using bloated general-purpose Linux distributions.]
...
![Bar chart showing JDKs used inside containers: Oracle/OpenJDK 48%, Adoptium/Temurin 21%, Amazon Corretto 14%, I do not use JVM containers 8%, BellSoft’s Liberica JDK 7%, and Other 2%. A callout notes that 69% of respondents report using JDKs that are shipped with known CVEs.]
...
1. **Human resources**: **Human errors** dominate at 62% — even with the best tools, implementation failures remain the primary vulnerability. 
...
3. **Human Resources & Support**: The third centers on **time constraints** at 49% and **lack of organizational priority** at 36%.
...
![Graphic showing main challenges in securing containers grouped by category. Human errors: 62% human errors. CVE management: 32% gaps before patches become available, 36% difficulties with regular patching, 29% false positives from scanning tools. Human Resources & Support: 36% lack of organizational priority, 49% time and resource constraints, 27% limitations of standard tools.]
