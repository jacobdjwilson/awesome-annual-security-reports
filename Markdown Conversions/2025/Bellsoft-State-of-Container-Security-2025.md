...
The official report URL is: [https://bell-sw.com/content-library/](https://bell-sw.com/content-library/)
...
![Bar chart showing the most important factors when choosing a base container image: Security (minimal number of CVEs) 29%, Performance 21%, Image size 17%, Java support 17%, Ease of use 11%, License compliance 4%, Other 1%]
...
![Bar chart showing critical parameters for Java application performance in containers: Efficient memory usage 86%, High throughput under load 68%, Fast startup time 66%, Minimal OS overhead 46%, Compatibility with cloud-native tools 32%]
...
Organizations face a real conflict: operational convenience during development vs. minimal attack surface in production. A practical approach is using hardened minimal runtime images, paired with fuller “debug builds” during development — allowing both security and diagnostics without compromise.

![Bar chart showing essential tools inside the base container: Shell 54%, Package manager (rpm, apt, apk, etc) 39%, curl/wget 34%, openssh 28%, Editor (nano, vi, vim, ed...) 25%, None of the above/distroless 17%, binutils/coreutils 14%]
...
![Pie chart showing Base Operating Systems: Ubuntu/Debian 34%, Red Hat/Red Hat-based (Rocky, AlmaLinux, CentOS) 21% (Totaling 55% general-purpose), Alpine Linux 41%, Alpaquita Linux 2%, Other distributions 1%]
...
![Bar chart showing JDKs used inside containers: Oracle/OpenJDK 48%, Adoptium/Temurin 21%, Amazon Corretto 14%, I do not use JVM containers 8%, BellSoft’s Liberica JDK 7%, Other 2%]

## Organizations Remain Stuck in Reactive Security Posture
Responses reveal different security maturity stages:
- **Trusted registries** (45%) and **vulnerability scanning** (43%) represent basic hygiene — catching known vulnerabilities reactively. 
- **SBOM generation** (18%) and **image signing** (16%) address supply chain visibility but remain challenging to implement. 
- Only **6%** use hardware isolation.
- **10%** report no additional security measures beyond standard tools.
...
![Bar chart showing container security mechanisms currently applied: Working only with trusted image registries 45%, Vulnerability scanning (e.g., Trivy, Clair...) 43%, Only standard Docker/Kubernetes tools 36%, Software Bill of Materials (SBOM) generation 18%, Image signing (e.g., Cosign) 16%, No additional measures used 10%, Secure container isolation using hardware virtualization (gVisor, Kata Containers) 6%]
...
![Bar chart showing container image update frequency: With every application release 31%, Only when critical vulnerabilities are found 26%, Monthly 19%, Rarely (every few months or less) 14%, Weekly 10%]
...
![Chart showing security incidents in the past 12 months: No incidents 60%, Yes, minor incident(s) 18%, Yes, major incident(s) 5%]

## Teams Drowning in Vulnerability Management Work
The challenges cluster into three related categories:
1. **Human Resources & Support**: Human errors dominate at 62% — even with the best tools, implementation failures remain the primary vulnerability.
2. **CVE Management**: The second cluster revolves around patching difficulties at 36%, gaps before patches at 32%, and false positives at 29%.
3. **Time/Priority**: The third centers on time constraints at 49% and lack of organizational priority at 36%.
...
![Bar chart showing main challenges in securing containers: Human errors 62%, Time and resource constraints 49%, Difficulties with regular patching 36%, Lack of organizational priority 36%, Gaps before patches become available 32%, False positives from scanning tools 29%, Limitations of standard tools 27%]

## Organizations Need Relief From Endless Vulnerability Management
The top three needs converge on the same requirement: relief from overwhelming vulnerability management. 
- **Pre-hardened images** (48%)
- **Better automation** (47%)
- **More resources** (36%)
...
![Bar chart showing what would help improve container security: Pre-hardened, security-focused base images 48%, Better/easier automated tools 43% (listed as 47% in text), More time and resources 36%, Clearer best practices and guidelines 29%, Better integration with development workflows 21%, Executive support and prioritization 19%]
