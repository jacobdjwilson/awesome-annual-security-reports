...
Official Report URL: [https://www.google.com/search?q=Bellsoft+State+Of+Container+Security](https://www.google.com/search?q=Bellsoft+State+Of+Container+Security)
...
![Chart: What is the most important factor for you when choosing a base container image? Security (minimal number of CVEs) 29%, Performance 21%, Image size 17%, Java support 17%, Ease of use 11%, License compliance 4%, Other 1%]
...
![Chart: What parameters are critical for your company for Java application performance in containers? Efficient memory usage 86%, High throughput under load 68%, Fast startup time 66%, Minimal OS overhead 46%, Compatibility with cloud-native tools 32%]
...
![Chart: Which of the following do you find essential inside the base container? Shell 54%, Package manager 39%, curl/wget 34%, openssh 28%, Editor (nano, vi, vim, ed…) 25%, none of the above/distroless 17%, binutils/coreutils 14%]
...
![Chart: Base Operating Systems. Ubuntu/Debian 34%, Red Hat/Red Hat-based distributions 21% (Total 55% bloated), Alpine Linux 41%, Alpaquita Linux 2%, other distributions 1%]
...
![Chart: Which JDKs do you use inside your containers? Oracle/OpenJDK 48%, Adoptium/Temurin 21% (Total 69%), Amazon Corretto 14%, I do not use JVM containers 8%, BellSoft’s Liberica JDK 7%, Other 2%]
...
![Chart: What container security mechanisms do you currently apply? Working only with trusted image registries 45%, Vulnerability scanning (e.g., Trivy, Clair...) 43%, Only standard Docker/Kubernetes tools 36%, Software Bill of Materials (SBOM) generation 18%, Image signing (e.g., Cosign) 16%, No additional measures used 10%, Secure container isolation using hardware virtualization 6%]
...
![Chart: How often do you update your container images? With every application release 31%, Only when critical vulnerabilities are found 26%, Monthly 19%, Rarely (every few months or less) 14%, Weekly 10%]
...
![Chart: In the past 12 months, has your organization experienced any security incidents related to containerized applications? No incidents 60%, Yes, minor incident(s) 18%, Yes, major incident(s) 5%]

## Teams Drowning in Vulnerability Management Work
The challenges cluster into three related categories:

1.  **Human Resources & Support**: Errors dominate at 62% — even with the best tools, implementation failures remain the primary vulnerability. 
...
3.  **Time Constraints**: The third centers on time constraints at 49% and lack of organizational priority at 36%.
...
![Chart: Main challenges you see in securing containers? Human errors 62%, Time and resource constraints 49%, Difficulties with regular patching 36%, Lack of organizational priority 36%, Gaps before patches become available 32%, False positives from scanning tools 29%, Limitations of standard tools 27%]
