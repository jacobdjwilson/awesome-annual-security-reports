...
**Report Title:** State-of-Container-Security  
**Year:** 2025
...
![Bar chart showing critical parameters for Java application performance in containers: Efficient memory usage (86%), High throughput under load (68%), Fast startup time (66%), Minimal OS overhead (46%), and Compatibility with cloud-native tools (32%).]
...
![Bar chart showing essential tools inside the base container: Shell (54%), Package manager (rpm, apt, apk, etc) (39%), curl/wget (34%), openssh (28%), Editor (nano, vi, vim, ed…) (25%), None of the above/distroless (17%), and binutils/coreutils (14%).]
...
![Chart showing Base Operating Systems: Alpine Linux (41%), Ubuntu/Debian (34%), Red Hat/Red Hat-based distributions (Rocky, AlmaLinux, CentOS) (21%), Alpaquita Linux (2%), and other distributions (1%). A note highlights that 55% of respondents report using bloated general-purpose Linux distributions.]
...
![Bar chart showing JDKs used inside containers: Oracle/OpenJDK (48%), Adoptium/Temurin (21%), Amazon Corretto (14%), I do not use JVM containers (8%), BellSoft’s Liberica JDK (7%), and Other (2%). A note highlights that 69% of respondents report using JDKs that are shipped with known CVEs.]
...
Ten percent report no additional security measures beyond standard tools. The focus remains heavily weighted toward detection and monitoring rather than prevention. Most organizations are trapped in reactive security, constantly responding to newly discovered vulnerabilities rather than building on foundations that minimize exposure from day one.

![Bar chart showing container security mechanisms currently applied: Working only with trusted image registries (45%), Vulnerability scanning (e.g., Trivy, Clair...) (43%), Only standard Docker/Kubernetes tools (36%), Software Bill of Materials (SBOM) generation (18%), Image signing (e.g., Cosign) (16%), No additional measures used (10%), and Secure container isolation using hardware virtualization (gVisor, Kata Containers) (6%).]
...
![Bar chart showing frequency of container image updates: With every application release (31%), Only when critical vulnerabilities are found (26%), Monthly (19%), Rarely (every few months or less) (14%), and Weekly (10%).]
...
![Pie chart showing security incidents related to containerized applications in the past 12 months: No incidents (60%), Yes, minor incident(s) (18%), and Yes, major incident(s) (5%).]

## Teams Drowning in Vulnerability Management Work
The challenges cluster into three related categories:

1.  **Human errors**: Mistakes dominate at 62%—even with the best tools, implementation failures remain the primary vulnerability.
2.  **CVE management**: This cluster revolves around patching difficulties at 36%, gaps before patches at 32%, and false positives at 29%.
3.  **Human Resources & Support**: This centers on time constraints at 49% and lack of organizational priority at 36%.
...
![Chart showing main challenges in securing containers: Human errors (62%), Time and resource constraints (49%), Difficulties with regular patching (36%), Lack of organizational priority (36%), Gaps before patches become available (32%), False positives from scanning tools (29%), and Limitations of standard tools (27%).]
...
![Bar chart showing what would most help organizations improve container security: Pre-hardened, security-focused base images (48%), Better/easier automated tools (43%), More time and resources (36%), Clearer best practices and guidelines (29%), Better integration with development workflows (21%), and Executive support and prioritization (19%).]
