...
![Image description: Bar chart showing the most important factors when choosing a base container image: Security (29%), Performance (21%), Image size (17%), Java support (17%), Ease of use (11%), License compliance (4%), and Other (1%).]
...
![Image description: Bar chart showing critical parameters for Java application performance: Efficient memory usage (86%), High throughput under load (68%), Fast startup time (66%), Minimal OS overhead (46%), and Compatibility with cloud-native tools (32%).]
...
![Image description: Bar chart showing essential items inside the base container: shell (54%), package manager (39%), curl/wget (34%), openssh (28%), Editor (25%), none of the above/distroless (17%), and binutils/coreutils (14%).]
...
![Image description: Pie chart showing Base Operating Systems: Ubuntu/Debian (34%), Red Hat-based distributions (21%), Alpine Linux (41%), Alpaquita Linux (2%), and other distributions (1%).]
...
![Image description: Bar chart showing JDK usage in containers: Oracle/OpenJDK (48%), Adoptium/Temurin (21%), Amazon Corretto (14%), I do not use JVM containers (8%), BellSoft’s Liberica JDK (7%), and Other (2%). Note: 69% of respondents report using JDKs that are shipped with known CVEs.]

---

## Organizations Remain Stuck in Reactive Security Posture
Responses reveal different security maturity stages. Trusted registries (45%) and vulnerability scanning (43%) represent basic hygiene — catching known vulnerabilities reactively. SBOM generation (18%) and image signing (16%) address supply chain visibility but remain challenging to implement. Only 6% use hardware isolation.
...
![Image description: Bar chart of container security mechanisms: Working only with trusted image registries (45%), Vulnerability scanning (43%), Only standard Docker/Kubernetes tools (36%), Software Bill of Materials (SBOM) generation (18%), Image signing (16%), No additional measures used (10%), and Secure container isolation using hardware virtualization (6%).]
...
Time between vulnerability disclosure and exploitation continues shrinking. What once took weeks now happens in days. Organizations updating monthly or less operate with known vulnerabilities attackers actively exploit. The longer you delay, the more changes accumulate, making each update riskier — a negative feedback loop that further discourages frequent updates.

![Image description: Bar chart showing update frequency: With every application release (31%), Only when critical vulnerabilities are found (26%), Monthly (19%), Rarely (14%), and Weekly (10%).]
...
The 60% reporting no incidents shouldn't breed complacency. Many haven't experienced attempted attacks yet or lack visibility to detect successful intrusions. The data reinforces a critical point: reactive security measures aren't sufficient.

![Image description: Pie chart showing security incidents in the past 12 months: No incidents (60%), Yes, minor incident(s) (18%), Yes, major incident(s) (5%), and implied remainder of no response.]
...
2. **CVE management**: Revolves around patching difficulties (36%), gaps before patches (32%), and false positives (29%).
3. **Human Resources & Support**: Centers on time constraints (49%) and lack of organizational priority (36%).
...
![Image description: Bar chart of main challenges: human errors (62%), time and resource constraints (49%), difficulties with regular patching (36%), lack of organizational priority (36%), gaps before patches become available (32%), false positives from scanning tools (29%), and limitations of standard tools (27%).]

---

## Organizations Need Relief From Endless Vulnerability Management
The top three needs converge on the same requirement: relief from overwhelming vulnerability management. Pre-hardened images (48%), better automation (47%), and more resources (36%) are different approaches to the same problem.
