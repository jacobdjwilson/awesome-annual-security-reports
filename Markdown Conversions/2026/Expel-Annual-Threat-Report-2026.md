...
PhaaS platforms provide cybercriminals means to phish for MFA or steal session tokens, allowing attackers to intercept and authenticate despite the use of MFA. One of the best known PhaaS platforms is Evilginx.
...
In 2024, we observed that Lumma infostealer was the most prominent infostealing malware. This was also true for the first few months of the year, until the infrastructure behind the malware was targeted by international law enforcement.
...
- **Server-side exploitation (11.5%)**: Vulnerabilities such as React2Shell (CVE-2025-55182).

### Supply chains cloud over
The end of 2025 saw a fundamental shift in the nature of supply chain attacks with the emergence of the **Shai Hulud 2.0 worm**. Unlike previous attacks, Shai Hulud 2.0 is a self-replicating entity that weaponizes the “trust layer” of the software supply chain, specifically targeting NPM package maintainers to harvest IAM secrets.
...
### Identity
- Ensure there are conditional access policies for authentication in place.
- Use security policies and tools to minimize the threat of phishing.
- Audit and evaluate your current authentication requirements (use FIDO2 standard).

### Endpoint
- Disable the use of the Windows Run hot key (Windows + R).
- Ensure employees have official means for downloading approved applications.
- Use application whitelisting to block unwanted applications.

### Cloud infrastructure
- Configure your package manager to disable lifecycle scripts.
- Implement proactive secret key security and monitoring (e.g., TruffleHog).
...
When we narrow our scope to look at **high and critical severity detections only**, Expel detections picked up the activity in 78.8% of the incidents we saw.
...
- **Dave Merkel (CEO & Co-founder)**: "I don’t think we’ll see AI eliminating existing cyber jobs... The adversary is still a human being. That human being will figure out how defenders are using AI, and exploit those behaviors to their advantage."
