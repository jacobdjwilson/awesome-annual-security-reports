...
Beyond C2 tools, its not unusual to see adversaries using remote monitoring 
...
The LOLBAS project is a great resource and searchable database thats mapped 
...
transfer here. 
While we havent observed it firsthand, numerous threats have reportedly 
...
TAKE ACTION
80
Why do adversaries use Process Injection?Process Injection is a versatile technique that adversaries leverage to perform a 
...
system activity.
In addition to being stealthy, code can inherit the privilege level of the process its 
...
With 12 sub\-techniques, theres no shortage of ways that an adversary can 
...
cant be killed, that naturally makes high volumes of network connections or 
OVERALL RANK
ORGANIZATIONS 
AFFECTED
THREATS DETECTED
...
browser processes (normalizing network connections, info stealingbanking 
trojans)
...
atomic tests to validate your coverage 
Theres no easy way to broadly mitigate all forms of Process Injection because its 
...
There isnt much variance in the ways that adversaries rename system utilities. 
...
302
TECHNIQUE
A behavior thats inherently suspicious in the context of one process can 
...
TAKE ACTION
87
Why do adversaries use LSASS Memory?Adversaries commonly abuse the Local Security Authority Subsystem Service 
...
ambiguous to maliciousstarting with processes.
OVERALL RANK
ORGANIZATIONS 
AFFECTED
THREATS DETECTED
...
atomic tests to validate your coverage
Microsofts additional LSA protections that prevent code injection into LSASS 
...
Procdump, Sysinternals Process Explorer, and Microsofts SQLDumper.exe.
ASSOCIATED THREATS
Mimikatz
Cobalt Strike
Impacket
...
TAKE ACTION
89
Why do adversaries modify the registry?The registry being a generic database used by Windows for myriad purposes 
...
OS Credential Dumping (T1003\)Example registry keys that facilitate this technique:
...
Adversaries will commonly store payloads andor key material to decryptdecode 
...
\[HKLM\|HKCU]SoftwareMicrosoftWindows ScriptSettings \- AmsiEnable 
...
Reference: Windows registry in forensic analysisExample registry keys that facilitate persistence:
...
91
How do adversaries modify the registry?Considering how common it is to perform registry operations in Windows and 
...
Win32 APIs, Native APIs, SyscallsAn adversary can interact with registry APIs directly, including RegCreateKey, 
RegSetValue, \[NtZw]CreateKey, and \[NtZw]SetValueKey among others.
Windows Script Host (VBScriptJScript)Both VBScript and JScript code can perform registry modifications by using the 
RegWrite method.
...
regini.exeThe built\-in regini.exe utility can be used to perform registry modifications. It 
...
priority over preventionmitigation beyond the default operating system ACLs.
TAKE ACTION
...
can circumvent it.
What is File Quarantine?Our previous research includes a thorough examination of File Quarantine that 
...
How do adversaries get around it?Non\-LSFileQuarantineEnabled apps andor binaries like: usrbincurl and
usrbinwget are two examples of binaries that do not append the quarantine 
...
Adversaries are finding new methods of subverting two of macOSs 
...
What are some of Gatekeepers security checks?The name of the game here is to trick macOS into launching an executable 
...
What are some of Gatekeepers security checks?Gatekeeper is a large security control on macOS with responsibilities ranging 
...
CVE\-2021\-30658, disclosed by Wojciech Regua: Enterprise cert\-signed iOS 
app .ipa. 
 
CVE\-2021\-1810, disclosed by Rasmus Sten: Directoryfile path length.
Symlinks
 
CVE\-2021\-30990, disclosed by Ron Masas: Generated an applet symlinking 
the Mach\-O binary at ContentsMacOS to a local copy on the system.
96
Clever app bundles
 
CVE\-2021\-30657 \[CISA Known Exploited Vulnerability], disclosed by 
Cedric Owens: Script in\-place of Mach\-O executable at ContentsMacOS.
...
definitions) file that contains HTMLJavaScript. 
 
CVE\-2021\-30669: AppleScript no further information provided.
WebKit
 
CVE\-2021\-30861, disclosed by Wojciech Regua and Ryan Pickren: Safari can 
...
Ensure macOSs security controls reflect the users use case. For example, 
...
98
FEATURED TECHNIQUE
Why do adversaries use setuid and setgid?Once an adversary gains access to a machine, they need to make sure they have 
...
PermissionsOwnerGroupFilename
\-rwsr\-xr\-xrootrootusrbinping\*
...
on PwnKit if youre looking for additional details. 
100
TAKE ACTION
...
ability to execute setuidsetgid binaries, so proceed with caution. 
 
Opt for tools that use capabilities over setuidsetgid. Capabilities are generally 
the preferred way to handle the problem setuidsetgid binaries were created 
...
Container file formats: you cant mark that which is un\-markable.
...
HostUrl\=https:the.earth.li\~sgtathamputty0\.78w64putty.exe
...
enable the adversarys initial access.
...
double click the embedded malicious file that wont have MotW applied.
2\.Does the victim have a utility that doesnt honor MotW? A common
...
4\.Does the file extension support the embeddingexecution of malicious 
...
that we see in detections associated with SMBWindows Admin Shares.
...
107
Visit the SMBWindows Admin Shares technique page to explore:
...
Block SMB connections inboundDepending on what controls your organization has, it may be possible to block 
inbound SMB connections to workstations and most servers, depending on their 
functionality. Its possible to do this via Group Policy Objects (GPO). 
Disable administrativehidden sharesBeyond limiting SMB\-based connections, it may be possible or worthwhile to 
...
Value \= 0
Disable the Lanman Server serviceAnother option is to disable the Lanman Server. This service enables support for file, 
print, and named\-pipe sharing over the network.
TAKE ACTION
108
Deploy Windows Local Administrator Password PolicyPer Microsoft, Windows Local Administrator Password Solution (LAPS) is a 
...
Log on through Remote Desktop ServicesReferences: Mandiant Ransomware Protection and Containment Strategies 
...
Over the past year weve witnessed countries, governments, and some of the 
...
Once an adversary has access to the victim organization, theyll want to be able 
...
atomic tests to validate your coverage 
Were conditioned to use MFA as a catch\-all method to prevent credential theft 
...
By using your unique physical featureslike fingerprints or facial recognition 
...
previous years. This report wouldnt be possible without all of you!
Special thanks to the following Canaries
who contributed to this years report: 
Jimmy Astle 
Dave Bogle 
Laura Brosnan 
Kelsey Compton
