...
The image depicts a diagram showing the Qualys platform with various sensors, scanners, and agents. It includes text indicating "Security Events collected in real time" with "Trillion 2+", "Scanner appliances" with "Thousand 50+", "IP Scans/Audits a Year" with "Billion 6+", and "Cloud Agents across servers, endpoints, clouds & containers" with "Million 84". The sensors listed include Physical, Virtual, Passive, Cloud/Container, API, and Cloud Agents.
...
The image is a stacked bar chart showing the evolution of the vulnerability threat landscape from 1988 to 2022. The total number of known vulnerabilities is 192,036. The chart breaks this down into subsets: 65,920 vulnerabilities with exploits available, 4,492 with weaponized exploit code, 1,043 exploited by malware, 868 CISA known exploited vulnerabilities, 575 named vulnerabilities, 440 exploited by threat actors, and 236 exploited by ransomware. The percentages for each subset are also shown.
...
The image is a stacked bar chart showing the evolution of the vulnerability threat landscape in 2022. The total number of known vulnerabilities is 25,228. The chart breaks this down into subsets: 7,786 vulnerabilities with exploits available, 159 with weaponized exploit code, 93 exploited by malware, 73 CISA known exploited vulnerabilities, 28 named vulnerabilities, 23 exploited by threat actors, and 18 exploited by ransomware. The percentages for each subset are also shown.
...
The image is a bar chart showing the count of vulnerabilities published prior to 2022 that are now being exploited in 2022. The categories are CISA KEV (453), WEAPONIZED (237), THREAT ACTOR (62), and RANSOMWARE (50).
...
### CVE-2022-24521 — Windows CLFS Driver Privilege Escalation Vulnerability
...
The image is a bar chart comparing the time to weaponize and mean time to remediate for vulnerabilities in 2022. It shows that the time to weaponize is 19.5 days, the exploitation opportunities are 11.1 days, and the mean time to remediate is 30.6 days. It also shows that 57.7% of weaponized vulnerabilities are remediated. A second bar chart compares the time to weaponize and mean time to remediation for different categories of vulnerabilities.
...
The image is a scatter plot comparing the age of weaponized vulnerabilities in 2022 (days since vulnerability publication) on the x-axis to the patch rate (percentage remediated) on the y-axis. The plot shows a wide distribution of points, indicating varying patch rates for vulnerabilities of different ages.
...
The image is a scatter plot comparing the age of weaponized Chrome/Windows vulnerabilities in 2022 (days since vulnerability publication) on the x-axis to the patch rate (percentage remediated) on the y-axis. The plot shows a trendline where the patch rate increases rapidly with age and levels out around 90%.
...
The image is a scatter plot comparing the age of weaponized non-Chrome/Windows vulnerabilities in 2022 (days since vulnerability publication) on the x-axis to the patch rate (percentage remediated) on the y-axis. The plot shows no clear trendline and a wide distribution of points, indicating varying patch rates for vulnerabilities of different ages.
...
The image is a bar chart comparing the time to remediation and patch rate for all vulnerabilities and Windows + Chrome vulnerabilities. It shows that the time to remediation for all vulnerabilities is 30.6 days, while for Windows + Chrome vulnerabilities it is 17.4 days. The patch rate for all vulnerabilities is 42.3%, while for Windows + Chrome vulnerabilities it is 82.9%. The text "Patched Twice as Fast" and "Patched Twice as Often" are also included.

*Figure 10: MTTR vs. Patch Rate for Chrome in Windows Vulnerabilities in 2022*

The image is a bar chart comparing the time to remediation and patch rate for different categories of vulnerabilities. It shows that the time to remediation for weaponized vulnerabilities is higher than for named vulnerabilities, CISA KEV vulnerabilities, and APT vulnerabilities. The patch rate for weaponized vulnerabilities is lower than for named vulnerabilities, CISA KEV vulnerabilities, and APT vulnerabilities. The chart also compares the time to remediation and patch rate for non-Windows + Chrome vulnerabilities and Windows + Chrome vulnerabilities.
...
The image is a bar chart comparing the time to remediation and patch rate for manual and automated patching. It shows that the time to remediation for manual patching is 39.8 days, while for automated patching it is 25.5 days. The patch rate for manual patching is 49.8%, while for automated patching it is 72.5%. The text "32.6% Improvement" and "54.6% Improvement" are also included.
...
The image is a diagram showing the attack paths used by Initial Access Brokers (IABs). The diagram shows that IABs can gain initial access through perimeter devices, valid credentials, or phishing attacks. Once they have established access, they can move to the victim organization's active directory and crown jewels, which they can then sell or leverage.
