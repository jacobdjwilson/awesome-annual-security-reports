...
exploitation of vulnerable software to gain access, particularly in the case of access brokers,<sup>1</sup> demonstrates the need 
...
>through June 30, 2023.<sup>2</sup> The findings relate specifically to interactive intrusion activity — that is, activity 
...
<sup>1</sup> Access brokers are threat actors that specialize in breaching networks with the intention of selling or providing that access to others. 
<sup>2</sup> Unless stated otherwise, the terms “this year,” “the last year” or “the past year” used throughout the report refer to the period from July 1, 2022, to June 30, 2023.
...
*This is a line graph showing the number of interactive intrusion campaigns discovered by Falcon OverWatch from 2019 to 2023. The number of intrusions has steadily increased each year.*
...
*This is a horizontal bar chart showing the top 10 targeted verticals by intrusion frequency from July 2022 to June 2023. The technology vertical is the most frequently targeted, followed by financial, retail, healthcare, and telecommunications.*

*Figure 3. Top five targeted verticals separated by adversary threat type, July 2022 to June 2023*

*This is a set of two horizontal bar charts. The first shows the top five targeted verticals by targeted intrusions, with technology being the highest. The second shows the top five targeted verticals by eCrime intrusions, with technology being the highest.*
...
*This is a heat map showing intrusion activity by threat actor and industry vertical. Each cell represents the number of distinct adversaries active within a particular vertical. The rows are industry verticals and the columns are threat actors. The colors indicate the level of activity.*
...
*This is a heat map showing the top five techniques observed across the interactive intrusion activity discovered by Falcon OverWatch in each tactic area during the past year. The rows are tactic areas and the columns are specific techniques. The colors indicate the level of activity.*

Falcon OverWatch tracks interactive intrusion activity against the MITRE ATT&CK® Enterprise Matrix, a framework that 
categorizes and tracks adversary behavior.<sup>3</sup>
...
<sup>3</sup> To learn more about MITRE ATT&CK, visit https://attack.mitre.org/matrices/enterprise/.

## Front-Line Observations

### Adversaries Advance the Frontier of Identity Threats
Today, 80% of breaches use compromised identities.<sup>4</sup> The abuse of identity, 
...
support for different networking protocols. This newly documented sub-technique<sup>5</sup> 
...
<sup>4</sup> As reported in the CrowdStrike 2023 Global Threat Report: https://www.crowdstrike.com/global-threat-report/.
<sup>5</sup> For more information on this sub-technique, see the MITRE website: https://attack.mitre.org/techniques/T1556/008/.

### Don’t Get Burned by Kerberoasting
Over the past year, Falcon OverWatch observed a staggering 583% increase 
in Kerberoasting attacks<sup>6</sup> to escalate privileges and enable lateral movement 
...
*This is a simple graphic showing a 583% increase in Kerberoasting incidents year over year.*
...
<sup>6</sup> For more information on this sub-technique, see the MITRE website (https://attack.mitre.org/techniques/T1558/003/) 
...
abuse of valid accounts<sup>7</sup> — which in most instances presents as username and password combinations — intrusions 
...
*This is a list of common methods of identification and authentication. The list includes username and password, APIs and secret keys, Kerberos and Kerberos tickets, biometrics, hardware and software tokens, smart card and PIN, valid account and Active Directory certificates, identity providers and protocols, and session-based authentication.*
...
<sup>7</sup> For more information on this technique, see the MITRE website: https://attack.mitre.org/techniques/T1078/.
...
intrusions involved attempts to dump credentials,<sup>8</sup> and 11% involved attempts to target unsecured credentials.<sup>9</sup> All 
of this can facilitate access to sensitive data or support privilege escalation or lateral movement. Falcon OverWatch 
also observed adversaries targeting credentials in password stores,<sup>10</sup> capturing user input<sup>11</sup> and modifying the 
authentication process<sup>12</sup> itself.
...
intrusions where valid accounts were used also involved a brute-force<sup>13</sup> attack. Of the remaining 86% of intrusions 
