...
![Average cost of a data breach in financial services ($5.9M) vs all other industries ($4.4M)]
...
- Consider instituting an internal AI Infosec working group to deal with governance and data sharing guidelines.
...
- Implement a routine vulnerability scan before installing any new applications, devices, or technology.
...
While the details of every breach and compromise may vary, there is a specific attack flow that typically occurs from the initial security bypass to escalation, compromise, followed by persistent home on your network and exfiltration and/or destruction of valuable data.

## Attack Flow Steps
1. **Initial Foothold**: Triggering a security bypass (phishing, vulnerability exploitation, or credential abuse).
2. **Initial Payload**: Downloading sophisticated tools and malware.
3. **Expansion / Pivoting**: Targeting higher-value accounts (Domain Admins, root accounts, database servers).
4. **Malware**: Deployment of RATs, infostealers, or ransomware.
5. **Exfiltration / Post Compromise**: Data theft.
...
- 24% of emails with malicious attachments attempt to spoof American Express.
- Payroll Diversion (48%) is the most used lure in BEC messages.
...
### The Threat
Access can occur if default credentials are not changed, weak passwords are used, or credentials have been purchased from underground forums.

### Trustwave SpiderLabs Insights
- Credential access is used in about 20% of all reported incidents.
- Brute-force attacks make up the majority of observations.
- Unsecured files containing credentials (e.g., "Username and Passwords.txt") are frequently found.
...
### The Threat
Exploiting bugs in software to circumvent security controls.

### Trustwave SpiderLabs Insights
- Common exploits include Apache Log4J, Exchange Server SSRF, and SQL Injection.
- A recent scan of Shodan turned up more than 110,000 open ports in financial services organizations.

### Mitigations to Reduce Risk
- Utilize vulnerability assessments and penetration testing.
- Place all servers behind a firewall and practice network segmentation.
...
### The Threat
Targeting trusted third-party partners to gain access to the primary target (the "Domino Risk").

### Mitigations to Reduce Risk
- Prioritize the security of your systems and those of third-party partners.
...
### The Threat
Once a foothold is established, attackers download sophisticated tools or leverage existing ones like PowerShell or LOLBins.

### Trustwave SpiderLabs Insights
- 28% of financial sector incidents involved Execution.
- PowerShell is the most prevalent tool for discovery and command execution.

### Mitigations to Reduce Risk
- Conduct regular audits of all applications.
- Implement highly granular whitelisting.
...
### The Threat
Moving from a low-value workstation to high-value targets like Domain Admins or Database servers.

### Trustwave SpiderLabs Insights
- Attackers use tools like Bloodhound, Sharphound, Lazagne, and Remcom.
- Attackers establish persistence via Account Creation and Account Manipulation.

---

## Malware: Infostealers
Infostealers focus on the extraction of sensitive information (credentials, browser data) to be sold on underground forums. Notable examples include XLoader, Lokibot, Formbook, and Snake Keylogger.

## Malware: RATs
Remote Access Trojans (RATs) allow attackers to maintain persistent control over a system, enabling them to execute commands, capture keystrokes, and exfiltrate data.

## Malware: Ransomware
Ransomware remains the most destructive threat, often involving double-extortion tactics where data is both encrypted and leaked publicly.

---

## Exfiltration / Post Compromise
The primary motive is data theft. Once data is exfiltrated, it is often sold on the Dark Web or used for further extortion.

---

## Key Takeaways and Recommendations
1. **Consumer Security Education**: Inform customers about phishing and safe online practices.
2. **Implementing Security Controls**: Use encryption, MFA, and intrusion detection systems.
3. **Customer Support and Incident Response**: Provide clear mechanisms for customers to secure compromised accounts.
...
### Threat Groups
- **8BASE**: Known for ransomware and extortion.
- **BlackCat/ALPHV**: A prominent RaaS (Ransomware-as-a-Service) group.
- **Black Basta**: Known for rapid deployment of ransomware.
- **Clop**: Highly active, known for exploiting zero-day vulnerabilities in file transfer software.
- **Medusa**: A ransomware group targeting various sectors.
- **LockBit**: One of the most prolific ransomware groups globally.
- **Play**: Known for double-extortion tactics.
- **Royal**: A sophisticated group often targeting large enterprises.

[^1]: Ponemon Institute data, 2023.
