...
## Our Findings
- **30,784** Exposed APIs included in the Fortune 1000 and CAC 40.
...
capitalization-weighted index of the 40 most significant stocks among the top 100 companies by market cap on Euronext Paris.
...
∼1K unique top-level domains analyzed
...
5.  7K API specifications programmatically generated
...
![Image description: The complete event-based architecture of the Escape Security Scanner, with the reinforcement loop in the center]

## Our Findings
- **30,784** API services exposed
- **3,945** development APIs exposed, including 6 organizations with more than 100 development APIs exposed per domain - 5 from Fortune 1000 and 1 from CAC40
- **1,189** maximum exposed APIs per one domain

- **3,001** REST APIs
- 91% APIs exposed, including more than 166 exposed staging APIs by one Fortune 1000 organization

![Image description: Number of Domains Exposing a Given Range of API Services]
...
- 8.1K Total vulnerabilities found
- 240 highly critical
- 107,368 vulnerabilities found among Fortune 1000 organizations (large tech orgs. like Amazon, Google or Meta excluded)
- 2,038 Highly critical
- 98.8K API vulnerabilities found among CAC40 organizations, including 240 highly critical
- 1.8K highly critical
- 11 API services vulnerable to SQL injection
- 10 API services vulnerable to BOLA

![Image description: Alignment of high-risk Fortune 1000 vulnerabilities with OWASP Top 10 2023 classification]

- 42 average vulnerabilities per domain
- domains are above the average
- the biggest number of medium-risk vulnerabilities per exposed domain (an insurance company based in the US)
- the biggest number of high-risk vulnerabilities per exposed domain (same insurance company...)

![Image description: Distribution of Vulnerabilities by Industry]

- Total secrets found

![Image description: Distribution of Secrets]
...
- **1.8K** highly critical vulnerabilities found (very large tech orgs. like Amazon, Google or Meta etc. were excluded)
- **316** companies impacted, some in highly regulated industries like financial services
...
1.  Disable Sensitive Actuator Endpoints in Production:
...
2.  Obfuscate Sensitive Data in `/env`:
...
3.  Monitor and Log Access Attempts to Actuator Endpoints:
...
![Image description: Top 5 CVE occurencies]

- **98** High Impact CVEs in 2024, with CVE-2024-5535 as a clear leader
...
CVE-2021-3711 (57 instances) As an older CVE, if it pertains to cryptographic weaknesses or data exposure, it would directly impact APIs by potentially making data-in-transit vulnerable

![Image description: Diiscovered CVEs map to several key CWEs associated with common API weaknesses.]
...
This extensive exposure of vulnerable APIs underscores a critical security issue. If you have publicly accessible APIs or are uncertain about whether your developers adhere to best practices to prevent unsecured staging or development APIs from being exposed, we recommend taking the following actions immediately:
...
7.  **Implement an Automated Security Solution that Can Start Scanning in Minutes**: Utilize a security solution that enables immediate scanning of your API ecosystem for vulnerabilities and misconfigurations. A solution that provides quick setup and fast scanning capabilities can help identify and mitigate risks efficiently without disrupting workflows.
8.  **Educate Your Internal Teams**: Ensure that all team members understand the importance of token security and adhere to best practices. Consider enhancing the security experience through gamification or implementing a Security Champion Program, following [The Security Champion Program Success Guide](URL to Security Champion Program Success Guide).

These measures are crucial for protecting your APIs and maintaining secure and compliant applications. For more detailed information, you can use our [API Security Checklist](URL to API Security Checklist) or refer to the resources provided on the [Escape blog](URL to Escape blog).

![Image description: Flowchart outlining API security steps]
