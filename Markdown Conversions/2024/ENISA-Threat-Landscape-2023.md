...
- [A ANNEX: MAPPING TO MITRE ATT&CK FRAMEWORK](#a-annex-mapping-to-mitre-attack-framework)
...
Methodology[^2], which was released in 2022. The ENISA CTL Methodology serves as a foundational framework for the 
...
- Ransomware 
According to ENISA’s Threat Landscape for Ransomware Attacks[^3] report, ransomware is defined as a type of 
...
mostly lie in how they happen[^4] [^5]. Data breach is an intentional cyber-attack brought by a cybercriminal with the 
...
landscape[^6] [^7]. Attacks occur when users of a system or service are not able to access relevant data, services or 
other resources. This can be accomplished by exhausting the service and its resources or overloading the 
components of the network infrastructure[^8]. 
...
- Supply Chain Attacks 
A supply chain attack targets the relationship between organisations and their suppliers[^9]. For this ETL report we 
use the definition as stated in the ENISA Threat Landscape for Supply Chain Attacks[^10] in which an attack is 
...
was observed that threat actors are continuing[^11] to feed on this source to conduct their operations and gain a 
...
- Several threat actors further professionalised[^12] [^13] their As-a-Service programmes. They not only used 
...
- Hacktivists are increasingly claiming[^14] that they target OT environments but public reporting indicate 
they often[^15] overestimate or do not substantiate their claims. 
...
manipulation has been an essential and well-established component of Russia’s security strategies[^16] [^17]. The 
...
include events affecting sectors beyond the scope of the NIS2 directive. Examples include Defence, Education[^19], 
