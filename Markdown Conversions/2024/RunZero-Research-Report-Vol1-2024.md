# RunZero Research Report Vol1

Organization: RunZero
Year: 2024

HD Moore // Rob King // Tom Sellers // Research Team
...
- [Chapter 1 Introduction](#chapter-1-introduction)
  ...
  - [runZero Research Objective](#runzero-research-objective)
  - [Scope & Methodology](#scope--methodology)
  - [runZero’s Unique Perspective](#runzeros-unique-perspective)
  - [Oddball Devices in Our Data Set](#oddball-devices-in-our-data-set)
- [Chapter 2 OT & Cloud Impacts on Attack Surfaces](#chapter-2-ot--cloud-impacts-on-attack-surfaces)
  - [OT & IT Are Converging](#ot--it-are-converging)
  - [OT/ICS Around the World](#otics-around-the-world)
  - [OT Scanning Moves From Passive to Sometimes Active](#ot-scanning-moves-from-passive-to-sometimes-active)
  - [Safe Active Scans](#safe-active-scans)
  - [The Cloud Is Someone Else’s Attack Surface](#the-cloud-is-someone-elses-attack-surface)
  - [Cloud Migrations](#cloud-migrations)
    - [Migration Realities](#migration-realities)
  - [New Exposures](#new-exposures)
  - [Hybrid Is Forever](#hybrid-is-forever)
- [Chapter 3 Unusual Assets Are Risky Assets](#chapter-3-unusual-assets-are-risky-assets)
  ...
  - [Calculating Outlier Scores](#calculating-outlier-scores)
  - [Outliers Are Riskier, on Average](#outliers-are-riskier-on-average)
  - [Guidance](#guidance)
- [Chapter 4 Some Old Enemies](#chapter-4-some-old-enemies)
  - [End-of-Life Is Not the End](#end-of-life-is-not-the-end)
  - [Operating System End-of-Life](#operating-system-end-of-life)
  - [Exposed Systems Past Extended EOL](#exposed-systems-past-extended-eol)
  - [Case Study: The Boa Web Server](#case-study-the-boa-web-server)
  - [Dark Matter: IoT & Embedded Devices](#dark-matter-iot--embedded-devices)
  - [The Decay of Segmentation](#the-decay-of-segmentation)
  - [Case Study: The Office Printer](#case-study-the-office-printer)
- [Chapter 5 Emerging Threats](#chapter-5-emerging-threats)
  - [Zero Days Are -1 Days](#zero-days-are--1-days)
  - [Case Study: Cisco IOS-XE](#case-study-cisco-ios-xe)
  - [Case Study: D-Link NAS](#case-study-d-link-nas)
  - [Secure. Gateway. Pick One.](#secure-gateway-pick-one)
  - [Enemy at the Gates](#enemy-at-the-gates)
  - [Supply Chains Under Attack: Witness XZ Utils](#supply-chains-under-attack-witness-xz-utils)
...
First and foremost, vulnerabilities are being exploited at a truly unprecedented pace. And it’s working. So much so that the SEC now requires 8K filings for data breaches, not to mention the constant flow of news about emerging vulnerabilities and successful compromises across organizations of every size and sector. While zero day attacks at the network edge have surged, suppliers are struggling to provide timely patches for their products, often leaving customers at the mercy of fire drills related to emerging threats and exposures. Defenders continue to juggle scoping, patch management, emergency response, and incident analysis on top of business requirements – all while security attackers for days or weeks. In response to the acceleration of exploitation, suppliers are now often releasing indicators of compromise (IOCs) in conjunction with their initial notifications to customers.
...
runZero was founded on the principle that applied research makes for better asset discovery, and that better asset discovery is the foundation of modern exposure management. Today, runZero is recognized as the leading edge of Cyber Asset Attack Surface Management (CAASM). This success is due to the work of the runZero research team: a group of industry veterans with decades of experience in information security. The practical output of their research is the accurate and in-depth identification of assets across cloud, on-premises, and remote environments.
...
![Figure 1: A list of devices with multiple attack surface designations found by runZero. Devices that span attack surfaces can provide entry points for attackers into internal organizational networks.](image-1.png)
...
Once considered an island unto itself, operational technology (OT) and industrial control systems (ICS) have converged with IT and further compounded complexity.

The whole world has, with very rare exceptions, settled on Ethernet and the Internet Protocol stack for IT. The vast, chaotic sea of proprietary protocols and competing standards of the OT/ICS world have now joined the fray in earnest, along with all the growing pains that come with it.
...
![Figure 2: A selection of industrial devices detected by runZero on the public Internet.](image-2.png)
...
![Figure 3: An Allen-Bradley industrial PLC indicating 100% CPU utilization due to the device receiving a high rate of packets from an active scan NOT conducted by runZero.](image-3.png)
...
![Figure 4: A partial screenshot of an OT device detected by a runZero active scan.](image-4.png)
...
runZero is careful to send only valid traffic to discovered services and specifically avoids any communication over OT protocols that could disrupt the device.
...
![Figure 5: A high-level overview of the “progressive enhancement” probing process. The diagram shows a flow: Pick Asset -> Assets to Scan? (Yes/No). If Yes: Initialize Probes -> Create Empty Flag Set. Then, For Each Stage in Probing: For Each Probe in Stage: Flags Indicate Probe is Safe for Asset? (Safe/Unsafe). If Safe: Run Probe. If Unsafe: Skip Probe. After each probe, Update Flags with Probe Results. This loops back to For Each Probe in Stage until all probes are done, then back to For Each Stage in Probing, and finally back to Assets to Scan?.](image-5.png)
...
![Figure 6: CPU-specific vulnerabilities targeting cloud operators.](image-6.png)
...
![Figure 7: Comparison of end-of-life operating system distribution between cloud and external attack surfaces. The chart shows Cloud: Past EOL 1.7%, Past Extended EOL 8.1%. External: Past EOL 1.1%, Past Extended EOL 10.3%.](image-7.png)
...
![Figure 8: Average risk rank versus runZero outlier score. The chart shows Average Risk Rank on the Y-axis from 0.0 to 4.0, and Outlier Score on the X-axis from 0 to ≥800. There is a clear positive correlation, with risk rank increasing as outlier score increases.](image-8.png)
...
![Figure 9: Top OS past extended EOL. The list shows: Microsoft Windows Server 2012 R2 (12.69%), Ubuntu Linux 14.04.x (7.66%), Microsoft Windows 10 (1809) (5.90%), Microsoft Windows 7 (5.30%), Microsoft Windows 10 (1909) (5.01%), Microsoft Windows 10 (1607) (4.68%), Microsoft Windows Server 2008 R2 (4.03%), VMware ESXi 6.7.0 (3.11%), VMware ESXi 6.5.0 (2.23%), Microsoft Windows 10 (20H2) (2.16%).](image-9.png)
...
![Figure 10: Windows 10 past extended EOL. The list shows: Microsoft Windows 10 (1809) (28.31%), Microsoft Windows 10 (1909) (24.06%), Microsoft Windows 10 (1607) (22.45%), Microsoft Windows 10 (20H2) (10.38%), Microsoft Windows 10 (1803) (7.69%), Microsoft Windows 10 (21H1) (3.67%), Microsoft Windows 10 (1709) (1.77%), Microsoft Windows 10 (1703) (1.33%), Microsoft Windows 10 (1511) (0.33%).](image-10.png)
...
![Figure 11: Server operating systems with external attack surface exposure, past extended EOL. The list shows: VMware ESXi (34.4%), Debian Linux (11.7%), Windows (8.7%), Ubuntu Linux (7.3%), Windows Server (3.1%), Enterprise Linux (RHEL & derivaties) (1.2%).](image-11.png)
...
![Figure 12: Boa web server version distribution in runZero data. The list shows: Boa/0.94.14rc21 (58.07%), Boa/0.93.15 (26.12%), Boa/0.94.13 (8.4%), Boa/0.94.14rc18 (2.57%), Boa/0.94.14rc19 (2.02%), Boa/0.94.14rc20 (1.42%), Boa/0.94.101wk (0.95%), Boa/0.92o (0.29%), Boa/0.94.11 (0.13%), Boa/0.94.8.3 (0.03%).](image-12.png)
...
![Figure 13: Device types still running Boa in sample runZero data. The list shows: Network-Attached Camera (92.3%), Media & Telephony Devices (5.5%), Environmental Control Devices (0.9%), Network Devices (0.9%), Industrial Control Devices (0.3%).](image-13.png)
...
![Figure 14: Device types by visibility. This table describes three categories: Monitored Devices, Devices w/ Limited Visibility, and Dark Matter.
| Category | Typical Visibility | Mechanism | Update Cadence | Relative Percentage of Total Device Types |
|---|---|---|---|---|
| Monitored Devices | High | EDR and EPM mechanisms, SNMP and other management protocols, frequent human interaction. | Often automatic and en masse, with rapid response to known threats. | 35.45% |
| Devices w/ Limited Visibility | Moderate | Some human interaction when people notice when they stop working. Particularly vigilant teams may monitor these devices more closely. | Much less often, and sometimes never. Devices often require manual intervention to apply security patches and updates. | 45.46% |
| Dark Matter | Low | A lot of security teams may not even know these devices are on the network, or simply view them as a curiosity when they see them. | Very often never, and may not even be upgradeable. Often run firmware that is many years out of date. | 19.09% |
](image-14.png)
...
![Figure 15: A simple example of network segmentation.](image-15.png)
...
![Figure 16: An example of a segmented network, with several devices potentially breaking that segmentation.](image-16.png)
...
![Figure 17: A network diagram showing unexpected network bridging points. The diagram illustrates various IP subnets (e.g., 192.168.1.0/24, 192.168.30.0/24, 192.168.40.0/24, 192.168.60.0/24, 192.0.0.0/24, 100.106.248.0/24) and devices (e.g., HTD2617-V1-320200105AA, RESEARCH-ROUTER, RAPID7-SECURITY-CONSOLE, TV-IP310PI20161101CCWR, OPNSENSE, SLAB, GITHUB-RUNNER-1) with unexpected connections and IP forwarding capabilities, bridging different network segments and potentially the global internet.](image-17.png)

![Figure 18: Unusual devices with IP forwarding capabilities. Box sizes indicate relative frequency in our dataset. The image lists various device types, with larger boxes for more frequent occurrences. Devices include: Printer or Multifunction Device (e.g., Ricoh IM C3000, Samsung SL-M3820ND, Xerox WorkCentre 6605DN), NAS or Storage Appliance (e.g., Synology DiskStation DS220+, Datto Backup Appliance, Western Digital WD2GO), IP Phone, Media Server, Print Server, Building Automation or Environmental Control/Monitoring, Sensor, DVR, Video Conferencing, IP Camera, Power Device, Smart TV, BACnet, Light Bulb, Media Player, Thermostat, Scanner, Access Control, Home Automation, Point of Sale, Payment Device, Barcode Scanner, Power Meter, Light Switch, Game Console, Voice Assistant, Wireless Presenter, Automobile, Thin Client, Vacuum Cleaner, Smart Plug, Video Encoder, Media Gateway, Industrial Control, HID, Test Instrument. A note indicates "Most surprising to runZero Research."](image-18.png)

## Chapter 5 Emerging Threats
...
The COVID-19 pandemic accelerated the growth of remote work. According to USA Today, 14% of the American workforce works remotely on a full-time basis and some estimate that number will increase to nearly 25% by the end of 2025.

- 14% American workforce working remotely 2024
- 25% Estimated increase of remote workforce 2025
...
SSH-2.0-OpenSSH_9.6p1 Debian-5
