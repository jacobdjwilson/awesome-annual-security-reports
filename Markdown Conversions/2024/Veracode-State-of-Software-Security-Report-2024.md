...
- [Measuring and Managing Security Debt](#measuring-and-managing-security-debt)
  ...
  - [Factors that contribute to security debt](#factors-that-contribute-to-security-debt)
    ...
    - [First vs. third-party code](#first-vs-third-party-code)
    - [Flaw types](#flaw-types)
    - [Firmographic factors](#firmographic-factors)
  - [Recommendations to minimize security debt](#recommendations-to-minimize-security-debt)
    - [Integrate security into the entire SDLC](#integrate-security-into-the-entire-sdlc)
    - [Move toward continuous remediation](#move-toward-continuous-remediation)
    - [Prioritize remediation of critical security debt](#prioritize-remediation-of-critical-security-debt)
    - [Build developer security competency](#build-developer-security-competency)
    - [Know your language’s debt profile](#know-your-languages-debt-profile)
    - [Develop a strategy to secure the software supply chain](#develop-a-strategy-to-secure-the-software-supply-chain)
...
6.  Almost 90% of all security debt across all active applications exists in first-party code. But third-party code claims two-thirds of security debt rated as critical severity.
7.  The prevalence of critical security debt across applications developed in large tech firms is 7x that of small firms in the retail and hospitality sector.
...
![Figure 1: Percent of applications with security flaws detected in most recent scan](Image description)
...
![Figure 2: Trend of flaw detection rates over time](Image description)
...
On average, a typical application has 42 flaws for every 1 MB of code. That seemed odd to us, so we asked Deep Thought to crunch the numbers. It took a while, but 42 was indeed verified to be the answer to life, the universe, and everything AppSec.<sup>1</sup>

![Figure 3: Density of flaws detected in applications](Image description)
...
![Figure 4: Rating of flaw severity and exploitability (percent of all flaws)](Image description)

<sup>1</sup> There are also 42 charts in this report and the original draft had 42 pages. Alas, there were some changes during the Vogon review process.
...
![Figure 5: Prevalence and intensity of CWE and OWASP flaws in applications](Image description)
...
![Figure 6: Prevalence of flaws in first-party (left) vs. third-party (right) code among applications](Image description)
...
![Figure 7: Probability of flaws converting to security debt in first-party vs. third-party code](Image description)
...
![Figure 8: Prevalence of security debt across all applications greater than one year old](Image description)

As shown here, 42%<sup>2</sup> of active applications (those that have been scanned for at least one year) have flaws that constitute security debt (and some of them carry a lot of debt)<sup>3</sup>. That’s an important finding that deserves much more than a passing glance. Let’s dig deeper into exactly how much debt, where it comes from, and how to begin eliminating it in the next section.

<sup>2</sup> We also verified this with Deep Thought to be sure.
<sup>3</sup> The proportion of apps with debt changes substantially depending on how we slice the data. Showing those different slices goes beyond the scope of this “at a Glance” section, so we added a special insert on the next page for those interested.
...
![Figure 9: Prevalence of security debt across all applications (NOT filtered to one year old)](Image description)
...
![Figure 10: Prevalence of security debt across all applications scanned with SAST and DAST](Image description)
...
![Figure 11: Prevalence of security debt (left) and critical security debt (right) among organizations](Image description)
...
Within this sample, 10% of organizations show no security debt.<sup>4</sup> Among those that do, as shown in Figure 12, the typical (median) firm has security debt in a third of its active applications. Security debt affects at least two of every three applications in a quarter of organizations. And just north of 1 in 10 firms has security debt in at least 90% of its applications.

<sup>4</sup> This differs from Figure 11, the preceding donut chart (29% with no debt), because we’ve removed firms with less than seven applications.

![Figure 12: Prevalence of security debt across applications >1yr old within organizations](Image description)
...
![Figure 13: Prevalence of critical security debt across applications >1yr old within organizations](Image description)
...
![Figure 14: Percent of flaws per organization that qualify as security debt](Image description)
...
![Figure 15: Distribution of security debt across applications >1yr old in 20 organizations](Image description)
...
![Figure 16: Proportion of flaws that qualify as security debt by age of application](Image description)
...
Ignoring the values in the shaded squares for moment, let’s note the overall prevalence of security debt listed below the size labels on the vertical axis. Large applications have the highest proportion of security debt (40% of applications) and critical debt (47%). Medium-sized applications fall a little below that, and the debt ratio among small applications is lower still in both charts.

![Figure 17: Distribution of security debt across application age and size](Image description)
...
![Figure 18: Prevalence of security debt by application development language](Image description)
...
![Figure 19: Proportion of security debt and critical debt in first-party vs. third-party code](Image description)
...
![Figure 20: Probability of first-party flaws converting to security debt by CWE category](Image description)
...
![Figure 21: Probability of third-party flaws converting to security debt by CWE category](Image description)
...
![Figure 22: Percent of applications >1yr old in each sector with critical security debt](Image description)
...
![Figure 23: Process for integrating security testing into the SDLC](Image description)
...
![Figure 24: Comparison of security flaw and debt detection rates among scan types](Image description)
...
Relying on a single testing method (e.g., SAST