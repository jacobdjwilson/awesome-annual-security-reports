...
Artificial Intelligence (AI) wasn’t born last year, but 2023 was its coming-of-age party. The proliferation of AI-generated code brings with it insecure code at scale and the likelihood of it becoming security debt. Research indicates that code developed by AI contains about the same percentage of security flaws as that generated by humans. Other research suggests that programmers with a variety of experience levels fail to identify incorrect ChatGPT answers more than a third of the time.
...
4.  Flaws that stick around longer than a year become security debt. That occurs in 42% of applications and 71% of all organizations.
5.  More concerning still, almost half (46%) of organizations have persistent, high-severity flaws that constitute critical security debt.
...
*   A bar chart shows the average monthly capacity of applications to remediate flaws. 64.1% of applications demonstrate a remediation capacity that’s sufficient to eliminate critical security debt. 11.2% of applications demonstrate a remediation capacity that’s sufficient to eliminate all critical flaws.
*   A line chart shows the percentage of applications at or below a given average monthly capacity.
...
7.  Continuous scanning must be accompanied by continuous remediation to be effective. Development teams that fix flaws the fastest are 4x less likely to let critical security debt materialize in their applications.
8.  Almost 90% of all security debt across all active applications exists in first-party code. But third-party code claims two-thirds of security debt rated as critical severity.
9.  Only 64% of applications demonstrate a sustained capacity to eliminate all critical security debt. Prioritizing flaws for remediation is essential!
...
*   **Figure 1:** Percent of applications with security flaws detected in most recent scan. A bar chart shows the percentage of applications with flaws detected by SAST, DAST, and SCA scans, as well as SAST scans alone. The combined scan method shows 80% of applications with flaws, while SAST alone shows 73%.
...
*   **Figure 2:** Trend of flaw detection rates over time. A line chart shows the trend of flaw detection rates over time from 2016 to 2023 for high-severity flaws, OWASP Top 10 flaws, and CWE Top 25 flaws. High-severity flaws show a clear downward trend.

Veracode State of Software Security 2024
10

## How many flaws do applications have?
*   **Figure 3:** Density of flaws detected in applications. A histogram shows the distribution of flaw density (flaws per MB of code) across applications. The average flaw density is 42 flaws per MB of code.
...
Overall, about 3% of all flaws are considered very high severity, and 16% are very likely to be exploited by attackers. Thankfully, less than 1% of all flaws earn the highest (worst) ratings for both criticality metrics. If we broaden the threshold to include issues that are high severity and likely to be exploited (the upper-right 2x2 quadrant), we get 5.5% of all flaws. It goes without saying that these rare but risky security flaws warrant quick attention from developers, though it must be acknowledged that the glut of Medium-Likely flaws (43.5% of all flaws) represents a substantial attack surface for many organizations that must be managed as well.

*   **Figure 4:** Rating of flaw severity and exploitability (percent of all flaws). A heat map shows the distribution of flaws based on their severity and exploitability ratings. The majority of flaws fall into the medium severity and likely exploitability categories.
...
*   **Figure 5:** Prevalence and intensity of CWE and OWASP flaws in applications. Two scatter plots show the prevalence and intensity of CWE and OWASP flaws in applications. The x-axis represents the percentage of applications affected, and the y-axis represents the average number of flaws per application.
...
## Where are flaws most likely to be introduced?
*   **Figure 6:** Prevalence of flaws in first-party (left) vs. third-party (right) code among applications. Two bar charts show the percentage of applications with flaws in first-party and third-party code. 63% of applications have flaws in first-party code, while 70% have flaws in third-party code.
...
*   **Figure 7:** Probability of flaws converting to security debt in first-party vs. third-party code. A survival curve chart shows the probability of flaws converting to security debt over time for first-party and third-party code. Third-party flaws have a higher probability of becoming security debt.
...
*   **Figure 8:** Prevalence of security debt across all applications greater than one year old. A waffle chart shows the prevalence of security debt across all applications older than one year. 42% of applications have security debt.
...
*   **Figure 9:** Prevalence of security debt across all applications (NOT filtered to one year old). A waffle chart shows the prevalence of security debt across all applications, including those less than one year old. 23% of applications have security debt.
*   **Figure 10:** Prevalence of security debt across all applications scanned with SAST and DAST. A waffle chart shows the prevalence of security debt across all applications scanned with SAST and DAST. 69% of applications have security debt.
...
*   **Figure 11:** Prevalence of security debt (left) and critical security debt (right) among organizations. Two donut charts show the prevalence of security debt and critical security debt among organizations. 71% of organizations have some security debt, and 46% have critical security debt.
...
*   **Figure 12:** Prevalence of security debt across applications >1yr old within organizations. A box plot shows the distribution of security debt across applications within organizations. The median organization has security debt in 33% of its applications.
*   **Figure 13:** Prevalence of critical security debt across applications >1yr old within organizations. A box plot shows the distribution of critical security debt across applications within organizations. The median organization has critical security debt in 16% of its applications.
...
*   **Figure 14:** Percent of flaws per organization that qualify as security debt. Two beeswarm charts show the distribution of the percentage of flaws per organization that qualify as security debt and critical security debt. The median organization has 47% of flaws that qualify as security debt, and 2% of flaws that qualify as critical security debt.
*   **Figure 15:** Distribution of security debt across applications >1yr old in 20 organizations. A heat map shows the distribution of security debt across applications in 20 different organizations. Each rectangle represents an organization, and the color of each cell represents the density of security debt in that application.
...
> All data used in this report is anonymized. We’re able to attribute flaws and applications to a randomized organization ID to enable analysis like what you see in Figure 15, but the data does not allow for identification of “Org #4.”
...
*   **Figure 16:** Proportion of flaws that qualify as security debt by age of application. A line chart shows the proportion of flaws that qualify as security debt based on the age of the application. The proportion of flaws that become security debt increases with the age of the application.
...
*   **Figure 17:** Distribution of security debt across application age and size. Two heat maps show the distribution of security debt and critical security debt across different application age and size categories. The highest concentration of debt is in older, larger applications.
...
*   **Figure 18:** Prevalence of security debt by application development language. Two scatter plots show the prevalence of security debt and critical security debt by application development language. The size of each dot corresponds to the number of applications for that language.
...
BUT there’s another side of the story (and chart) here. When we narrow in on critical security debt, third-party code comes to the forefront with a two-thirds majority. This suggests that organizations wanting to drive down debt representing the highest risk should focus first on third-party libraries that often constitute a large proportion of their codebase (