...
![Table: Measurement and Aggregation Options]
...
We observed MFA adoption rates of between 61 and 68% for the Americas (AMER), Asia Pacific (APAC) and EMEA (Europe, Middle East and Africa). Location is not a primary determining factor in MFA adoption.

### MFA adoption by industry
The technology sector continues to record the highest MFA adoption rate (88%). Government and Education sectors saw an increase of above 5% year-over-year in MFA adoption.
...
To help CIOs, CSOs, and policymakers make informed decisions, we assessed authenticators on both usability and security properties.

### Authenticator challenge time
Authenticators that combine possession and inherence (such as biometric checks) offer the fastest challenge times (4 seconds). FIDO2 WebAuthn, Okta FastPass, and smart cards offer a dramatically more efficient user authentication process.
...
Knowledge-based authenticators impose the most considerable burden on users. The password has the worst failure rate (at almost 10%).
...
We conducted a survey of Okta IT and Security practitioners to understand the relative importance of each of the usability and security metrics to an authenticator’s usability and security properties.
...
4. Create dynamic access policies that evaluate user attributes and device context.
