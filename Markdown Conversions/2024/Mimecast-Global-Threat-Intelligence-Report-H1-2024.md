# Global Threat Intelligence Report H1 2024
...
- [Major events](#major-events)
  - [Trello leaks 15 million user records](#trello-leaks-15-million-user-records)
  - [i-SOON insider leaks hacking info](#i-soon-insider-leaks-hacking-info)
  - [Lockbit operation disrupted by global effort](#lockbit-operation-disrupted-by-global-effort)
  - [CISA releases details of Microsoft email hack](#cisa-releases-details-of-microsoft-email-hack)
  - [Insecure credentials leak data from Snowflake](#insecure-credentials-leak-data-from-snowflake)
  - [Disinformation attacks influence EU elections](#disinformation-attacks-influence-eu-elections)
- [Threat landscape in charts](#threat-landscape-in-charts)
  - [Chart one: TPUs by Business Size](#chart-one-tpus-by-business-size)
  - [Chart two: Impact of Attack Type on TPUs](#chart-two-impact-of-attack-type-on-tpus)
  - [Chart three: Top Industries by TPU](#chart-three-top-industries-by-tpu)
  - [Chart four: File Share Abuse Trends](#chart-four-file-share-abuse-trends)
  - [Chart five: Top Vulnerabilities Over Time](#chart-five-top-vulnerabilities-over-time)
...
![Mimecast statistics: 1.7 billion emails, 42,000 customers](Global Threat Intelligence Report cover page with statistics)
...
[^threatcast-1]: 2024 is the biggest election year in history. The Economist. The World Ahead. 23 Nov 2023

## Major events

### Trello leaks 15 million user records[^trello-1]
...
### i-SOON insider leaks hacking info[^isoon-1]
...
### Lockbit operation disrupted by global effort[^lockbit-1]
...
### Disinformation attacks influence EU elections[^eu-elections-1][^eu-elections-2]
...
In May and June, disinformation surrounding European Union governments surged, according to the European Digital Media Observatory, a group focused on countering disinformation. Policymakers and democracy advocates worry that a similar surge in disinformation by foreign adversaries will not be effectively countered in the United States, as right-wing efforts to shut down disinformation clearinghouses have had some effect.

[^cisa-ms-1]: Report on Microsoft Online Exchange Incident. CISA. Department of Homeland Security Advisory. 2 April 2024
[^cisa-ms-2]: Microsoft Actions Following Attack by Midnight Blizzard. Microsoft Security Response Center. Microsoft. 19 Jan 2024
[^snowflake-1]: UNC5537 Targets Snowflake Customer Instances for Data Theft and Extortion. Google Mandiant Blog. 10 June 2024
[^eu-elections-1]: Convergence of Anger’ Drives Disinformation Around E.U. Elections. Tsu, Tiffany. The New York Times. 7 June 2024
[^eu-elections-2]: Stanford’s top disinformation research group collapses under pressure. Menn, Joseph. The Washington Post. 14 June 2024
...
### Chart one: TPUs by Business Size
...
![Chart showing Threats per User by Business Size over time for Small, Medium, Large, and All businesses, with values 40, 30, 20, 10, 0 on Y-axis and 2023 Q1 to 2024 Q2 on X-axis.](Figure 1: Threats per User by Business Size)

Small businesses, however, remained the company size that attackers focused on most.

VIEW YOUR REGION

### Chart two: Impact of Attack Type on TPUs
...
![Chart showing Threats Per User by Attack Type: Spam, Malicious Links, Impersonation, Unknown Malware, Known Malware, with values 25, 20, 15, 10, 5, 0 on Y-axis and Q1 2023 to Q2 2024 on X-axis.](Figure 2a: Threats Per User by Attack Type)

![Chart showing Threats Per User by Attack Type: Known Malware, Unknown Malware, Malicious Links (detailed view), with values 2.0, 1.5, 1.0, 0.5, 0.0 on Y-axis and Q1 2023 to Q2 2024 on X-axis.](Figure 2b: Threats Per User by Attack Type)

VIEW YOUR REGION

### Chart three: Top Industries by TPU
...
![Chart showing Top-5 Industries by Threats Per User: Banking, Travel & Hospitality, Arts & Entertainment, Media & Publishing, Agriculture, Forestry & Fishing, with values 12.00 to 0.00 on Y-axis and Q1 2023 to Q2 2024 on X-axis.](Figure 3: Top-5 Industries by TPU)

VIEW YOUR REGION

### Chart four: File Share Abuse Trends
...
![Chart showing Top-10 File-Sharing Services Used in Attacks: evernote.com, google.com, sharepoint.com, adobe.com, imspublishergroup.com, live.com, gamma.app, airtable.com, publuu.com, flow.page, with values 0 to 3500 on X-axis and services on Y-axis.](Figure 4: Top-10 File-Sharing Services Used in Attacks)

### Chart five: Top Vulnerabilities Over Time
...
![Chart showing Top-10 Vulnerabilities Used in Attacks: CVE-2022-42889, CVE-2021-44228, CVE-2016-7262, CVE-2017-0199, CVE-2017-11882, CVE-2024-21413, CVE-2018-0802, CVE-2018-8414, CVE-2014-1761, CVE-2012-1856, with values 0 to 35000 on X-axis and CVEs on Y-axis.](Figure 5: Top-10 Vulnerabilities Used in Attacks)
...
However, BlackMatter shut down in 2021and its source code subsequently used by other groups, such as LockBit 3.0 and Kasseika. Given previous leaks of ransomware source code and its reuse in other families of ransomware[^blackmatter-1], Mimecast threat researchers gauge it highly likely that parts of BlackMatter’s ransomware code is currently in active use by other groups and affiliates.

[^blackmatter-1]: Lockbit 3.0 has BlackMatter ransomware code, wormable traits Tech target. Alexander Culafi. 30 Nov 2022
...
2.  **Segment the network and log internal traffic**
...
3.  **Harden user credentials, deploy MFA**
...
-   **Third-party threat feeds**
    Leverage bring-your-own threat intelligence to take advantage of any third-party threat feeds for automatic rejection of matching indicators. [Read more](Read more)

-   **End user reporting**
    It is recommended for end users to report potentially malicious messages received through Mimecast user tools to the Mimecast SOC for additional analysis. [Read more](Read more)

-   **Single sign-on**
    It is recommended to utilize single sign-on from your identity provider or leverage Mimecast’s built in multi-factor authentication to reduce an attacker’s ability to leverage email as their attack vector. [Read more](Read more)

-   **DNS authentication policies**
    Ensure DNS authentication policies honor DMARC records. A second policy scoped to a policy group with the DMARC Fail action set to Ignore/Managed and Permitted Senders will provide an effective bypass for any legit mail being rejected/quarantined for DMARC failures. [Read more](Read more)

-   **Re-writing of URLs**
    Setting an aggressive re-writing of URLs will ensure all URLs are scanned upon click, but be aware that anything that looks like a URL will be re-written e.g., IP addresses and internal links. [Read more](Read more)

-   **Auto-Allow Policies**
    Consider setting Auto-Allow policies to ‘strict’ instead of ‘allow’ to ensure that spam scanning is not bypassed at an organizational level for external email recipients. This should be set in conjunction with ‘Auto Allow Spam Detection’ to hold for review to ensure no potentially malicious messages bypass scanning. [Read more](Read more)

-   **Impersonation protection**
    Optimize Impersonation Protection as per best practice guidelines of two hits set to tag Subject/Body and include a separate C-Level/VIP policy based on name match with a hold for admin review. In addition, create another policy for any detections of three hits or more with the admin hold action. [Read more](Read more)

-   **SIEM and XDR vendors**
    Utilize pre-built integrations with the majority of SIEM and XDR vendors to provide log capture and analysis for security policy enforcement. [View Integrations](View Integrations)
...
[mimecast.com](mimecast.com) | ©2024 Mimecast All Rights Reserved | GL-06322
...
from notifications to reports & regional webinars:

[Mimecast Threat Intelligence Hub](Mimecast Threat Intelligence Hub)

### Understand the largest cybersecurity gaps

[State of Email and Collaboration Security Report 2024](State of Email and Collaboration Security Report 2024)