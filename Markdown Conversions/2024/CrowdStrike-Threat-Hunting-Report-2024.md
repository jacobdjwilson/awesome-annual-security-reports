...
| GEORGIA                   | OCELOT    |
| COLOMBIA                  | PANDA     |
| PEOPLE’S REPUBLIC OF CHINA | SAIGA     |
| KAZAKHSTAN                | SPHINX    |
| EGYPT                     | SPIDER    |
| INDIA                     | TIGER     |
| TURKEY                    | WOLF      |
...
![Interactive Intrusions Over Time Graph](https://via.placeholder.com/600x300?text=Interactive+Intrusions+Over+Time+Graph)

### Interactive Intrusions by Motivation
Q3 2023-Q2 2024
![Interactive Intrusions by Motivation Graph](https://via.placeholder.com/600x300?text=Interactive+Intrusions+by+Motivation+Graph)

### Top Verticals by Intrusion Frequency
Q3 2023-Q2 2024
![Top Verticals by Intrusion Frequency Graph](https://via.placeholder.com/600x300?text=Top+Verticals+by+Intrusion+Frequency+Graph)
...
| :----------------------------- | :----------------- | :----- |
| TECHNOLOGY                     | 82%                | 52%    |
| CONSULTING AND PROFESSIONAL SERVICES | 141%               | 109%   |
| HEALTHCARE                     | 160%               | 58%    |
| RETAIL                         | 25%                | 70%    |
| MANUFACTURING                  | 55%                | 57%    |
| TELECOMMUNICATIONS             | 42%                | 84%    |
| GOVERNMENT                     | 93%                | 94%    |
| INDUSTRIALS AND ENGINEERING    | 2%                 | 60%    |
| ACADEMIC                       | 7%                 | 44%    |
| REAL ESTATE                    | 41%                | 75%    |
| MEDIA                          | 36%                | 43%    |
| ENERGY                         | 94%                | 60%    |
...
![MITRE ATT&CK Heat Map](https://via.placeholder.com/800x600?text=MITRE+ATT%26CK+Heat+Map)
...
![Interactive Adversary Disruptions Map](https://via.placeholder.com/800x600?text=Interactive+Adversary+Disruptions+Map)
...
Cross-domain intrusions can vary significantly in complexity, but CrowdStrike commonly sees adversaries moving either back and forth between the endpoint and identity planes or from the cloud to an endpoint. The latter is a particularly dangerous and increasingly prevalent occurrence that is enabled by improvements in phishing and the spread of infostealers. If adversaries can find or steal credentials, they can gain direct access to poorly configured cloud environments, bypassing the need to compromise heavily defended endpoints. From this vantage point, they are then able to find over-privileged users and roles to further compromise cloud environments or use their access to descend into endpoint environments. With this access, they can deploy remote management tools instead of malware, making these attacks challenging to disrupt.
...
![SCATTERED SPIDER Cross-Domain Attack Diagram](https://via.placeholder.com/800x600?text=SCATTERED+SPIDER+Cross-Domain+Attack+Diagram)
...
*   **Hunting for RMM**: RustDesk, AnyDesk, TinyPilot, VS Code Dev Tunnels, Google Chrome Remote Desktop
...
**Reader Note**: Identity data is any information (such as data associated with accounts) that uniquely identifies an individual or entity and includes information regarding authentication and access controls (such as credentials, permissions, security tokens or digital certificates).

### Cloud Hunting

The boundary between the endpoint domain and cloud domain is increasingly blurred as adversaries develop their cross-domain prowess. As the number of organizations moving