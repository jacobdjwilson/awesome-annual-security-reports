...
- [Spotlight On: Artificial Intelligence](#spotlight-on-artificial-intelligence)
...
Even if you’re not directly responsible for protecting your organization from cyberattacks, cybersecurity considerations touch all of us. Communications professionals may need to prepare for public disclosures. Legal departments may benefit from understanding how sensitive data can be put at risk. And human resources departments may need to consider how to protect employees from threat actor harassment.

No matter your role, understanding today’s threat landscape will deepen your ability to meet today’s challenges.

### We’re Ready to Help

If you find yourself wanting an experienced guide through that landscape–whether you’re faced with an incident or considering how to prepare for or prevent one–Unit 42 is always glad to help.
...
> ![Listen to Unit 42’s podcast where we share unique threat intelligence insights, new threat actor TTPs, and real-world case studies. Subscribe Now]
...
Perhaps the highest-impact access method is scamming the target’s IT help desk. A threat actor will call the target’s IT help desk and impersonate a valid user. They will ask for help with resetting the user’s password and/or changing the mobile phone number associated with the account.

Often, the request comes with a “sob story” or other emotion intended to manipulate the help desk worker. This attack only takes a few minutes.
...
### If Social Engineering Doesn't Work...
...
Muddled Libra even attempts to access and use existing endpoint and systems management tools–such as endpoint detection and response (EDR/XDR) consoles, patch management, and the like–for reconnaissance, data theft and malware deployment. And naturally, they use credential-stealing tools such as Mimikatz to help them move laterally and escalate privileges.
...
- **Protect Credentials:** Training end-users not to approve MFA requests they didn’t solicit is important. You can further reduce the human-error burden by requiring number matching as well. Watch for velocity changes in MFA enrollment. Guard against SIM swapping. Watch account creation and reactivation.
- **Monitor Behavior:** Log and analyze the usage patterns of your key security tools. Watch for changes in your cloud infrastructure. Check your code repositories. Watch the behavior of your virtual desktop systems.
- **Know Your Applications:** If you see red-teaming tools in your environment, make sure there is an authorized red-team engagement underway. Watch which remote management tools are being used in your organization. Endpoint management and inventory tools can help here, too.
- **Watch the Network:** Be suspicious of connections to your network from commercial VPN providers. Hold privileged users to a higher standard. Watch outbound access, too. And watch for commercial file-hosting providers.
...
Extortion groups have increasingly sought to gain leverage wherever possible, particularly through multi-extortion tactics. This means that, in addition to encrypting data and holding business operations hostage until a ransom is paid, attackers often pile on additional nasty inducements to push their victims to pay. This may include stealing data and offering it for sale to the highest bidder–often publicly, through dark web leak sites.

In other cases, Unit 42 incident responders have encountered groups that harass an organization’s employees or customers.
...
Attackers may also need to identify where they are and what they can access, move around in the target environment, and create or use credentials to access data.

Ultimately, they can collect and exfiltrate data, adversely impacting the organization. They may also (or instead) choose to make the data and systems unavailable.
...
Spotlight On: Speed Incident Response Report 2024
...
In certain kinds of cases, exfiltration happens In one case involving Black Basta ransomware, it
even faster. While we expected that extortion took attackers less than 14 hours to accomplish
cases would be the fastest to exfiltration, that extensive compromise. Starting with a simple
wasn’t the case. In matters that don’t involve phishing email, the threat group gained initial
extortion, attackers are working within hours more entry to the organization a mere half hour later.
often than not. In 2022 and 2023, the median From there, they explored the network, escalated
time to exfiltration for non-extortion matters has privileges, and began communicating with their
been less than one day. command and control (C2) server).
While we can’t say for sure why this is, it may be Most of this took place while the organization’s
because in some cases data exfiltration is the security team was home, asleep and unaware.
attacker’s main goal. Attacker speed may be By the time the team began to realize something
related to their focus–the faster the threat actors was wrong, the attack was well underway. The
complete their mission, the more money they attackers were exfiltrating terabytes of data.
make. And the faster they work, the less time They had created multiple custom versions of
defenders have to get in their way. ransomware. And those tools were staged on a
...
Latest Attacks between the first detection of the attacker and the earliest evidence
...
Attackers
exfiltrated All other
data in less matters
than a day
54.4%
45.6%
Figure 9. Median dwell time has dropped since 2021,
suggesting that defenders are noticing attackers sooner.
Speed of Detection and On the flip side, even this decrease may not be enough. For
Response is Key example, in the US, the SEC adopted a rule requiring that material
...
legal have even assembled a what to say to customers, regulators, and the market.
...
After the Compromise Incident Response Report 2024
Persistence
...
After the Compromise Incident Response Report 2024
These measures are an attempt to minimize the risk of being profiled by IP scanners.
...
cloud services to get that initial access to find the
credentials they need.
4411

Spotlight On: Cloud Incidents Incident Response Report 2024
Sheltering from the Clouds Planning for a Rainy Day
Many organizations believe that choosing cloud services This increase of interest in cloud
obviates the need for them to think about security. adoption for both organizations and
And in fairness, many cloud service providers sell their threat actors gives us some unique
...
three-quarters (76%) of organizations don’t enforce MFA There are several things you can focus on to
for console users, and 58% of organizations don’t enforce make a big difference in your cloud security:
MFA for root or admin users.
• Enforce MFA. Having proper
While every organization has a small set of risky behaviors authentication on cloud service accounts
that are repeatedly observed in their cloud workloads, that is a way to make things harder for threat
also means that a few changes will make a big difference actors to access your environment.
...
Forecasts Include Increasing Cloud Threat Report, personally identifiable
...
is still more cloud coverage in our immediate forecast. • Monitor abnormal activity. If services
Threat actors know this and are adapting their techniques within your cloud account are accessing
to take advantage of this change in infrastructure. or being accessed by new and unusual
IP addresses or over unusual ports,
Figure 10 shows that we’ve seen an increase in incident make sure your monitoring is configured
responses involving cloud cases, from 6% in 2021 to 16.6% to alert on this activity.
...
After the Compromise Incident Response Report 2024
Credential Access Collection and Exfiltration
We continue to see threat actors using Over the past year, we saw vulnerabilities
credential-dumping tools such as Mimikatz and weaponized and exploited more rapidly and
MiniDump to extract user account credentials and on a larger scale than in prior years. We have
password hashes. They also use tools targeting observed a trend toward the automation of
Windows Active Directory domain controllers campaigns–similar to the “scaling up” process
themselves. Endpoint detections are the best way seen in manufacturing–from prototype to mass
to find these. Investigate them promptly when production.
they occur.
The first attempts at exploitation are conducted
And watch for customized versions that will interactively, by hand. Then, there’s a progression
evade hash-based detection. We observed a of automation from tools to assist human
threat actor using their own version of Mimikatz, operators to machine-driven exploitation of the
requiring a password on the command line and weaknesses.
...
More importantly, attackers are shifting more the collection and exfiltration of the data.
...
Incident Response Report 2024
Common Contributing Factors Too Much Complexity
...
Organizations chronically lack comprehensive This data issue gets worse when teams struggle
visibility across their network, cloud, and to maintain expertise in the myriad of tools
endpoints. This shortfall led to critical they’ve adopted for security operations. As a
unmonitored areas, delaying the detection of and result, even when the early signs of an attack
response to active threats. exist, the signal is lost in the complexity of the IT
landscape.
Lack of visibility also led to uncontrolled and
unmanaged vulnerabilities. In 11.5% of the Defenders miss critical attack indicators when
incidents, insufficient patch management was a they’re overwhelmed by the sheer volume of
contributing factor. information and operational demands. They can’t
...
workloads. These exposures contributed to 9.6% Allocating excess privileges to accounts is a
of cases. too-common contributing factor in cybersecurity
...
it effectively–significantly contributed to incident
...
was a contributing factor. Finally, the lack of MFA remains significant. In
2023, the absence of MFA was a contributing
factor in over a third of incidents.
49

Recommendations for Defenders Incident Response Report 2024
...
To make matters worse, the modern attack surface is in constant flux. On average, 20% of
...
Unit 42 Services: Palo Alto Network Products:
...
Recommended Recommended
Unit 42 Services: Palo Alto Network Products:
•Cyber Risk Assessment •Strata™ and
CDSS
•Prisma Cloud
•Prisma SASE
•Prisma Access
...
Recommendations for Defenders Incident Response Report 2024
Reduce Mean Time to Detect and Mean And these results aren’t unique to us. Our
Time to Respond Using Security Tool customers have reported a sixfold rise in the
Consolidation, Analytics and Automation volume of security data they ingest and analyze
...
from endpoints, networks, and cloud •SOC Assessment
...
detection and response times, shifting the Recommended
advantage back to the defenders. Palo Alto Network Products:
•Cortex XDR
In our own networks, we have put this into
practice: •Cortex Xpanse
...
Recommended Recommended
Unit 42 Services: Palo Alto Network Products:
•Compromise Assessment •Cortex Xpanse
•SOC Assessment •Cortex XDR
•Managed Threat Hunting •Cortex XSIAM
•Managed Detection and
Response
56
...
Recommendations for Defenders Incident Response Report 2024
Improve Resilience and Speed Recovery Refresh the incident response plan. A
with System Redundancies and Specialized well-documented incident response plan
Expertise outlines roles, procedures, and timelines. The
...
Immutable, usable backups preserve value. Recommended
Ransomware attackers target backups, so it’s Unit 42 Services:
imperative to protect them through segmentation,
whether that means storing them offline or using •Breach Readiness
other access controls. Regularly test and confirm Review
the effectiveness of your backups. Assess how • Tabletop Exercises
...
is the only viable option. This process–often •Purple Teaming Exercises
...
Recommendations for Defenders Incident Response Report 2024
Attack Surface Management Cortex XDR also uses AI-driven technology
...
Cortex Xpanse is an active ASM solution that vulnerabilities in your organization.
...
Reactively, we use Cortex Xpanse during develop that strategy, too.) Then you can move
incident response to find a client’s entire forward with implementation more confident that
attack surface. Several times, it’s shown us an you’re strengthening your defenses.
...
defend your organization. In our own Palo Alto Networks SOC, we
...
Lack of centrally managed endpoint protection analysts an average of 2,600 hours per month.
was a significant contributing factor in 2023 If you have invested in creating playbooks and
incidents. Attackers are great at finding the processes but struggle to get them automated,
unprotected sections of the infrastructure. XSOAR is a great place to start.
Naturally, we recommend Cortex XDR as our first Unit 42 Incident Response offers several
choice. It’s an extended detection and response proactive assessments that relate to automation.
platform that integrates data from the endpoint If you’ve built or are thinking about building a
as well as other sources. We particularly like next-generation SOC, our SOC Assessment can
how it can stitch together endpoint, network, help you design and build that capability. If you
cloud, and identity data. That helps with attack already have processes in place, consider our
detection, and it also simplifies our investigations Ransomware Readiness Assessment or BEC
by putting all the data together for us. It can do Readiness Assessment to evaluate them.
the same for you.
60
...
Define and practice your incident response process. The most effective incident
...
against them. For even more benefit, consider a Purple Teaming Exercise, where our
...
Spotlight On: Muddled Libra Incident Response Report 2024
AI and Social Engineering DevOps Expertise Will Expand
Further
...
around social engineering. We expect attackers will demonstrate their skills and
...
And we expect to see this activity play out within environment – without needing malware.
...
the resulting detection gap. Now is the time Large Language Models Will
...
Encrypted Data Stockpiles anyone who needs to create large volumes of text.
LLMs are good at creating plausible text, even if it’s
Quantum computing is still a decidedly not always grounded in reality.
...
We expect to see attackers harvesting and storing attackers’ own purposes.
...
Organizations should take stock of the encrypted verified data–not conjecture or hallucination–will
data under their control, noting what has been remain the best way to operate in an increasingly
stolen in the past that could become unprotected untrustworthy information environment.
...
A Few Words About the Data Incident Response Report 2024
