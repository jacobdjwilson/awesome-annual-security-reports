...
- [The Most Frequently Triggered Detection Rules](#the-most-frequently-triggered-detection-rules)
...
Figure 1: Most attacked industries
![Figure 1: Most attacked industries graph showing number of customers, number of reported incidents, and number of reported high-severity incidents by industry sector.]
...
Figure 2: Kaspersky MDR alerts processing funnel
![Figure 2: Kaspersky MDR alerts processing funnel diagram showing the flow from security alerts received to incidents reported to customers.]
...
|---|---|---|
| High | 53.99 min | The most complex incidents require more time to collect additional information and build an incident timeline. In 2024, this time increased by approximately 48% compared to previous periods2, reflecting the nature of high-severity incidents during the year. |
| Medium | 41.03 min | Medium-severity incidents were the most frequent severity level. Most of these incidents were caused by malware activity, and fully automated remediation proved effective. However, the time required increased by 26% compared to 2024, due to a slight increase in the number of medium-severity incidents in 2024. |
| Low | 37.85 min | Incidents with the lowest severity were mostly related to the consequences of potentially unwanted software. in most cases, processing these incidents was largely automated. |
...
Figure 3: Incident severity level
![Figure 3: Incident severity level pie chart for 2024 showing the distribution of High, Medium, and Low severity incidents.]

Figure 4: Severity of incidents detected by MDR over the years
![Figure 4: Bar chart showing the severity of incidents detected by MDR over the years from 2020 to 2024.]
...
Figure 5: Distribution of expected number of incidents from 10,000 endpoints by severity and industry
![Figure 5: Stacked bar chart showing the distribution of expected number of incidents from 10,000 endpoints by severity and industry.]

The diagram shows that the highest relative number of incidents occurred in the mass media, development, and
telecoms industries.

Figure 6: Distribution of expected number of incidents from 10,000 endpoints by severity and industry compared to the previous year
![Figure 6: Stacked bar chart comparing the distribution of expected number of incidents from 10,000 endpoints by severity and industry to the previous year.]
...
Figure 7: The expected number of critical incidents from 10,000 endpoints by industry compared to the previous year
![Figure 7: Bar chart showing the expected number of critical incidents from 10,000 endpoints by industry compared to the previous year.]
...
Figure 8: Distribution of incidents by number of relevant alerts
![Figure 8: Pie chart showing the distribution of incidents by the number of relevant alerts (1 alert, 2-10 alerts, >10 alerts).]

Figure 9: Distribution of incidents by severity and number of relevant alerts
![Figure 9: Stacked bar chart showing the distribution of incidents by severity (High, Medium, Low) and the number of relevant alerts.]
...
![Figure 10: Bar chart showing the number of critical incidents by type.]

Figure 11: The number of companies where critical incidents were observed, by type
![Figure 11: Bar chart showing the number of companies where critical incidents were observed, by type.]
...
![Figure 12: Heatmap showing the number of high-severity incidents by type and industry.]
...
![Figure 13: Heatmap showing the number of MDR customers that experienced high-severity incidents by industry.]
...
Figure 14: Number of attacked organizations across sectors and within a sector
![Figure 14: Heatmap showing the number of attacked organizations across sectors and within a sector.]
...
![Figure 15: Stacked bar chart showing the distribution of adversary tactics by incident severity (High, Medium, Low).]

Adversary tactics that Kaspersky uses to detect incidents:

*   **TA0043: Reconnaissance**: Incidents detected at this stage are mainly related to various types of scans. The
    ...
    at this stage.
*   **TA0042: Resource Development**: Incidents attributed to this tactic are primarily associated with the detection
...
*   **TA0001: Initial Access**: The vast majority of incidents detected at this stage involve phishing emails containing
    ...
    remediation.
*   **TA0002: Execution**: Because launching specialized attack tools tends to be noisy, the largest number
...
*   **TA0003: Persistence**: Incidents at this stage include the substitution of accessibility features, suspicious
...
*   **TA0004: Privilege Escalation**: The vast majority of incidents where this was the earliest tactic — adding an account
    ...
    to bypass UAC.
*   **TA0005: Defense Evasion**: A relatively small percentage of incidents are detected at this stage, but the variety
...
*   **TA0006: Credential Access**: The vast majority of incidents related to this tactic are attempts
    to access LSASS process memory, dumps of sensitive registry hives, detects on different types of keyloggers, brute force
    or password spraying attempts. As in the previous case,
    incidents identified here are rarely false positives, with the exception of some types of confirmed cyber exercises.
*   **TA0007: Discovery**: Detection at this stage is associated with a high number of false positives, so there are
...
*   **TA0008: Lateral Movement**: As Lateral Movement has a low false positive rate, it is promising tactic for planning the
...
*   **TA0009: Collection**: Observed activity at this stage is based on detection of special tools. Some incidents
    were also identified by an anomaly detection engine powered by machine learning.
*   **TA0010: Exfiltration**: In 2024, only a few incidents reached this stage. The detected incidents are extremely
    difficult to distinguish from TA0011, as the most common scenario is T1041: Exfiltration over C2 channel13 using standard application layer protocols. Incidents were attributed
...
*   **TA0011: Command and Control**: The vast majority of detections at this stage were made based on Threat Intelligence:
    ...
    this category.
*   **TA0040: Impact**: In this tactic, most incidents are identified through the detection of specific
...
12 MITRE ATT&CK. S0521 BloodHound
13 MITRE ATT&CK. T1041 Exfiltration Over C2 Channel
14 MITRE ATT&CK. S0154 Cobalt Strike
15 MITRE ATT&CK. S0521 BloodHound
16 MITRE ATT&CK. T1041 Exfiltration Over C2 Channel
...
Figure 16: Proportion of incidents detected by different types of sensors:
![Figure 16: Pie chart showing the proportion of incidents initially detected by EPP, NIDS, and SB sensors.]

The high efficiency of the Sandbox at the TA0001: Initial Access stage is driven by KATA’s common use case
of detecting phishing attacks at the network perimeter. The network IDS is efficient at the TA0011: Command and
control stage. In addition to these scenarios, the IDS is working well detecting network scans, which explains
...
Table 2: The most popular LOLBins and the frequency of their usage
| LOLBin | All incidents | High-severity incidents |
|---|---|---|
| powershell.exe | 10.51% | 1.64% |
| rundll32.exe | 6.85% | 0.81% |
| comsvcs.dll | 3.82% | 0.26% |
| reg.exe | 2.07% | 0.23% |
| msiexec.exe | 1.59% | 0.67% |
| certutil.exe | 1.59% | 0.15% |
| mshta.exe | 1.43% | 0.22% |
| msbuild.exe | 1.27% | 0.07% |
| esentutl.exe | 1.