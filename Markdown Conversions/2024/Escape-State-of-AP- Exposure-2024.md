...
## Table of Contents
  ...
  * [THE CONSEQUENCES OF UNSECURED APIS EXPOSED IN THE WILD](#the-consequences-of-unsecured-apis-exposed-in-the-wild)
...
This vulnerability allowed unauthorized
access to approximately 49 million customer
records, including names, physical
addresses, and order information. The
...
## Methodology
API specifications found in the wild
...
## Methodology
API services scanned for vulnerabilities
...
2.  **Fuzzing and Payload Injection:** In the next stage, the algorithm uses fuzzing techniques to
    test each endpoint by sending random, unexpected, or malformed inputs. This helps identify weaknesses like input validation flaws, injection vulnerabilities, and misconfigurations. The algorithm adapts these payloads based on real-time feedback, simulating complex attacks more effectively than conventional static methods.
3.  **Behavioral Monitoring:** As it interacts with each endpoint, the algorithm monitors response
    patterns and behaviors to detect anomalies. By observing responses over multiple interactions, it can identify issues such as data exposure, error leakage, and misconfigured permissions, providing insights into the API’s security posture.
4.  **Risk Prioritization:** Escape’s DAST concludes with an analysis that ranks detected
    vulnerabilities based on risk and classifies them based on the metric named Escape severity.

You can find an in-depth technical explanation of the algorithm [here](link-to-algorithm-explanation).  
...
*Image description: A diagram illustrating the event-based architecture of the Escape Security Scanner. The diagram shows a central loop labeled "Reinforcement Loop" with arrows indicating the flow of data and actions. Around the loop are various components such as "API Discovery", "API Specification Generation", "API Security Scanning", and "Vulnerability Reporting". The diagram highlights the iterative and continuous nature of the security scanning process.*
...
*A bar chart showing the distribution of domains exposing different ranges of API services. The x-axis represents ranges of API services (0-10, 11-50, 51-100, 101-200, 200+), and the y-axis represents the number of domains. The chart shows that most domains expose a smaller number of APIs, with fewer domains exposing a large number of APIs.*

*A pie chart showing the distribution of cloud providers used by the analyzed domains. The chart shows that AWS is the most used provider (51.1%), followed by Azure (16%), Cloudflare (10.7%), Akamai (8.2%), GCP (7.5%), Fastly (4.9%), and IBM (0.6%).*
...
*A bar chart showing the distribution of highly critical vulnerabilities across Fortune 1000 and CAC 40 organizations. The chart shows that the Fortune 1000 organizations have a higher number of highly critical vulnerabilities compared to CAC 40 organizations.*
...
*A bar chart showing the alignment of high-risk Fortune 1000 vulnerabilities with the OWASP Top 10 2023 classification. The chart shows the number of vulnerabilities for each category: API8:2023 (Security Misconfiguration), API2:2023 (Broken Authentication), API9:2023 (Improper Assets Management), API7:2023 (Insufficient Logging & Monitoring), and API1:2023 (Broken Object Level Authorization).*
...
*A bar chart showing the number of different types of secrets found. The chart displays the count for each secret type, such as AWS Access Token, Bearer Token, Generic API Keys, etc.*
...
*A pie chart showing the alignment of high-risk Fortune 1000 vulnerabilities with the OWASP Top 10 2023 classification. The chart shows the percentage of vulnerabilities for each category: API8:2023 (Security Misconfiguration - 62.2%), API2:2023 (Broken Authentication - 31.8%), API7:2023 (Insufficient Logging & Monitoring - 2.7%), and API9:2023 (Improper Assets Management - 2.5%).*
...
1.  Configure the Spring Boot Actuator settings to disable endpoints like `/env`, `/mappings`, and `/httptrace` in the production environment, or at least restrict them to authorized, internal users only.

**Obfuscate Sensitive Data in /env:**
2.  In cases where `/env` must be used, consider redacting or obfuscating sensitive environment variables, such as database credentials and API keys, to prevent accidental exposure.

**Monitor and Log Access Attempts to Actuator Endpoints:**
3.  Implement logging and monitoring of all access attempts to these endpoints, enabling detection of any unauthorized access and facilitating incident response.
...
4.  Implement logging and monitoring of all access attempts to these endpoints, enabling
...
*A bar chart showing the top 5 CVE occurrences. The x-axis represents the CVE IDs, and the y-axis represents the number of occurrences. The chart shows that CVE-2024-5535 has the highest number of occurrences (98), followed by CVE-2022-2274 (70), CVE-2022-2068 (70), CVE-2022-1292 (57), and CVE-2021-3711 (57).*
...
exposed, we recommend taking the following actions immediately:
...
*   **Conduct a Comprehensive API Audit:** Review all public-facing APIs to identify any
    ...
    , development, and unused APIs that may have been left accessible. Categorize APIs based on sensitivity, usage, and the type of data they handle. This helps prioritize which APIs require immediate attention and remediation.
*   **Deactivate Unused or Duplicate APIs:** Regularly review and disable APIs that are no
     longer in use or required. This practice helps reduce the attack surface of your
    application and minimizes potential security risks associated with outdated or redundant APIs.
*   **Recheck for Exposed API Secrets:** Conduct a thorough check to ensure that API keys,
     tokens, and other secrets are