...
* [Top Takeaways](#top-takeaways)
    * [The rise of supply chain attacks](#the-rise-of-supply-chain-attacks)
    * [The shift to impact and high-profile attacks vs. stealth and long dwell-time attacks](#the-shift-to-impact-and-high-profile-attacks-vs-stealth-and-long-dwell-time-attacks)
    * [Public and private sector collaborations become more common](#public-and-private-sector-collaborations-become-more-common)
    * [The quick trickling down on zero-day](#the-quick-trickling-down-on-zero-day)
    * [Cloud as a gateway for attackers](#cloud-as-a-gateway-for-attackers)
...
* [Interesting trends and campaigns in 2021](#interesting-trends-and-campaigns-in-2021)
    * [Python and Go Malware became more common](#python-and-go-malware-became-more-common)
    * [RedLine Stealer ups its game](#redline-stealer-ups-its-game)
    * [The use of cloud service providers as part of malware infection chain](#the-use-of-cloud-service-providers-as-part-of-malware-infection-chain)
...
* [Attacks on Microsoft Exchange Servers](#attacks-on-microsoft-exchange-servers)
    * [ProxyShell](#proxyshell-1)
    * [CVE-2021-42321](#cve-2021-42321)
* [Deep Instinct discoveries in 2021](#deep-instinct-discoveries-in-2021)
    * [LSAAS memory dumps](#lsaas-memory-dumps)
    * [Identifying Excel 4.0 Macros strains using anomaly detection](#identifying-excel-40-macros-strains-using-anomaly-detection)
    * [Emotet re-emerges with new variants](#emotet-re-emerges-with-new-variants)
...
* [Cyber Insights: A Look Back at Our 2021 Predictions](#cyber-insights-a-look-back-at-our-2021-predictions)
    * [COVID-19 aftereffects](#covid-19-aftereffects)
    * [Proliferation of botnets and Access-as-a-Service](#proliferation-of-botnets-and-access-as-a-service)
    * [Organized cybersecurity cooperation between government and private enterprise](#organized-cybersecurity-cooperation-between-government-and-private-enterprise)
    * [Escalating adoption of adversarial machine learning malware](#escalating-adoption-of-adversarial-machine-learning-malware)
    * [Ransomware to target mission-critical organizations](#ransomware-to-target-mission-critical-organizations)
* [Cyber Insights: 2022 Predictions](#cyber-insights-2022-predictions)
    * [VPN as a breach vector](#vpn-as-a-breach-vector)
    * [Prepare for more supply chain attacks](#prepare-for-more-supply-chain-attacks)
    * [The ongoing rise of Malware-As-a-Service](#the-ongoing-rise-of-malware-as-a-service)
    * [Defense evasion becoming a greater focus](#defense-evasion-becoming-a-greater-focus)
    * [Health sector increasingly targeted](#health-sector-increasingly-targeted)
...
### The shift to impact and high-profile attacks vs. stealth and long dwell-time attacks
...
### Public and private sector collaborations become more common
...
*The following is a description of a graph. The graph shows the number of new malware samples each month, since January 2020, grouped by malware type and shown in arbitrary units. The amount of spyware samples in January 2020 is represented as one. The x-axis shows the months from January 2020 to December 2021. The y-axis shows arbitrary units. The graph contains multiple lines, each representing a different malware type: Backdoor, Ransomware, Dropper, Spyware, Virus, Worm, and Miner. The graph shows that all malware types have increased since January 2020, with a significant spike in all malware types in 2021. Spyware shows a decrease in early 2021, likely due to the Emotet takedown.*
...
*The following is a description of a pie chart. The pie chart shows the top 5 malware families of 2021. The chart contains 5 slices, each representing a different malware family. The slices are: Dridex (58%), AgentTesla (10%), Emotet (13%), TrickBot (10%), and IcedID (9%).*
...
We have seen changes in new Emotet variants, from using a different communication protocol with a constantly changing decryption routine, to the abuse of an old Excel capability (Excel 4.0), to execute the malicious macro.
...
*The following is a description of a bar chart. The bar chart shows the top 5 ransomware families in 2021 based on data from Deep Instinct’s D-Cloud. The numbers are shown in arbitrary units, where the number of DarkSide samples is represented by one. The x-axis shows arbitrary units. The y-axis shows the ransomware families: STOP, Cerber, REvil, Conti, and DarkSide. The bars show that STOP is the most prevalent ransomware family, followed by REvil, Conti, Cerber, and DarkSide.*
...
Conti is operated in a RaaS manner by the Russian cybercrime group Wizard Spider and is spread using exploits for vulnerable firewalls, through mass phishing email campaigns, or as a secondary payload of TrickBot. Conti encrypts all files except PE files, system files, and shortcuts in the victims’ local storage or remote SMB networks. It can also exfiltrate sensitive data and stored credentials to be used in its double-extortion practice.
...
In the next stage, several sensitive artifacts are exfiltrated and PowerShell is used to download the DarkSide payload, “update.exe,” to several locations on the victim’s computer, including a network share created by the attackers. After patient zero is fully infected, the threat actors move laterally in the environment with the goal of reaching the Domain Controller (DC). If they succeed, they exfiltrate sensitive information, such as files and the SAM registry hive. They copy “update.exe” from the previously created network share into the DC, use Task Scheduler to set an execution time, and copy the ransomware payload to yet another network share, which this time resides on the DC and is used to transfer the ransomware to other targets in the environment for maximum damage.
...
*The following is a description of a pie chart. The pie chart shows the top 5 Linux malware families of 2021. The chart contains 6 slices, each representing a different malware family. The slices are: Mirai (13.72%), Gafgyt (5.17%), Other (0.47%), XorDDoS (80.64%), Tsunami (0.22%), and Hajine (0.25%).*
...
*The following is a description of a line chart. The line chart shows the number of samples per malware that were seen in D-Cloud each month in 2021. The numbers are shown in arbitrary units, where the amount of QakBot samples in August is represented by one. The x-axis shows the months from January 2021 to December 2021. The y-axis shows arbitrary units. The chart contains 5 lines, each representing a different malware family: Dridex, QakBot, IcedID, TrickBot, and Zloader. The chart shows that Dridex is the most prevalent financial malware family throughout the year, with other families showing varying levels of activity.*

*The following is a description of a pie chart. The pie chart shows the top 5 financial malware families of 2021. The chart contains 5 slices, each representing a different malware family. The slices are: Dridex (68%), IcedID (11%), TrickBot (10%), Zloader (8%), and QakBot (3%).*
...
This malware usually spreads via mass email campaigns. Dridex uses malicious email attachments that include either a Word document containing a malicious macro or a PDF that utilizes a malicious JavaScript. Following successful infection, Dridex will collect and deliver banking information, credit card data, credentials, and additional sensitive data found on the victims’ computer to its’ C&C servers. Other variants include a crypto-currency wallet credential stealing mechanism. On several occasions the Dridex infection infrastructure has also been used to spread other financial malware/spyware such as TrickBot and Emotet, sharing the same droppers or dropping each other as a secondary payload.
...
Zloader also utilizes different techniques — its main modules