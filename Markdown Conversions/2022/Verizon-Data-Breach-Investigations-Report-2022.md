...
### Table of contents
1. [DBIR Master’s Guide](#dbir-masters-guide)
2. [Introduction](#introduction)
3. [Summary of findings](#summary-of-findings)
4. [Results and Analysis](#results-and-analysis)
    1. [Introduction](#results-and-analysis-introduction)
    2. [Actor](#actor-friends-in-low-places)
    3. [Actions](#actions)
    4. [Assets](#assets)
    5. [Attribute](#attribute)
    6. [Timeline](#timeline)
5. [Incident Classification Patterns](#incident-classification-patterns)
    1. [Introduction](#incident-classification-patterns-introduction)
    2. [System Intrusion](#system-intrusion)
    3. [Scratching the Surface](#scratching-the-surface)
    4. [Social Engineering](#social-engineering)
    5. [Basic Web Application Attacks](#basic-web-application-attacks)
    6. [Miscellaneous Errors](#miscellaneous-errors)
    7. [Denial of Service](#denial-of-service)
    8. [Lost and Stolen Assets](#lost-and-stolen-assets)
    9. [Organic Free-Range Data](#organic-free-range-data)
    10. [Privilege Misuse](#privilege-misuse)
6. [Industries](#industries)
    1. [Introduction](#introduction-1)
    2. [Accommodation and Food Services](#accommodation-and-food-services)
    3. [Arts, Entertainment and Recreation](#arts-entertainment-and-recreation)
    4. [Educational Services](#educational-services)
    5. [Financial and Insurance](#financial-and-insurance)
    6. [Healthcare](#healthcare)
    7. [Information](#information)
    8. [Manufacturing](#manufacturing)
    9. [Mining, Quarrying, and Oil & Gas Extraction + Utilities](#mining-quarrying-and-oil--gas-extraction--utilities)
    10. [Professional, Scientific and Technical Services](#professional-scientific-and-technical-services)
    11. [Public Administration](#public-administration)
    12. [Retail](#retail)
    13. [Very Small Business](#very-small-business-cybercrime-protection-sheet)
7. [Regions](#regions)
    1. [Introduction](#introduction-2)
    2. [Asia Pacific (APAC)](#asia-pacific-apac)
    3. [Europe, Middle East and Africa (EMEA)](#europe-middle-east-and-africa-emea)
    4. [Northern America (NA)](#northern-america-na)
    5. [Latin America and the Caribbean](#latin-america-and-the-caribbean)
8. [Wrap-up](#wrap-up)
    1. [Year in review](#year-in-review)
9. [Appendices](#appendices)
    1. [Appendix A: Methodology](#appendix-a-methodology)
    2. [Appendix B: VERIS and Standards](#appendix-b-veris-and-standards)
    3. [Appendix C: Changing Behavior](#appendix-c-changing-behavior)
    4. [Appendix D: U.S. Secret Service](#appendix-d-us-secret-service)
    5. [Appendix E: Ransomware Pays](#appendix-e-ransomware-pays)
    6. [Appendix F: Contributing Organizations](#appendix-f-contributing-organizations)
...
Learn more here:
*   [github.com/vz-risk/dbir/tree/gh-pages/2022](github.com/vz-risk/dbir/tree/gh-pages/2022) – DBIR facts, figures and figure data.
*   [veriscommunity.net](veriscommunity.net) features information on the framework with examples and enumeration listings.
*   [github.com/vz-risk/veris](github.com/vz-risk/veris) features information on the framework with examples and enumeration listings
...
*Figure 1. Example slanted bar chart (n=205)*. The image shows a slanted bar chart with multiple bars, each with a different slant, representing the uncertainty of the data.

*Figure 2. Example spaghetti chart*. The image shows a spaghetti chart with multiple lines, each with a different path, representing the range of possible values.

*Figure 3. Example dot plot (n=672). Each dot represents 0.5% of organizations. Orange: lower half of 80%. Yellow: upper half of 80%. Green: 80%-95%. Blue: Outliers. 95% of orgs: 148 - 1,594,648. 80%: 1,274 - 438,499. Median: 29,774 (log scale)*. The image shows a dot plot with multiple colored dots, each representing a percentage of organizations, with different colors indicating different ranges.

*Figure 4. Example pictogram plot (n=4,110). Each glyph represents 40 breaches.*. The image shows a pictogram plot with multiple glyphs, each representing a number of breaches.
...
### Summary of findings
*Figure 5. Select enumerations in non-Error, non-Misuse breaches (n=4,250)*. The image shows a pictogram plot with multiple glyphs, each representing a number of breaches, with different glyphs representing different enumerations.

*Figure 6. Ransomware over time in breaches*. The image shows a line chart with multiple lines, each representing the trend of ransomware over time.

*Figure 7. Partner vector in System Intrusion incidents (n=3,403) Each glyph represents 25 incidents.*. The image shows a pictogram plot with multiple glyphs, each representing a number of incidents, with different glyphs representing different partner vectors.
...
*Figure 8. Misconfiguration over time in breaches*. The image shows a line chart with multiple lines, each representing the trend of misconfiguration over time.

*Figure 9. The human element in breaches (n=4,110) Each glyph represents 25 breaches.*. The image shows a pictogram plot with multiple glyphs, each representing a number of breaches, with different glyphs representing different aspects of the human element.
...
*Figure 10. Sources of Data Breaches (2008 DBIR Figure 3)*. The image shows a bar chart comparing different sources of data breaches.

*Figure 11. Actors in breaches (n=5,146)*. The image shows a slanted bar chart comparing different actors in breaches.
...
*Figure 12. Median Number of Records Compromised (2008 DBIR Figure 5)*. The image shows a bar chart comparing the median number of records compromised by different actors.

*Figure 13. Records by Actor in breaches (n=54)*. The image shows a slanted bar chart comparing the number of records compromised by different actors.
...
*Figure 14. Motive in external agents by percent of breaches within external (2012 DBIR Figure 15)*. The image shows a bar chart comparing the motives of external agents in breaches.

*Figure 15. Motives in External actors by org size*. The image shows a dot plot comparing the motives of external actors by organization size.
...
*Figure 16. Top Action vectors in incidents (n=18,419)*. The image shows a slanted bar chart comparing the top action vectors in incidents.

*Figure 17. Top Action varieties in incidents (n=18,511)*. The image shows a slanted bar chart comparing the top action varieties in incidents.
...
*Figure 18. Top Action vectors in breaches (n=3,279)*. The image shows a slanted bar chart comparing the top action vectors in breaches.

*Figure 19. Top Action varieties in breaches (n=3,875)*. The image shows a slanted bar chart comparing the top action varieties in breaches.

*Figure 20. Threat Categories (2008 DBIR Figure 9) Dark = Caused, Light = Contributed*. The image shows a bar chart comparing different threat categories, with dark bars representing the direct cause of the breach and light bars representing contributing factors.

*Figure 21. Actions in breaches (n=5,212)*. The image shows a slanted bar chart comparing the actions in breaches.
...
*Figure 22. Assets in breaches (n=4,384)*. The image shows a slanted bar chart comparing the assets in breaches.

*Figure 23. Compromised Assets (2008 DBIR Figure 18)*. The image shows a bar chart comparing different types of compromised assets.

*Figure 24. Top Asset varieties in breaches (n=2,796)*. The image shows a slanted bar chart comparing the top asset varieties in breaches.

Figure 22 illustrates that the top varieties of Assets impacted in breaches are Servers, People and their devices. When we start to zoom into the specific types of servers (Figure 24) we find Web application (56%) and Mail (28%) servers accounting for the top two varieties, which is rather intuitive when one considers that email servers and web applications are the Assets that are most likely to be internet-facing. As such, they provide a useful venue for attackers to slip through the organization’s “perimeter” by using clever tricks like (spoiler alert) stolen credentials. Dropping down the list a bit farther to the folks that are socially engineered. These are commonly the individuals who deal with company money and have the ability to do things with it (like update where it is deposited).
...
### Attribute
*Figure 25. Attributes over time in incidents*. The image shows a line chart comparing different attributes over time in incidents.
...
*Figure 26. Compromised Data Types (2008 DBIR Figure 20)*. The image shows a bar chart comparing different types of compromised data.

*Figure 27. Top Confidentiality data varieties over time in breaches*. The image shows a line chart comparing the top confidentiality data varieties over time in breaches.
...
*Figure 28. Top Availability varieties in breaches (n=1,109)*. The image shows a slanted bar chart comparing the top availability varieties in breaches.

Once attackers are inside the victim’s network they often install malware, which violates the Integrity of a system (as does any other