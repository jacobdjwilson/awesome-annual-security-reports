...
## Contents
[Letter from the CTO](#letter-from-the-cto)	2
[The future of ransomware](#the-future-of-ransomware)	4
[Ransomware-as-a-service subsumes attacks by solo groups](#ransomware-as-a-service-subsumes-attacks-by-solo-groups)	4
[Expanding extortion](#expanding-extortion)	6
[Malware begets malware](#malware-begets-malware)	8
[The rise of Cobalt Strike](#the-rise-of-cobalt-strike)	8
[Malware distribution frameworks](#malware-distribution-frameworks)	9
[Shotgun attacks, with pinpoint targeting](#shotgun-attacks-with-pinpoint-targeting)	10
[Security and AI in 2022 and beyond](#security-and-ai-in-2022-and-beyond)	12
[AI in 2021](#ai-in-2021)	12
[AI is increasingly accessible to threat actors](#ai-is-increasingly-accessible-to-threat-actors)	12
[The ongoing surprises from AI](#the-ongoing-surprises-from-ai)	13
[Unstoppable mobile malware](#unstoppable-mobile-malware)	15
[Catching Flubot: it’s pretty serious](#catching-flubot-its-pretty-serious)	15
[Fake iPhone finance apps steal millions from vulnerable users](#fake-iphone-finance-apps-steal-millions-from-vulnerable-users)	16
[Why so serious about Joker Android malware?](#why-so-serious-about-joker-android-malware)	18
[Infrastructure under attack](#infrastructure-under-attack)	19
[Initial access brokers deliver victims to attackers](#initial-access-brokers-deliver-victims-to-attackers)	19
[New threats target Linux, IoT devices](#new-threats-target-linux-iot-devices)	20
[Attackers turn to commercial tools](#attackers-turn-to-commercial-tools)	21
[The year of computing dangerously](#the-year-of-computing-dangerously)	22
[Malware bypasses international sanctions](#malware-bypasses-international-sanctions)	23
...
*Figure 1. While ransomware attack response accounted for most of the incidents the Sophos Rapid Response team was involved in during the past year, it didn’t account for them all. Removal of Cobalt Strike Beacons, cryptominers, and even web shells also prompted extra attention, especially in the days following the revelations of the ProxyLogon, and later ProxyShell, exploits, which resulted in a lot of people quickly becoming familiar with how dangerous a web shell could be.*

The figure is a pie chart showing the reasons for Sophos Rapid Response incident response engagements in 2020-2021. The largest slice, 79%, is labeled "Ransomware". Other slices include "Cobalt Strike" (6%), "Web shells" (4%), "Data exfiltration" (3%), "Miners" (3%), and "Misc. malware" (5%).
...
*Figure 2. Nearly four in five calls to Sophos Rapid Response service came as the result of a ransomware attack, and among those calls, Conti was the most prevalent ransomware we encountered, at 16% of engagements. The next most frequent were the three Rs – Ryuk, REvil, and Ragnarok – who together accounted for the next 28% of attacks. Among the remaining 56% of incidents, we encountered ransomware under 39 different names.*

The figure is a pie chart showing the ransomware families investigated by Sophos Rapid Response in 2020-2021. The largest slice is "All other ransomware (35 different 'families')" at 43%. Other slices include "Conti" (16%), "REvil" (15%), "Ryuk" (9%), "LockBit" (4%), "Ragnarok" (4%), "Black Kingdom" (3%), "DarkSide" (3%), and "Maze" (3%).
...
*Figure 3. Atom Silo, like many ransomware threat groups, engages in extortion with a threat of leaking sensitive information, as well as maliciously encrypting files*

The figure is a screenshot of a ransom note from the Atom Silo ransomware group. The note includes a threat to leak sensitive information if the ransom is not paid.
...
*Figure 4. Beacons are a key feature of the Cobalt Strike attack suite, providing a capable backdoor to Windows machines. The malware appears as a payload of “conventional” malware such as Trickbot, IcedID, or BazarLoader, and features prominently in hands-on-keyboard attack incidents investigated by Sophos Rapid Response.*

The figure is a line graph showing the increasing popularity of Cobalt Strike Beacons among attackers from October 2020 to August 2021. The graph shows a significant increase in the percentage of attacks involving Cobalt Strike Beacons, peaking at 20% in March 2021.
...
*Figure 5. Gootloader malware relies on the effectiveness of its ability to poison Google search results in order to spread, and a few weeks after the March 1, 2021 publication of our report about the malware group’s activities, we saw a sharp drop in the number of machines with either a detection of the malware loader or the “reflective loading” behavior it engages in to filelessly infect machines.*

The figure is a line graph showing Gootloader detections dropping after the 2021 report publication. The graph shows a sharp decline in detections of both AMSI/GootLdr-A and AMSI/Reflect-H malware after March 1, 2021.
...
*Figure 6. Gootloader attacks begin when the victim searches for terms the attackers have “poisoned” in Google results, usually involving legal documentation. The malicious SEO promotes web sites the attackers control high in the result rankings, delivering visitors to those sites into a trap that looks exactly like this contrived “message board,” which delivers the infectious payload.*

The figure is a screenshot of a fake message board used by Gootloader to deliver malware. The message board appears to be a forum where users discuss legal documents.
...
The figure is a line graph showing the relationship between the size of a neural network and its ability to solve programming problems. The graph shows that as the number of parameters in the neural network increases, the percentage of problems solved also increases.
...
The figure is a tweet from Ian Goodfellow, the inventor of GANs, showing a synthesized image of a person that looks real.
...
*Figure 9. The Flubot malware arrives in the form of a text message that appears to originate from a large, international delivery firm like DHL or UPS, or sometimes from a service