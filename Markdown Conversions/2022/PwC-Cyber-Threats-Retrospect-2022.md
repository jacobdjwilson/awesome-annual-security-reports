...
and writes it to the users machine. With a log of a user receiving an email attachment of a single 
HTML file, a chained detection can be built upon the subsequent execution of an HTML file by a 
browser, where the file originates from the users downloads directory. 
...
also used to post recruitment adverts for EmployeesInsiders at several major technology, 
...
https:unit42\.paloaltonetworks.combrute\-ratel\-c4\-tool (5th July 2022\) 
...
memoryon\-disk \- for example using YARA \- or network traffic that features the default SSL 
...
reference \= "https:github.comBishopFoxsliver" 
strings: 
$ \= ".sliverpb." 
condition: 
...
interaction and system interdiction as possible. With Microsofts 2022 update to disable 
...
malicious payloads.
193 Macros from the internet will be blocked by default in Office, Microsoft, https:learn.microsoft.comen\-
usdeployofficesecurityinternet\-macros\-blocked (11th October 2022\) 
...
exploiting a dormant account to gain access to the victims Microsoft Azure Active 
...
the victims Microsoft Azure AD and O365 tenant, effectively providing the threat actor with 
administrative access to the victims Microsoft cloud environment. 
...
access and exfiltrate the emails of all of the victims O365 user accounts. Blue Dev 5 
...
custom piece of malware designed to gather information about the victims EDR, and RDP 
Facilitator, a piece of malware used to configure backdoors to facilitate RDP. To reinforce 
its foothold in the victims network, White Baku installed the GoToAssist remote support 
...
network, and followed by learning more about the victims environment. Defenders can search 
...
remote administration, PsExecs ability to execute code on remote systems over Server Message 
...
operation. MITRE ATT\&CK T1021\.002 \- Remote Services: SMBWindows Admin Shares 
64\|Cyber Threats 2022: A Year in Retrospect 

Collection: Once more, its important to stress that whilst compression tools like WinRAR and
...
achieved persistent access to the victims environment and returned to the network to 
...
Our review of surviving evidence led us to assess the initial compromise into the victims 
...
208 Dtrack activity targeting Europe and Latin America, Kaspersky, https:securelist.comdtrack\-targeting\-europe\-
latin\-america107798 (15th November 2022\) 
...
Often in combination with Flynet, an open source Go TCPUDP proxying tool for
Windows; and, 
...
antivirus present in the victims environment. We assess Andariel used these tools likely to 
proxy traffic in and out of the victims network. This is consistent with other Andariel activity 
...
209 Lazarus and the tale of three RATs, Cisco Talos, https:blog.talosintelligence.comlazarus\-three\-rats (8th 
September 2022\) 
...
Autostart Execution: Registry Run KeysStartup Folder, T1543\.003 \- Create or Modify System 
Process: Windows Service 

Lateral Movement: Please see this section in the White Baku case study above.
MITRE ATT\&CK T1021\.002 \- Remote Services: SMBWindows Admin Shares 
...
https:symantec\-enterprise\-blogs.security.comblogsthreat\-intelligencestonefly\-north\-korea\-espionage (27th April 
1\. 67\|Cyber Threats 2022: A Year in Retrospect 
...
that vulnerable Internet\-of\-Things (IoT) and small officehome office (SOHO) devices will 
continue to be some ofthe primary targets for exploitation and co\-option systems run by 
...
retrospectyir\-cyber\-threats\-report\-download.pdf (28th April 2022\).
69\|Cyber Threats 2022: A Year in Retrospectuse of wipers, pursuit of long\-standing destruction operations against industrial control 
systems (ICS) and evolving techniques that could include data alteration\-style attacks.Threat actor\-specific targeting anticipated in 2023With Russia's protracted war in Ukraine and fallout of diplomatic relations across much of 
...
experiences significant supply chain challengesakin to industrial espionage targeting. 
Russia\-based threat actors will also continue to show interest in government, defence and 
related entities as part of longstanding espionage operations.We expect China\-based threat actors to increase their targeting operations against the 
...
activities will be an area to watch.Iran\-based threat actors will continue targeting sectors relevant to the Iranian regime, as 
...
continuing their tempo for inward targeting against domestic targets and dissidents.We anticipate Western\-based actions emanating from countries like the United States will 
likely remain consistent and reflect geopolitical issues, as seen in the USs admission in 
...
Malware delivery system: A criminal service which uploads a customers
...
years by CEOs in PwCs 26th Annual Global CEO Survey. In 2022, cyber threat actors 
...
landscape evolved and the risks increased, PwCs 2023 Global Digital Trust Insights 
...
1\. Ransomwares evolution and outlook \- pg. 39\-48
...
1\. Sabotage operations escalate \- pg. 1\-2, 68\-69
...
1\. Multifactor Authentication (MFA) bypassingevasion and fatigue \- pg. 49\-52,
58\-60
...
1\. Targeting digital identity and privileged access \- pg. 1, 58\-60, 68

Protecting identity and privileged access is the single highest priority to
secure an organisations environment and data.
...
1\. Cloud environments in the crosshairs \- pg. 59\-60
...
Oil and gas \- pg. 30, 36214 CTO\-SIB\-20230223\-01A \- Sector shifts and insights \- 2022 
...
36\-37More from the PwC Threat Intelligence team: 
...
View our talk at Virus Bulletin 202280\|Cyber Threats 2022: A Year in Retrospect 
Appendix D \- Defender index 
...
1\. Endpoint: In todays decentralised, cloud\-native environment, having effective
...
1\. Network: Whilst Transport Layer Security (TLS) remains a challenge, almost all
...
lateral movement.
1\. Security Information and Event Management (SIEM)Security Orchestration,
...
the essence.
1\. YARA: Whilst rarely used for real time detection, YARA is incredibly useful for
...
81\|Cyber Threats 2022: A Year in RetrospectCommon Vulnerabilities and Exposures (CVEs) cited in this report 
...
breaks into a victims network and encrypts the network, first extorting the victim to 
regain access to their network, and then extorting the victim again when threatening to 
sell or leak the victims stolen data. \- pg. 39, 40 (visual of typical attack chain) 
...
using legitimate, dual\-use tools while inside a victims environment, such as admin 
services and forensic tools, and these tools are also referred to as LOL binaries 
(LOLBins). \- pg. 36 
82\|Cyber Threats 2022: A Year in RetrospectLLVM\-based obfuscation: In the context of this report, LLVM\-based obfuscation refers to an 
anti\-analysis technique where a threat actor uses LLVM to obfuscate malware code. \- 
pg. 20, 25 
LNKshortcut file: File extension denoting a Windows shortcut, or link, and used by threat 
actors to masquerade as legitimate documents and execute malicious payloads. \- pg. 
22, 25, 32, 46\-47, 56\-58 
Macros and threat actor responses to Microsofts default disabling of Mark of the Web 
(MotW)215 \- pg. 46\-48, 56
Microsoft Installer (MSI) exploitation \- pg. 32, 34, 47 
Multifactor authentication (MFA) bypassingevasion and fatigue \- pg. 49\-52, 58\-60 
...
Typosquatting (Yellow Liderc example) \- pg. 30Detection logic and methods 
...
Windows Defender disabling (Bumblebee, IcedID and Qakbot examples) \- pg. 48215 Macros from the internet will be blocked by default in Office, Microsoft, https:learn.microsoft.comen\-
usdeployofficesecurityinternet\-macros\-blocked (11th October 2022\) 
...
Red Scyllas (a.k.a. a.k.a. CHROMIUM, ControlX, Earth Lusca, Aquatic Panda)
...
T1547\.001 \- Boot or Logon Autostart Execution: Registry Run KeysStartup Folder \-
pg. 66
