...
We also power all Cisco Secure services with the threat intelligence of Cisco Talos. Talos is the largest non-governmental threat research organization in the world, made up of an elite group of security experts.
...
Since most organizations don’t bother to secure the DNS layer, they might be missing opportunities to block cyberattacks. A recent report by Global Cyber Alliance explored the “Value of DNS Security”, and found that:
...
* [Security Report](#security-report)
    * [2022 DNS Discoveries](#2022-dns-discoveries)
        * [Using DNS to Uncover Trends and Protect Against Threats](#using-dns-to-uncover-trends-and-protect-against-threats)
...
The malware campaign Masslogger focuses on business users and is delivered by email. The malicious email contains an RAR file with a compiled HTML (.chm) attachment. When the user opens the attachment, it launches the chain of exploitation, which results in MassLogger malware being installed on the workstation.
...
*Image description: A diagram shows the attack flow of Masslogger. It starts with a "Weaponized RAR file" which is part of "Malspam". This leads to a "First stage CHM file with JS", which then leads to a "Second stage PS script". This then causes "Data extraction" and "Downloads MassLoger". The text "Cisco Umbrella" is above the attack flow.*
...
*Image description: A diagram shows the attack flow of Conti ransomware. It starts with "Phishing" which leads to "Malicious JS". This then leads to "IcedID DDL" which then leads to "Cobalt Strike Beacon". This then leads to "RDP" and "Data Exfiltration" and finally "Data Encryption". The text "Cisco Umbrella" is above the attack flow.*
...
When a user tries to unsubscribe, the page delivers a malicious Excel or Word document, which ultimately downloads BazarLoader. This malware strain uses C2 domains with the .bazar and hosting that is designed to make it very difficult, if not impossible, for law enforcement to take over these domains.
...
**Potential damage:**
Can deliver wide variety of secondary payloads, such as ransomeware, infostealers, or tools providing further control of network.

*Image description: A diagram shows the attack flow of BazarLoader. It starts with "Malspam" which leads to "Social Engineering". This then leads to "Downloads Weaponized Documents" which then leads to "Downloads BazarLoader" and finally "C&C". The text "Cisco Umbrella" is above the attack flow.*
...
1. A vulnerability is discovered in software frequently found running on publicly accessibile company servers.
...
*Image description: A diagram shows the process of detecting newly seen domains. It starts with "Attacker registers domain". Then "1. Any user (free or paid) requests the domain". Then "2. Every minute, we sample from our streamine DNS logs". Then "3. Check if domain was seen before and if allowlisted". Then "4. If not, add to category, & within minutes DNS resolvers update globally". This leads to "Before expiration if any user requests this domain, it's logged or blocked as newly seen". This is marked as "protected" and "Minutes". Then "Later, Talos intelligence models or reputation systems identify as malicious". This is marked as "protected" and "Days to Weeks". The text "Cisco Umbrella" is above the flow. There is also a section marked "not yet a threat" and "potentially unprotected" which leads to "24 Hours" and "Reputation Systems" which leads to "unprotected".*
...
*Image description: A diagram shows the process of DNS tunneling. It starts with an "Attacker" sending an "Encoded command: Collect credentials" to "Authoritative DNS" which is a "Nameserver for example.com". This then sends "aop1.18-ququ.example.com" to "Recursive DNS". This then sends "aop1.18-ququ.example.com" to "Malware with Tunneling client". The malware sends "Encoded credentials: eui8" back to "Recursive DNS" as "eui8.18-ququ.example.com" which then sends "eui8.18-ququ.example.com" to "Authoritative DNS" which then sends "eui8" to the "Attacker". The text "username" and "password" is next to the attacker. Below the diagram is "Encoded information in DNS communications: {“qname”: “vaaaakaw0i2.iodine.umbrella-tunnel-test.com.”, qtype”: 1} {“qname”: “vaaaakaw0ja.iodine.umbrella-tunnel-test.com.”, qtype”: 1} {“qname”: “lafbgb04eecdkobxayfjgfpsugf3nusi. iodine.umbrella-tunnel-test.com.”, qtype”: 1} {“qname”: “lafbgb04eecdkobxayfjgfpsugf3nusq. iodine.umbrella-tunnel-test.com.”, qtype”: 1} {“qname”: “ytbwsl.iodine.umbrella-tunnel- test.com.”, qtype”: 1} {“qname”: “ytbwsm.iodine.umbrella-tunnel- test.com.”, qtype”: 1}"*
...
>
> Brett Stone, Network Operations Manager Cape Air
...
>
> Brandon Wood, Information Security Officer University of Alaska Anchorage
