...
![Diagram showing the Masslogger attack chain: Malspam -> Weaponized RAR file -> First stage CHM file with JS -> Second stage PS script -> Downloads MassLoger -> Data extraction](Diagram showing the Masslogger attack chain: Malspam -> Weaponized RAR file -> First stage CHM file with JS -> Second stage PS script -> Downloads MassLoger -> Data extraction)

**Potential damage**: Can steal credentials from email clients, messaging applications, browsers, VPN, and FTP clients, which can be used to take over further systems.
...
![Diagram showing the Conti ransomware attack chain: Phishing -> Malicious JS -> IcedID DDL -> Cobalt Strike Beacon -> RDP -> Data Exfiltration -> Data Encryption](Diagram showing the Conti ransomware attack chain: Phishing -> Malicious JS -> IcedID DDL -> Cobalt Strike Beacon -> RDP -> Data Exfiltration -> Data Encryption)
...
![Diagram showing the BazarLoader attack chain: Malspam -> Social Engineering -> Downloads Weaponized Documents -> Downloads BazarLoader -> C&C](Diagram showing the BazarLoader attack chain: Malspam -> Social Engineering -> Downloads Weaponized Documents -> Downloads BazarLoader -> C&C)

**Potential damage**: Can deliver wide variety of secondary payloads, such as ransomeware, infostealers, or tools providing further control of network.
...
![Diagram showing the lifecycle of a newly seen domain: Attacker registers domain -> Any user (free or paid) requests the domain -> Every minute, we sample from our streamine DNS logs -> Check if domain was seen before and if allowlisted -> If not, add to category, & within minutes DNS resolvers update globally -> Domain used in an attack -> Before expiration if any user requests this domain, it's logged or blocked as newly seen -> Later, Talos intelligence models or reputation systems identify as malicious](Diagram showing the lifecycle of a newly seen domain: Attacker registers domain -> Any user (free or paid) requests the domain -> Every minute, we sample from our streamine DNS logs -> Check if domain was seen before and if allowlisted -> If not, add to category, & within minutes DNS resolvers update globally -> Domain used in an attack -> Before expiration if any user requests this domain, it's logged or blocked as newly seen -> Later, Talos intelligence models or reputation systems identify as malicious)
...
![Diagram showing a DNS tunneling example: Malware with Tunneling client -> Recursive DNS -> Authoritative DNS -> Attacker. The diagram illustrates encoded information in DNS communications, with examples of encoded credentials and commands.](Diagram showing a DNS tunneling example: Malware with Tunneling client -> Recursive DNS -> Authoritative DNS -> Attacker. The diagram illustrates encoded information in DNS communications, with examples of encoded credentials and commands.)
