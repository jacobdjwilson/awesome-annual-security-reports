...
![Table showing discovered vulnerabilities in Azure and AWS, including AutoWarp, SynLapse, Superglue, and BreakingFormation with their respective severities and remediation times.]
...
- **33%** of organizations have a cloud provider root account without multi-factor authentication.
...
- **On average, organizations take 18 days** to fix an ‘imminent compromise’ security alert.
...
![Diagram showing the Shared Responsibility Model for IaaS, PaaS, and Serverless, illustrating the division between customer-managed and provider-managed components.]
...
- Authentication: 6 days
- Data at risk: 13 days
- Network misconfigurations: 25 days
- Malicious IAM activity: 7 days
- Neglected assets: 27 days
- Vulnerabilities: 22 days
- Lateral movement: 47 days
...
- 51% have a Google Storage bucket without bucket-level access.
...
- **56% of organizations on AWS**, **11% on Azure**, and **55% on GCP** have sensitive keys on their system.
...
- **6% of the unresolved Log4j vulnerabilities** potentially expose PII.
...
- **8.2 Top initial attack vectors**: The vast majority of attacks start by exploiting a vulnerability. Neglected assets account for nearly one fifth of initial attack vectors.
...
- 39% of Google Cloud environments have at least one publicly-accessible Google storage bucket.
...
- 73% do not use logging services for their databases.
...
- 28% have at least one container with the Spring4Shell RCE vulnerability.
...
- 37% have at least one serverless function with admin privileges.
- 58% have an AWS Lambda function or Google Cloud function with unsupported runtimes.

---

## 10. Key Recommendations
1. **Identify your crown jewels**: Identify which of your cloud assets are business critical and/or contain sensitive data.
2. **Maintain a cloud asset inventory**: You can only patch a vulnerability if you know it exists.
3. **Patch, patch, patch**: Whenever possible, systems with known vulnerabilities should be patched.
4. **Adhere to the Principle of Least Privilege**: Ensure that administrator privileges are only given to those who really need them.
5. **Encrypt sensitive data and keys**: Use encryption for critical assets to limit the impact of potential data and key exposure.
6. **Apply MFA and strong password management**: Always implement Multi-Factor Authentication (MFA) where possible.
7. **Secure templates and images**: Ensure that Infrastructure as Code (IaC) templates and container images are checked for misconfigurations.
8. **Eliminate unused assets**: Make sure that any users, files or systems that are no longer being used are deleted.
9. **Perform regular audits**: Perform cloud configuration audits at least twice a year.
10. **Perform backups**: Regularly backup critical data and store offline if possible.
11. **Utilize checklists**: To minimize human error, use checklists when creating and configuring cloud assets.
