...
### Ransomware
For more than three years, ransomware has been the top attack type observed by X-Force, and 2021 was no exception. Twenty-one percent of attacks remediated by X-Force incident response in 2021 were ransomware attacks.

> "Many ransomware actors have continued operations under new names"

According to X-Force research, 17 months is the average time before a ransomware group either rebrands or shuts down. X-Force is aware of many ransomware actors that have rebranded and continued operations under new names, with GandCrab to REvil, Maze to Egregor, and DoppelPaymer to Grief as examples.
...
X-Force has developed a five-stage model that defines the common pattern observed in most ransomware incidents:
1. **Initial access**: Phishing email or exploit of internet-facing service.
2. **Post exploitation**: Second stage malware/post-exploitation toolkit; interactive attacker establishes access.
3. **Understand and expand**: Gather credentials, local reconnaissance, Active Directory reconnaissance.
4. **Data collection and exfiltration**: Data exfiltration (WinSCP, Rclone, etc.).
5. **Ransomware deployment**: Stage ransomware on share, deploy using domain administrator credentials.
...
After a downturn in business email compromise (BEC) attacks in 2020, X-Force observed a decrease in this attack type again in 2021. BEC was the third-most common attack type remediated by our X-Force IR team.
...
### Phishing
Phishing emerged as the top infection vector in 2021, observed in 41% of the incidents X-Force remediated.

### Phishing kit deployments short-lived, abuse technology and bank brands
IBM analyzed thousands of phishing kits from around the world. Our investigation showed:
- Phishing kit deployments generally had a short lifespan, with almost one-third of deployed kits being used for no longer than a day.
- Each deployment on average had no greater than 75 potential victims.
- Phishing kits asked for user credentials in nearly every kit observed (close to 100%).
...
Despite dropping to the second-most common in 2021, the number of incidents that were caused by vulnerability exploitation this past year rose 33% from 2020.

**Top 10 vulnerabilities of 2021**:
1. CVE-2021-34523 (ProxyLogon)
2. CVE-2021-44228 (Log4j)
3. CVE-2021-26857 (Microsoft Exchange)
4. CVE-2020-1472 (Netlogon)
5. CVE-2021-27101 (Accellion FTA)
6. CVE-2020-7961 (Liferay Portal)
7. CVE-2020-15505 (MobileIron)
8. CVE-2018-20062 (NoneCMS ThinkPHP)
9. CVE-2021-35464 (ForgeRock AM)
10. CVE-2019-19781 (Citrix Server)

## Threats to operational technology and Internet of Things

### Threat actors accelerate reconnaissance against OT devices
Between January and September of 2021, X-Force observed a 2,204% increase in adversarial reconnaissance activity targeting port 502 (Modbus).

### Manufacturing most targeted of OT industries
Manufacturing was the most attacked in 2021, victimized in 61% of incidents X-Force assisted in remediating.

### Mozi botnet continues to threaten IoT and OT assets
The Mozi botnet continued to make up the most significant volume of IoT malware, at 74% of the total volume of IoT malware X-Force observed in 2021.

## Top threat actors of 2021

- **Criminals**: 97% of engagements.
- **Nation-state**: 2% of engagements.
- **Hacktivist**: 1% of engagements.

- **ITG17 (MuddyWater)**: Suspected Iranian nation-state group using the "Aclip" backdoor.
- **ITG23 (Trickbot Gang)**: Enables Conti ransomware operations.
- **Hive0109 (LemonDuck)**: Adept at leveraging ProxyLogon vulnerabilities; targets both Linux and Windows.

## Trends in malware development

- **Next-level detection evasion**: Ransomware authors are using intermittent encryption and cloud messaging services for C2 communications.
- **Malware focus on Docker**: Malware families like XorDDoS, Groundhog, and Tsunami are shifting focus to Docker containers.
- **Ransomware focus on ESXi**: Ransomware families like REvil, HelloKitty, and BlackMatter are targeting Linux-based VMWare ESXi servers.
- **Nim is in**: Threat actors are increasingly using the Nim programming language to compile malware.

## Geographic trends

Asia moved into first place as the most-attacked region in 2021, receiving 26% of attacks.

- **Asia**: 26% of attacks. Top industry: Finance and insurance.
- **Europe**: 24% of attacks. Top industry: Manufacturing.
- **North America**: 23% of attacks. Top industry: Manufacturing.
- **Middle East and Africa**: 14% of attacks. Top industry: Finance and insurance.
- **Latin America**: 13% of attacks. Top industry: Manufacturing.

## Industry trends

For the first time in over five years, finance and insurance was not the top-attacked industry, as manufacturing overtook it.

1. **Manufacturing**: 23.2%
2. **Finance and insurance**: 22.4%
3. **Professional and business services**: 12.7%
4. **Energy**: 10.2%
5. **Retail and wholesale**: 6.6%
6. **Healthcare**: 5.1%
7. **Transportation**: 5.1%
8. **Government**: 2.8%
9. **Education**: 2.8%
10. **Media**: 4.0%

### #1 | Manufacturing
Taking first place in X-Force’s ranking for the first time since 2016, manufacturing was the top attacked industry in 2021, targeted in 23.2% of the attacks X-Force remediated. Ransomware was the top attack type.

---

acturing organizations and underscoring the heavy focus ransomware
...
implementation easier every year—both for implementation teams and for end-users.
