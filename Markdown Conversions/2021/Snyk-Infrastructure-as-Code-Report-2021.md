...
- **76%** - Teams with fully automated security checks
- **59%** - Teams with no or only partially automated security checks
- **38%** - Teams that only check security after deployment
...
| In use today | Considering for the future |
| --- | --- |
| AWS CloudFormation (36%) | Cloud SDKs (AWS CDK, Azure SDKs) (26%) |
| Azure Resource Manager (30%) | AWS CloudFormation (24%) |
| Kubernetes (incl. YAML, JSON and Helm) (25%) | Kubernetes (incl. YAML, JSON and Helm) (22%) |
| Ansible (17%) | Azure Resource Manager (18%) |
| Terraform (14%) | Google Cloud Deployment Manager (18%) |
| Docker Compose (14%) | Other Kubernetes tools (16%) |
| Google Cloud Deployment Manager (14%) | Serverless Framework (14%) |
...
- Fully automated: **34%**
- Not automated: **14%**
- Checks after deployment: **18%**

**Can you fix an issue in less than 1 day?**
- Fully automated: **76%**
- Not automated: **59%**
- Checks after deployment: **38%**

**How often do you go 1 week or longer before finding an issue?**
- Fully automated: **30%**
- Not automated: **54%**
- Checks after deployment: **60%**

**How often does it take you over a day to fix an issue?**
- Fully automated: **25%**
- Not automated: **41%**
- Checks after deployment: **62%**

---

## Does Speed Equate to Safety?

Currently, modern applications deploy automatically on infrastructure created and configured via code. As a result, security takes a back seat to a speedy deployment, meaning configuration issues are not uncovered until after these applications have been deployed. Even Gartner* states, “By 2025, 70% of attacks against containers will be from known vulnerabilities and misconfigurations that could have been remediated.”
...
- **45%** Audit running environments after deployment
- **43%** Pen testing
- **35%** Manual code scans
- **33%** From investigating security incidents
- **32%** Automated testing/CI
- **21%** Tools from our IaC or public cloud provider
...
- **62%** Each team makes their own decisions about what to test against
- **41%** We do not have a clear set of benchmarks on what to test
- **22%** We do not have the right testing tools
- **22%** We have not decided what is important for us to test
- **16%** Concerned it would slow us down too much
- **11%** There is no clear owner to address issues that are discovered
...
- **13%** Never - we always fix the code first
- **34%** Most of the time - we usually change the infrastructure directly instead of modifying the source code
- **18%** Often
- **35%** Rarely

**Why do you directly modify the infrastructure instead of fixing the code?**
- **52%** Faster/easier to tweak the infrastructure
- **39%** Lack of standardized workflow and practices
- **38%** Concern that redeploying from code will create new issues
...
- **9%** No automated tests to ensure the IaC changes work before
...
- **67%** Professional training
- **48%** Automated code testing for IaC in CI/CD
- **42%** Audit tools specific to IaC and configuration
- **37%** Playbooks to follow and vendor benchmarks
- **29%** Tools built-in IDEs
- **26%** Peer mentoring
- **22%** Industry or infrastructure
...
- **31%** Infrastructure Architects
- **30%** Developers and DevOps
...
![Image description: Promotional call to action showing Snyk IaC in action]
