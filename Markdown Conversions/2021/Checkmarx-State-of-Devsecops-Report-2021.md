...
The scary statistic is that 44% measure success based on security vulnerability identification, not remediation.

About the same number (42%) are achieving release speed and quality by integrating security testing and automation, typically in a CI/CD pipeline that is highly automated otherwise.
...
- **SCM integration (Webhooks on pull request / pushes)**: 18%
- **Nightly / Weekly cronjobs**: 15%
...
Jira tickets are the top means of communicating with developers (48% / 50%) since so many DevOps teams already use it.
