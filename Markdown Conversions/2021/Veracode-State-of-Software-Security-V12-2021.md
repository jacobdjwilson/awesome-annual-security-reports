...
## Section 01: The State of Software Security at a Glance
...
*   **2011**: [Image: A bar chart showing the increase in new applications scanned per quarter from 2007 to 2021. The number of applications scanned per quarter in 2011 is approximately 5.]
*   **2021**: [Image: A bar chart showing the increase in new applications scanned per quarter from 2007 to 2021. The number of applications scanned per quarter in 2021 is approximately 17.]
...
*   In 2018, roughly 20 percent of applications incorporated multiple languages. This year, less than 5 percent of apps used multiple languages, suggesting a pivot to smaller, one-language applications or microservices.
    *   **2018**: [Image: A line graph showing the percentage of multi-language applications over time. In 2018, the percentage is around 20%.]
    *   **2021**: [Image: A line graph showing the percentage of multi-language applications over time. In 2021, the percentage is below 5%.]
...
*   **2010**: [Image: A bar chart showing the median scan cadence. In 2010, the median scan cadence was less than once a month.]
*   **2021**: [Image: A bar chart showing the median scan cadence. In 2021, 90% of applications are scanned more than once a week, with the majority scanned three times a week.]
*   **20X INCREASE IN MEDIAN SCAN CADENCE FROM 2010 TO 2021**

### Flaw Prevalence

*   Third-party libraries have fewer flaws.
    *   **35%** of libraries used had a known flaw (2017)
    *   **10%** of libraries used had a known flaw (2021)
...
*   **2017**: [Image: A bar chart showing the percentage of organizations using multiple scan types. In 2017, the percentage is lower.]
*   **2021**: [Image: A bar chart showing the percentage of organizations using multiple scan types. In 2021, the percentage is higher, showing a 31% increase.]
*   **31% INCREASE in the use of multiple scan types**

### Third-Party Libraries

*   77% of flaws in third-party libraries remain unfixed after three months.
*   On a positive note, there is a noticeable improvement in time to remediation for third-party flaws. Back in 2017, it would take over three years to get to the 50 percent (half-life) closed point, and now it takes just over a year.
    *   **2017**: [Image: A timeline showing the half-life for fixing third-party flaws. In 2017, it was over 3 years.]
    *   **2021**: [Image: A timeline showing the half-life for fixing third-party flaws. In 2021, it is just over 1 year.]
...
*   **35% DECREASE in the time it takes for organizations to fix flaws**

### Open Source

*   Open-source libraries are still a significant cause for concern.
*   **97%** of Java applications are made up of open source libraries.
...
## Section 03: How Software Development Has Changed
...
[Image: Figure 1: Application creation over time. A line graph showing the number of new applications scanned per quarter from 2007 to 2022. The trend shows a significant increase, reaching over 17 applications per quarter by 2021.]
...
[Image: Figure 2: Application criticality over time. A stacked area chart showing the percentage of applications by criticality (Very Low, Low, Medium, High, Very High) from 2010 to 2021.]
...
[Image: Figure 3: Use of multiple languages in new applications. A line graph showing the percentage of multi-language applications over time. It shows a peak around 2018 and a subsequent sharp decline.]
...
[Image: Figure 4a: Application size over time. Multiple line graphs showing the mean size (MB) of applications written in Java, C++, PHP, JavaScript, Python, and .NET from 2009 to 2021.]
...
*   **SCALA**: Scala applications (not shown) have seen a decline in size, and the popularity of Scala compared to its more heavyweight godfather Java may have something to do with different architectural goals.
*   **GO**: Interestingly, Go (not shown), a language commonly associated with microservices, has actually seen an increase in application size.
...
[Image: Figure 4b: Android’s abrupt size shift. A line graph showing the scan size (MB) of Android applications over time.]

### Increase in Median Scan Cadence

“It is no longer sufficient to scan software as a pre-production step in the last phase of the software development lifecycle. Just as software is now deployed continuously, software security scanning must also happen continuously as a fully integrated part of the software development process.” - SAM KING, CEO, VERACODE
...
[Image: Figure 5: Scanning cadence over time. Multiple line graphs showing the mean time between scans (days) for Manual, Dynamic, Static, and SCA Agent scans from 2010 to 2022. The trend shows a significant decrease in time between scans.]

*   **Median Scan Cadence**:
    *   **2010**: Median application was scanned less than once a month (only 10 percent of apps scanned more often than weekly).
    *   **2021**: 90 percent of apps scanned more than once a week (majority scanned three times a week).
*   **20X increase**
...
Organizations that used dynamic in addition to static scanning were able to remediate 50% of flaws On average 24 days faster.

And including SCA shaves off another 6 days.

So fire up those scanners, folks!

## Section 04: Software Bill of Mistakes
...
## Section 04: Organizations Heavily Leverage Open-Source Libraries
...
[Image: Figure 6: Third-party code by language. Stacked area charts showing the proportion of third-party code in .NET, Java, PHP, JavaScript, Python, and C++ applications from 2019 to 2021.]
...
[Image: Figure 7: Popular libraries by language. Stacked area charts showing the relative popularity of the top 10 libraries for .NET, Java, PHP, JavaScript, Python, and Ruby from 2018 to 2021.]
...
[Image: Figure 8: Percent of flawed libraries by language. Line graphs showing the percentage of vulnerable libraries in .NET, Java, PHP, Python, JavaScript, and Ruby from 2017 to 2021.]

## Section 05: The Flaws of Yesterday Are (Still) the Flaws of Today
...
[Image: Figure 9: Percent of applications with various flaw types in static analysis. Line graphs showing the percentage of applications with "Any Flaws", "OWASP", and "CWE/SANS Top 25", and "High Severity" flaws from 2016 to 2020.]
...
*   **Static Analysis**: Static analysis looks directly at the source code and thus needs to know what to look for in different languages. It’s very strong at detecting places where maybe memory isn’t managed correctly or input isn’t properly validated or sanitized.
*   **Dynamic Analysis**: Dynamic scanning takes advantage of, and is run against, the runtime environment. It doesn’t dive deeply into the idiosyncrasies of the underlying languages but instead can find more flaws in the execution and interface of the code.
*   **Software Composition Analysis**: Software composition analysis is the third type of scan, and it operates by tracking the various open-source projects and packages and then identifying which are included in the code base. This allows all the existing information in those libraries to be shared with the developers.

[Image: Figure 10: Top software weaknesses discovered by scan type. Bar charts showing the prevalence of various flaw types for Static Analysis, Dynamic Analysis, and SCA.]
...
*   **JAVASCRIPT**: Has some growing challenges with identity management: Both credentials management and authentication issues are trending upward over time.
*   **JAVA**: And the overall declines across Java are clearly more pronounced than the very subtle declines seen in PHP applications.

[Image: Figure 11: Software weaknesses found by static analysis by language. Multiple bar charts showing the proportion of applications with various flaw types for .NET, C++, Java, PHP, JavaScript, and Python from 2017 to 2021.]

#### Dynamic Analysis

Notice the top types of flaws here generally don’t overlap with the static analysis findings. Server configurations and information leakage were consistently the leading categories of flaws found across all the underlying languages. The flaws were so consistent across languages that it’s not worth showing the differences. Each individual language looked like the overall trends shown in Figure 12.

[Image: Figure 12: Software weaknesses found by dynamic analysis. A bar chart showing the prevalence of various flaw types for dynamic analysis from 2017 to 2021.]

#### Software Composition Analysis

The flaws in third-party software can be reported from a variety of sources such as static code scans, manual code review, security researchers reporting flaws, etc. Once again, we find that these types of flaws vary by language, as you can see in Figure 13 (but note that the vertical axis uses different scales for different languages). Some languages (Java, JavaScript, and Python) show clear declines while .NET and C++ don’t appear to show that type of decline in their third-party libraries.

[Image: Figure 13: Software weaknesses found by SCA, by language. Multiple bar charts showing the prevalence of various flaw types for SCA in Java, JavaScript, Python, .NET, C++, and PHP from 2017 to 2021.]

## Section 05: Fix Rate Comparisons by Scan Type
...
[Image: Figure 14: Probability of flaw remaining open by scan type. A survival curve showing the probability of a flaw remaining open over time for Dynamic, SCA, and Static analysis.]
...
*   **57%** of flaws from static analysis are closed within the first 290 days.
*   **68%** of flaws from dynamic analysis are closed within the first 143 days.
*   **77%** of flaws in third-party libraries remain unﬁxed after the FIRST THREE MONTHS, while only 57% of ﬂaws found through dynamic analysis are still around.
...
[Image: Figure 15: Half-life by scan type. Line graphs showing the half-life (in days) for Dynamic Analysis, Static Analysis, and SCA from 2017 to 2021.]
...
Part of the challenge here is that the competitive market demands timely releases, and software development teams are forced to make difficult tradeoffs between a timely delivery and risk. Some flaws may be left unaddressed to meet deadlines.
...
## Section 05: Capacity for Flaw Remediation by Scan Type
...
[Image: Figure 16: Monthly capacity for flaw remediation by scan type. Line graphs showing the monthly capacity for flaw remediation for Dynamic Analysis, Static Analysis, and SCA from 2017 to 2021.]
...
## Section 06: Where Do We Go From Here?
...
Before you roll your eyes, we want you to know this isn’t just a pile of training videos that developers are