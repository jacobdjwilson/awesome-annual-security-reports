...
def get_existing_report_url(org):
    """
    Parse README.md to find the existing report URL for the given organization.
    Returns: URL string or None
    """
    try:
        with open("README.md", "r", encoding="utf-8") as f:
            content = f.read()
            # Pattern: - [Org Name](url) - [Report Title](url) (year)
            # We want to extract the Report URL (second URL in the pattern)
            pattern = rf'-\s*\[{re.escape(org)}\]\([^)]+\)\s*-\s*\[[^\]]+\]\(([^)]+)\)'
            match = re.search(pattern, content)
            if match:
                return match.group(1)
    except Exception as e:
        print(f"WARNING: Could not parse README for {org}: {e}")
    return None
...
def main():
    ...
    for org, report_title, last_year in targets:
        ...
        existing_report_url = get_existing_report_url(org)
        existing_report_domain = extract_domain(existing_report_url) if existing_report_url else None
